 ...
 * MMM | Abbreviation representation of month (available since 1.1.11; locale
 * aware since 1.1.13)
 ...
 * </pre>
 *
 * Locale specific patterns such as MMM and MMMM uses {@link CLocale} for
...
class CDateTimeParser {
	/**
	 *
	 * @var boolean whether 'mbstring' PHP extension available. This static
	 *      property introduced for
	 *      the better overall performance of the class functionality. Checking
	 *      'mbstring' availability
	 *      through static property with predefined status value is much faster
	 *      than direct calling
	 *      of function_exists('...').
	 ...
	 *        	the default values for year, month, day, hour, minute and
	 *        	second.
	 *        	The default values will be used in case when the pattern
	 *        	doesn't specify the
	 *        	corresponding fields. For example, if the pattern is
	 *        	'MM/dd/yyyy' and this
	 *        	parameter is array('minute'=>0, 'second'=>0), then the actual
	 *        	minute and second
	 *        	for the parsing result will take value 0, while the actual
	 *        	hour value will be
	 *        	the current hour obtained by date('H'). This parameter has
	...
	protected static function parseMonth($value, $offset, $width, &$monthName) {
		...
		for($len = 1; $offset + $len <= $valueLength; $len ++) {
			...
			if (! preg_match ( '/^[\p{L}\p{M}]+$/u', $monthName )) 			// unicode aware
			                                                 // replacement for
			                                                 // ctype_alpha($monthName)
			{
