...
class CTimestamp {
	...
	public static function getDayofWeek($year, $month, $day) {
		/*
		 * Pope Gregory removed 10 days - October 5 to October 14 - from the
		 * year 1582 and proclaimed that from that time onwards 3 days would be
		 * dropped from the calendar every 400 years. Thursday, October 4, 1582
		 * (Julian) was followed immediately by Friday, October 15, 1582
		 * (Gregorian).
		 */
		if ($year <= 1582) {
	...
	public static function isLeapYear($year) {
		...
		if ($year % 400 == 0)
			return true;
			// if gregorian calendar (>1582), century not-divisible by 400 is
		// not leap
	...
	protected static function digitCheck($y) {
		if ($y < 100) {
			...
			$c1 *= 100;
			// if 2-digit year is less than 30 years in future, set it to this
			// century
			// otherwise if more than 30 years in future, then we set 2-digit
			// year to the prev century.
			if (($y + $c1) < $yr + 30)
				$y = $y + $c1;
			else
				$y = $y + $c0 * 100;
	 ...
	 *        	second
	 * @param boolean $hs24
	 *        	whether the hours should be 0 through 23 (default) or 1
	 *        	through 12.
	 * @return boolean true if valid date, semantic check only.
	 */
	 ...
	 * This is the same as the PHP function {@link mktime
	 ...
	 * @return integer float timestamp given a local time.
	 */
