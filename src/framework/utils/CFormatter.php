 ...
 * <code>formatXyz</code>.
 * The behavior of some of them may be configured via the properties of
 * CFormatter. For example,
 * by configuring {@link dateFormat}, one may control how {@link formatDate}
 * formats the value into a date string.
 *
 * For convenience, CFormatter also implements the mechanism of calling
 * formatting methods with their shortcuts (called types).
 * In particular, if a formatting method is named <code>formatXyz</code>, then
 * its shortcut method is <code>xyz</code>
 * (case-insensitive). For example, calling
 * <code>$formatter->date($value)</code> is equivalent to calling
 ...
 * <li>ntext: the {@link formatNtext} method will be called to format the
 * attribute value as a HTML-encoded plain text with newlines converted as the
 * HTML &lt;br /&gt; or &lt;p&gt;&lt;/p&gt; tags.</li>
 * <li>html: the attribute value will be purified and then returned.</li>
 * <li>date: the {@link formatDate} method will be called to format the
 * attribute value as a date.</li>
 * <li>time: the {@link formatTime} method will be called to format the
 * attribute value as a time.</li>
 * <li>datetime: the {@link formatDatetime} method will be called to format the
 * attribute value as a date with time.</li>
 * <li>boolean: the {@link formatBoolean} method will be called to format the
 * attribute value as a boolean display.</li>
 * <li>number: the {@link formatNumber} method will be called to format the
 * attribute value as a number display.</li>
 * <li>email: the {@link formatEmail} method will be called to format the
 * attribute value as a mailto link.</li>
 * <li>image: the {@link formatImage} method will be called to format the
 * attribute value as an image tag where the attribute value is the image
 * URL.</li>
 * <li>url: the {@link formatUrl} method will be called to format the attribute
 * value as a hyperlink where the attribute value is the URL.</li>
 * <li>size: the {@link formatSize} method will be called to format the
 * attribute value, interpreted as a number of bytes, as a size in human
 * readable form.</li>
 * </ul>
 *
 * By default, {@link CApplication} registers {@link CFormatter} as an
 * application component whose ID is 'format'.
 * Therefore, one may call <code>Yii::app()->format->boolean(1)</code>.
 * You might want to replace this component with {@link CLocalizedFormatter} to
 * enable formatting based on the
...
class CFormatter extends CApplicationComponent {
	...
	/**
	 *
	 * @var string the format string to be used to format a date using PHP
	 *      date() function. Defaults to 'Y/m/d'.
	 */
	public $dateFormat = 'Y/m/d';
	/**
	 *
	 * @var string the format string to be used to format a time using PHP
	 *      date() function. Defaults to 'h:i:s A'.
	 */
	public $timeFormat = 'h:i:s A';
	/**
	 *
	 * @var string the format string to be used to format a date and time using
	 *      PHP date() function. Defaults to 'Y/m/d h:i:s A'.
	 */
	 ...
	 *      "thousandSeparator".
	 *      They correspond to the number of digits after the decimal point, the
	 *      character displayed as the decimal point
	...
	/**
	 *
	 * @var array the text to be displayed when formatting a boolean value. The
	 *      first element corresponds
	 *      to the text display for false, the second element for true. Defaults
	 *      to <code>array('No', 'Yes')</code>.
	 */
	...
	/**
	 *
	 * @var array the options to be passed to CHtmlPurifier instance used in
	 *      this class. CHtmlPurifier is used
	 *      in {@link formatHtml} method, so this property could be useful to
	 *      customize HTML filtering behavior.
	 * @since 1.1.13
	 */
	public $htmlPurifierOptions = array ();
	/**
	 *
	 * @var array the format used to format size (bytes). Three elements may be
	 *      specified: "base", "decimals" and "decimalSeparator".
	 *      They correspond to the base at which a kilobyte is calculated (1000
	 *      or 1024 bytes per kilobyte, defaults to 1024),
	 *      the number of digits after the decimal point (defaults to 2) and the
	 *      character displayed as the decimal point.
	 ...
	 * This is a PHP magic method that we override to implement the shortcut
	 * format methods.
	 * 
	 * @param string $name
	 *        	the method name
	 ...
	 * @param string $type
	 *        	the data type. This must correspond to a format method
	 *        	available in CFormatter.
	 *        	For example, we can use 'text' here because there is method
	 *        	named {@link formatText}.
	 ...
	 *        	whether newlines should be converted to HTML
	 *        	&lt;p&gt;&lt;/p&gt; tags,
	 *        	false by default meaning that HTML &lt;br /&gt; tags will be
	 *        	used
	 ...
	 *        	Defaults to false meaning that short names will be used (e.g.
	 *        	B, KB, ...).
	 * @return string the formatted result
	 * @see sizeFormat
	 * @since 1.1.11
