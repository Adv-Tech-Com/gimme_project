 ...
 * </pre>
 *
 * @property boolean $readOnly Whether this map is read-only or not. Defaults to
 *           false.
...
class CMap extends CComponent implements IteratorAggregate, ArrayAccess, Countable {
	 ...
	 * @throws CException If data is not null and neither an array nor an
	 *         iterator.
	 */
	 ...
	 * Note, if the specified key already exists, the old value will be
	 * overwritten.
	 * 
	 * @param mixed $key
	 *        	key
	...
	public function remove($key) {
		if (! $this->_r) {
			if (isset ( $this->_d [$key] )) {
				...
				// it is possible the value is null, which is not detected by
				// isset
				unset ( $this->_d [$key] );
				return null;
			}
	 ...
	 * 
	 * @param mixed $data
	 *        	the data to be copied from, must be an array or object
	 ...
	 *
	 * Existing elements in the map will be overwritten if their keys are the
	 * same as those in the source.
	 ...
	 * <li>if $a and $b both have an array indexed at the same string key, the
	 * arrays will be merged using this algorithm;</li>
	 * <li>any integer-indexed elements in $b will be appended to $a and
	 * reindexed accordingly;</li>
	 * <li>any string-indexed elements in $b will overwrite elements in $a with
	 * the same index;</li>
	 * </ul>
	 *
	 * @param mixed $data
	 *        	the data to be merged with, must be an array or object
	 *        	implementing Traversable
