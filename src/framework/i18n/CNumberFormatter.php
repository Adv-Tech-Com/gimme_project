 ...
 * a locale,
 * and thus generates the string representation of the number in a
 * locale-dependent fashion.
 *
 * CNumberFormatter currently supports currency format, percentage format,
 * decimal format,
 * and custom format. The first three formats are specified in the locale data,
 * while the custom
 ...
 * the {@link http://www.unicode.org/reports/tr35/ Unicode Technical Standard
 * #35}.
...
class CNumberFormatter extends CComponent {
	 ...
	 * Note, if the format contains '%', the number will be multiplied by 100
	 * first.
	 ...
	 *        	The currency placeholder in the pattern will be replaced with
	 *        	the currency symbol.
	 ...
	 * Note, if the percentage format contains '%', the number will be
	...
	protected function parseFormat($pattern) {
		...
		if (isset ( $patterns [1] ) && preg_match ( '/^(.*?)[#,\.0]+(.*?)$/', $patterns [1], $matches )) 		// with
		                                                                                      // a
		                                                                                      // negative
		                                                                                      // pattern
		{
