 ...
 * <li>Translations in one language are kept as PHP files under an individual
 * subdirectory
 * whose name is the same as the language ID. Each PHP file contains messages
 * belonging to the same category, and the file name is the same as the category
 * name.</li>
 ...
 * </li>
 * </ul>
 * When {@link cachingDuration} is set as a positive number, message
 * translations will be cached.
 *
 * Messages for an extension class (e.g. a widget, a module) can be specially
 * managed and used.
 * In particular, if a message belongs to an extension whose class name is Xyz,
 * then the message category
 * can be specified in the format of 'Xyz.categoryName'. And the corresponding
 * message file
 * is assumed to be 'BasePath/messages/LanguageID/categoryName.php', where
 * 'BasePath' refers to
 * the directory that contains the extension class file. When using Yii::t() to
 * translate an extension message,
...
class CPhpMessageSource extends CMessageSource {
	...
	/**
	 *
	 * @var integer the time in seconds that the messages can remain valid in
	 *      cache.
	 ...
	 *      component.
	 *      Set this property to false if you want to disable caching the
	 *      messages.
	 */
	public $cacheID = 'cache';
	/**
	 *
	 * @var string the base path for all translated messages. Defaults to null,
	 *      meaning
	...
	/**
	 *
	 * @var array the message paths for extensions that do not have a base class
	 *      to use as category prefix.
	 ...
	 *      )
	 *      </pre>
	 *      Where the key is the name of the extension and the value is the
	 *      alias to the path
	 *      of the "messages" subdirectory of the extension.
	 *      When using Yii::t() to translate an extension message, the category
	 *      name should be
	 ...
	 * language.
	 * If the category name contains a dot, it will be split into the module
	 * class name and the category name.
	 * In this case, the message file will be assumed to be located within the
	 * 'messages' subdirectory of
	...
	protected function getMessageFile($category, $language) {
		if (! isset ( $this->_files [$category] [$language] )) {
			if (($pos = strpos ( $category, '.' )) !== false) {
				...
				// First check if there's an extension registered for this
				// class.
