 ...
 * </pre>
 * where each expression should be a valid PHP expression with <code>'n'</code>
 * as the only variable.
 * For example, <code>'n==1'</code> and <code>'n%10==2 && n>10'</code> are both
 * valid expressions.
 * The variable <code>'n'</code> will take the given number value, and if an
 * expression evaluates true,
 * the corresponding message will be returned.
 *
 * For example, given the candidate messages
 * <code>'n==1#one|n==2#two|n>2#others'</code> and
 * the number value 2, the resulting message will be <code>'two'</code>.
 *
 * For expressions like <code>'n==1'</code>, we can also use a shortcut
 * <code>'1'</code>. So the above example
 * candidate messages can be simplified as
 * <code>'1#one|2#two|n>2#others'</code>.
 *
...
class CChoiceFormat {
	 ...
	 *        	the candidate messages in the format of
	 *        	'expr1#message1|expr2#message2|expr3#message3'.
