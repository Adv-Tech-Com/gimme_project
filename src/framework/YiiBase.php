...
/**
 * This constant defines how much call stack information (file name and line
 * number) should be logged by Yii::trace().
 * Defaults to 0, meaning no backtrace information. If it is greater than 0,
 * at most that number of call stacks will be logged. Note, only user
 * application call stacks are considered.
 */
defined ( 'YII_TRACE_LEVEL' ) or define ( 'YII_TRACE_LEVEL', 0 );
/**
 * This constant defines whether exception handling should be enabled.
 * Defaults to true.
 */
...
class YiiBase {
	 ...
	 *      The array keys are the class names and the array values are the
	 *      corresponding class file paths.
	 * @since 1.1.5
	 */
	public static $classMap = array ();
	/**
	 *
	 * @var boolean whether to rely on PHP include path to autoload class files.
	 *      Defaults to true.
	 *      You may set this to be false if your hosting environment doesn't
	 *      allow changing the PHP
	 *      include path, or if you want to append additional autoloaders to the
	 *      default Yii autoloader.
	 * @since 1.1.8
	 */
	 ...
	 *        	If a string, it is treated as the path of the file that
	 *        	contains the configuration;
	 *        	If an array, it is the actual configuration information.
	 *        	Please make sure you specify the {@link CApplication::basePath
	 *        	basePath} property in the configuration,
	 *        	which should point to the directory containing all application
	 ...
	 * @param mixed $config
	 *        	application configuration. This parameter will be passed as
	 *        	the parameter
	...
	/**
	 * Returns the application singleton or null if the singleton has not been
	 * created yet.
	 * 
	 * @return CApplication the application singleton, null if the singleton has
	 ...
	 * be either the class name or {@link YiiBase::getPathOfAlias class path
	 * alias}.
	 ...
	 * The main difference is that importing a class is much lighter because it
	 * only
	 ...
	 * include path.
	 * If multiple directories are imported, the directories imported later will
	 * take
	 * precedence in class file searching (i.e., they are added to the front of
	 * the PHP include path).
	 *
	 * Path aliases are used to import a class or directory. For example,
	 * <ul>
	 * <li><code>application.components.GoogleMap</code>: import the
	 * <code>GoogleMap</code> class.</li>
	 * <li><code>application.components.*</code>: import the
	 * <code>components</code> directory.</li>
	 * </ul>
	 *
	 * The same path alias can be imported multiple times, but only the first
	 * time is effective.
	 * Importing a directory does not import any of its subdirectories.
	 *
	 * Starting from version 1.1.5, this method can also be used to import a
	 * class in namespace format
	 * (available for PHP 5.3 or above only). It is similar to importing a class
	 * in path alias format,
	 * except that the dot separator is replaced by the backslash separator. For
	 * example, importing
	 * <code>application\components\GoogleMap</code> is similar to importing
	 * <code>application.components.GoogleMap</code>.
	 * The difference is that the former class is using qualified name, while
	 * the latter unqualified.
	 *
	 * Note, importing a class in namespace format requires that the namespace
	 * corresponds to
	 * a valid path alias once backslash characters are replaced with dot
	 * characters.
	 * For example, the namespace <code>application\components</code> must
	 ...
	 *        	whether to include the class file immediately. If false, the
	 *        	class file
	 *        	will be included only when the class is being used. This
	...
	public static function import($alias, $forceInclude = false) {
		...
		if (($pos = strrpos ( $alias, '\\' )) !== false) 		// a class name in PHP 5.3
		                                        // namespace format
		{
	 ...
	 * Note, this method does not ensure the existence of the resulting file
	 * path.
	 * It only checks if the root alias is valid or not.
	 * 
	 * @param string $alias
	 *        	alias (e.g. system.web.CController)
	 * @return mixed file path corresponding to the alias, false if the alias is
	 *         invalid.
	 */
	...
	/**
	 * Create a path alias.
	 * Note, this method neither checks the existence of the path nor normalizes
	 * the path.
	 * 
	 * @param string $alias
	 *        	alias to the path
	 * @param string $path
	 *        	the path corresponding to the alias. If this is null, the
	 *        	corresponding
	 ...
	 * This method will only log a message when the application is in debug
	 * mode.
	 * 
	 * @param string $msg
	...
	/**
	 * Logs a message.
	 * Messages logged by this method may be retrieved via {@link
	 * CLogger::getLogs}
	 * and may be recorded in different media, such as file, email, database,
	 * using
	 * {@link CLogRouter}.
	 * 
	 * @param string $msg
	 *        	message to be logged
	 * @param string $level
	 *        	level of the message (e.g. 'trace', 'warning', 'error'). It is
	 *        	case-insensitive.
	 * @param string $category
	 *        	category of the message (e.g. 'system.web'). It is
	 *        	case-insensitive.
	 */
	 ...
	 * This has to be matched with a call to {@link endProfile()} with the same
	 * token.
	 ...
	 * This has to be matched with a previous call to {@link beginProfile()}
	 * with the same token.
	 * 
	 * @param string $token
	 *        	token for the code block
	 ...
	 * i.e., the message returned will be chosen from a few candidates according
	 * to the given
	 * number value. This feature is mainly used to solve plural format issue in
	 * case
	 * a message has different plural forms in some languages.
	 * 
	 * @param string $category
	 *        	message category. Please use only word letters. Note, category
	 *        	'yii' is
	 *        	reserved for Yii framework core code use. See {@link
	 *        	CPhpMessageSource} for
	 *        	more interpretation about message category.
	 * @param string $message
	 *        	the original message
	 * @param array $params
	 *        	parameters to be applied to the message using
	 *        	<code>strtr</code>.
	 *        	The first parameter can be a number without key.
	 *        	And in this case, the method will call {@link
	 *        	CChoiceFormat::format} to choose
	 *        	an appropriate message translation.
	 *        	Starting from version 1.1.6 you can pass parameter for {@link
	 *        	CChoiceFormat::format}
	 *        	or plural forms format without wrapping it with array.
	 *        	This parameter is then available as <code>{n}</code> in the
	 *        	message translation string.
	 * @param string $source
	 *        	which message source application component to use.
	 *        	Defaults to null, meaning using 'coreMessages' for messages
	 *        	belonging to
	 *        	the 'yii' category and using 'messages' for the rest messages.
	 * @param string $language
	 *        	the target language. If null (default), the {@link
	 *        	CApplication::getLanguage application language} will be used.
	 ...
	 *        	a valid PHP callback (function name or
	 *        	array($className,$methodName)).
	 * @param boolean $append
	 *        	whether to append the new autoloader after the default Yii
	 *        	autoloader.
	 *        	Be careful using this option as it will disable {@link
	 *        	enableIncludePath autoloading via include path}
	 *        	when set to true. After this the Yii autoloader can not rely
	 *        	on loading classes via simple include anymore
	 ...
	 *      NOTE, DO NOT MODIFY THIS ARRAY MANUALLY. IF YOU CHANGE OR ADD SOME
	 *      CORE CLASSES,
