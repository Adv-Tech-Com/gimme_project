 ...
 * which triggers the corresponding
 * actions on the model of the row.
 *
 * By configuring {@link buttons} and {@link template} properties, the column
 * can display other buttons
...
class CButtonColumn extends CGridColumn {
	 ...
	 *      cell.
	 *      These default tokens are recognized: {view}, {update} and {delete}.
	 *      If the {@link buttons} property
	 *      defines additional buttons, their IDs are also recognized here. For
	 *      example, if a button named 'preview'
	 *      is declared in {@link buttons}, we can use the token '{preview}'
	 *      here to specify where to display the button.
	 */
	...
	/**
	 *
	 * @var string the image URL for the view button. If not set, an integrated
	 *      image will be used.
	...
	/**
	 *
	 * @var string a PHP expression that is evaluated for every view button and
	 *      whose result is used
	 *      as the URL for the view button. In this expression, you can use the
	 *      following variables:
	 *      <ul>
	...
	/**
	 *
	 * @var string the image URL for the update button. If not set, an
	 *      integrated image will be used.
	 *      You may set this property to be false to render a text link instead.
	 */
	public $updateButtonImageUrl;
	/**
	 *
	 * @var string a PHP expression that is evaluated for every update button
	 *      and whose result is used
	 *      as the URL for the update button. In this expression, you can use
	...
	/**
	 *
	 * @var string the image URL for the delete button. If not set, an
	 *      integrated image will be used.
	 *      You may set this property to be false to render a text link instead.
	 */
	...
	/**
	 *
	 * @var string a PHP expression that is evaluated for every delete button
	 *      and whose result is used
	 *      as the URL for the delete button. In this expression, you can use
	 *      the following variables:
	 *      <ul>
	 *      <li><code>$row</code> the row number (zero-based)</li>
	 *      <li><code>$data</code> the data model for the row</li>
	...
	/**
	 *
	 * @var string the confirmation message to be displayed when delete button
	 *      is clicked.
	 *      By setting this property to be false, no confirmation message will
	 *      be displayed.
	 *      This property is used only if
	 *      <code>$this->buttons['delete']['click']</code> is not set.
	 */
	public $deleteConfirmation;
	/**
	 *
	 * @var string a javascript function that will be invoked after the delete
	 *      ajax call.
	 *      This property is used only if
	 *      <code>$this->buttons['delete']['click']</code> is not set.
	 *     
	 ...
	 *      <li><code>success</code> status of the ajax call, true if the ajax
	 *      call was successful, false if the ajax call failed.
	 *      <li><code>data</code> the data returned by the server in case of a
	 *      successful call or XHR object in case of error.
	 *      </ul>
	 *      Note that if success is true it does not mean that the delete was
	 *      successful, it only means that the ajax call was successful.
	 *     
	 *      Example:
	 *      <pre>
	 *      array(
	...
	/**
	 *
	 * @var array the configuration for buttons. Each array element specifies a
	 *      single button
	 ...
	 *      button
	 *      'imageUrl'=>'...', // image URL of the button. If not set or false,
	 *      a text link is used
	 ...
	 *      </pre>
	 *     
	 *      In the PHP expression for the 'url' option and/or 'visible' option,
	 *      the variable <code>$row</code>
	 *      refers to the current row number (zero-based), and
	 *      <code>$data</code> refers to the data model for
	 *      the row.
	 *      The PHP expression will be evaluated using {@link
	 *      evaluateExpression}.
	 *      A PHP expression can be any PHP code that has a value. To learn more
	 ...
	 *      http://www.php.net/manual/en/language.expressions.php php manual}.
	 *     
	 *      If the 'buttonID' is 'view', 'update' or 'delete' the options will
	 *      be applied to the default buttons.
	 *     
	 *      Note that in order to display non-default buttons, the {@link
	 *      template} property needs to
	 *      be configured so that the corresponding button IDs appear as tokens
	 *      in the template.
	 */
...
EOD;
	 ...
	 * @param array $button
	 *        	the button configuration which may contain 'label', 'url',
	 *        	'imageUrl' and 'options' elements.
