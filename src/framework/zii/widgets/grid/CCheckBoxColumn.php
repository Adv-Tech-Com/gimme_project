 ...
 * checking.
 * The mode is determined according to {@link selectableRows}. When in multiple
 * checking mode, the header cell will display
 * an additional checkbox, clicking on which will check or uncheck all of the
 * checkboxes in the data cells.
...
class CCheckBoxColumn extends CGridColumn {
	 ...
	 *      in each data cell as the checkbox value. Note that if {@link value}
	 *      is specified, this property will be ignored.
	 * @see value
	 */
	public $name;
	/**
	 ...
	 *      and whose result will be rendered
	 *      in each data cell as the checkbox value. In this expression, you can
	 ...
	 *
	 * @var string a PHP expression that will be evaluated for every data cell
	 *      and whose result will
	 *      determine if checkbox for each data cell is checked. In this
	 ...
	 * @var string a PHP expression that will be evaluated for every data cell
	 *      and whose result will
	 *      determine if checkbox for each data cell is disabled. In this
	 *      expression, you can use the following variables:
	 *      <ul>
	 *      <li><code>$row</code> the row number (zero-based)</li>
	 *      <li><code>$data</code> the data model for the row</li>
	 ...
	 *      http://www.php.net/manual/en/language.expressions.php php manual}.
	 *     
	 *      Note that expression result will overwrite value set with
	 *      <code>checkBoxHtmlOptions['disabled']</code>.
	 * @since 1.1.13
	 */
	public $disabled;
	/**
	 ...
	 *      Possible values:
	 *      <ul>
	 *      <li>0 - the state of the checkbox cannot be changed (read-only
	 *      mode)</li>
	 *      <li>1 - only one row can be checked. Checking a checkbox has nothing
	 *      to do with selecting the row</li>
	 *      <li>2 or more - multiple checkboxes can be checked. Checking a
	 *      checkbox has nothing to do with selecting the row</li>
	 *      <li>null - {@link CGridView::selectableRows} is used to control how
	 *      many checkboxes can be checked.
	 *      Checking a checkbox will also select the row.</li>
	 *      </ul>
	 *      You may also call the JavaScript function
	 *      <code>$(gridID).yiiGridView('getChecked', columnID)</code>
	...
	/**
	 *
	 * @var string the template to be used to control the layout of the header
	 *      cell.
	 *      The token "{item}" is recognized and it will be replaced with a
	 *      "check all" checkbox.
	 *      By default if in multiple checking mode, the header cell will
	 *      display an additional checkbox,
	 *      clicking on which will check or uncheck all of the checkboxes in the
	 *      data cells.
...
EOD;
	 ...
	 * This method will render a checkbox in the header when {@link
	 * selectableRows} is greater than 1
	 * or in case {@link selectableRows} is null when {@link
	 * CGridView::selectableRows} is greater than 1.
	 */
