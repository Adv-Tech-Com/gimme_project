 ...
 * }
 * </pre>
 * The {@link createAttribute} and {@link updateAttribute} options actually
 * default to 'create_time' and 'update_time'
 * respectively, so it is not required that you configure them. If you do not
 * wish CTimestampBehavior
 * to set a timestamp for record update or creation, set the corresponding
 * attribute option to null.
 *
 * By default, the update attribute is only set on record update. If you also
 * wish it to be set on record creation,
 ...
 * inject into the timestamp attribute,
...
class CTimestampBehavior extends CActiveRecordBehavior {
	/**
	 *
	 * @var mixed The name of the attribute to store the creation time. Set to
	 *      null to not
	 *      use a timestamp for the creation attribute. Defaults to
	 *      'create_time'
	 */
	public $createAttribute = 'create_time';
	/**
	 *
	 * @var mixed The name of the attribute to store the modification time. Set
	 *      to null to not
	...
	/**
	 *
	 * @var bool Whether to set the update attribute to the creation timestamp
	 *      upon creation.
	 ...
	 *      This can be either a string representing a PHP expression (e.g.
	 *      'time()'),
	 *      or a {@link CDbExpression} object representing a DB expression (e.g.
	 *      new CDbExpression('NOW()')).
	 *      Defaults to null, meaning that we will attempt to figure out the
	 *      appropriate timestamp
	 *      automatically. If we fail at finding the appropriate timestamp, then
	 *      it will
