 ...
 * set.
 *
 * To read the current row of data, call {@link read}. The method {@link
 * readAll}
...
class CDbDataReader extends CComponent implements Iterator, Countable {
	 ...
	 * will be set in the variable. Note, the fetch mode must include
	 * PDO::FETCH_BOUND.
	 * 
	 * @param mixed $column
	 *        	Number of the column (1-indexed) or name of the column
	 *        	in the result set. If using the column name, be aware that the
	 *        	name
	 *        	should match the case of the column, as returned by the
	 *        	driver.
	 * @param mixed $value
	 ...
	 * 
	 * @return array false current row, false if no more row available
	 */
	 ...
	 * @return mixed false column of the current row, false if no more row
	 *         available
	 */
	 ...
	 * @return mixed false populated object, false if no more row of data
	 *         available
	 */
	 ...
	 * 
	 * @return array the result set (each array element represents a row of
	 *         data).
	...
	/**
	 * Advances the reader to the next result when reading the results of a
	 * batch of statements.
	 ...
	 * In this case, use "SELECT COUNT(*) FROM tableName" to obtain the number
	 * of rows.
	 * 
