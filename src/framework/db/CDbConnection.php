 ...
 * in a common set of APIs. They are a thin wrapper of the {@link
 * http://www.php.net/manual/en/ref.pdo.php PDO}
 * PHP extension.
 *
 ...
 * be used as an application component and be configured in application
 * configuration,
 * like the following,
 * <pre>
 * array(
 ...
 * @property PDO $pdoInstance The PDO instance, null if the connection is not
 *           established yet.
 * @property CDbTransaction $currentTransaction The currently active
 *           transaction. Null if no active transaction.
 ...
 * @property string $lastInsertID The row ID of the last row inserted, or the
 ...
 * @property boolean $autoCommit Whether creating or updating a DB record will
 ...
 * @property array $attributes Attributes (name=>value) that are previously
 *           explicitly set for the DB connection.
 * @property array $stats The first element indicates the number of SQL
 *           statements executed,
...
class CDbConnection extends CApplicationComponent {
	/**
	 *
	 * @var string The Data Source Name, or DSN, contains the information
	 *      required to connect to the database.
	 * @see http://www.php.net/manual/en/function.PDO-construct.php Note that if
	 *      you're using GBK or BIG5 then it's highly recommended to
	...
	/**
	 *
	 * @var integer number of seconds that table metadata can remain valid in
	 *      cache.
	 *      Use 0 or negative value to indicate not caching schema.
	 *      If greater than 0 and the primary cache is enabled, the table
	...
	/**
	 *
	 * @var array list of tables whose metadata should NOT be cached. Defaults
	 *      to empty array.
	 ...
	 *      component.
	 *      Set this property to false if you want to disable caching table
	 *      metadata.
	 */
	public $schemaCacheID = 'cache';
	/**
	 *
	 * @var integer number of seconds that query results can remain valid in
	 *      cache.
	 *      Use 0 or negative value to indicate not caching query results (the
	 *      default behavior).
	 *     
	 *      In order to enable query caching, this property must be a positive
	 *      integer and {@link queryCacheID} must point to a valid cache
	 *      component ID.
	 *     
	 *      The method {@link cache()} is provided as a convenient way of
	...
	/**
	 *
	 * @var CCacheDependency ICacheDependency dependency that will be used when
	 ...
	 *      If this is 0, then even if query caching is enabled, no query will
	 *      be cached.
	 *      Note that each time after executing a SQL statement (whether
	 *      executed on DB server or fetched from
	...
	/**
	 *
	 * @var string the ID of the cache application component that is used for
	 *      query caching.
	 *      Defaults to 'cache' which refers to the primary cache application
	 *      component.
	...
	/**
	 *
	 * @var boolean whether the database connection should be automatically
	 *      established
	 *      the component is being initialized. Defaults to true. Note, this
	 *      property is only
	 *      effective when the CDbConnection object is used as an application
	 *      component.
	 */
	public $autoConnect = true;
	/**
	 *
	 * @var string the charset used for database connection. The property is
	 *      only used
	 *      for MySQL and PostgreSQL databases. Defaults to null, meaning using
	 *      default charset
	 *      as specified by the database.
	 *     
	 *      Note that if you're using GBK or BIG5 then it's highly recommended
	 *      to
	 *      update to PHP 5.3.6+ and to specify charset via DSN like
	 *      'mysql:dbname=mydatabase;host=127.0.0.1;charset=GBK;'.
	 */
	...
	/**
	 *
	 * @var boolean whether to turn on prepare emulation. Defaults to false,
	 *      meaning PDO
	 *      will use the native prepare support if available. For some databases
	 *      (such as MySQL),
	 *      this may need to be set true so that PDO can emulate the prepare
	 *      support to bypass
	 *      the buggy native prepare support. Note, this property is only
	 *      effective for PHP 5.1.3 or above.
	 *      The default value is null, which will not change the
	 *      ATTR_EMULATE_PREPARES value of PDO.
	 */
	public $emulatePrepare;
	/**
	 *
	 * @var boolean whether to log the values that are bound to a prepare SQL
	 *      statement.
	 *      Defaults to false. During development, you may consider setting this
	 *      property to true
	 *      so that parameter values bound to SQL statements are logged for
	 *      debugging purpose.
	 *      You should be aware that logging parameter values could be expensive
	...
	/**
	 *
	 * @var boolean whether to enable profiling the SQL statements being
	 *      executed.
	 *      Defaults to false. This should be mainly enabled and used during
	 *      development
	...
	/**
	 *
	 * @var string the default prefix for table names. Defaults to null, meaning
	 *      no table prefix.
	 *      By setting this property, any token like '{{tableName}}' in {@link
	 *      CDbCommand::text} will
	 *      be replaced by 'prefixTableName', where 'prefix' refers to this
	 *      property value.
	 * @since 1.1.0
	 */
	public $tablePrefix;
	/**
	 *
	 * @var array list of SQL statements that should be executed right after the
	...
	public $driverMap = array (
			...
			'dblib' => 'CMssqlSchema', // dblib drivers on linux (and maybe others
			                         // os) hosts
	 ...
	 * 
	 * @param string $dsn
	 *        	The Data Source Name, or DSN, contains the information
	 *        	required to connect to the database.
	 ...
	 * This method is required by {@link IApplicationComponent} and is invoked
	 * by application
	 ...
	 * By setting the $duration parameter to be 0, the query caching will be
	 * disabled.
	 * Otherwise, query results of the new SQL statements executed next will be
	 * saved in cache
	 * and remain valid for the specified duration.
	 * If the same query is executed again, the result may be fetched from cache
	 * directly
	 * without actually executing the SQL statement.
	 * 
	 * @param integer $duration
	 *        	the number of seconds that query results may remain valid in
	 *        	cache.
	 ...
	 *        	number of SQL queries that need to be cached after calling
	 *        	this method. Defaults to 1,
	 ...
	 * The default implementation is to set the charset for MySQL and PostgreSQL
	 ...
	 * 
	 * @return PDO the PDO instance, null if the connection is not established
	 *         yet
	 */
	 ...
	 * 
	 * @param mixed $query
	 *        	the DB query to be executed. This can be either a string
	 *        	representing a SQL statement,
	 *        	or an array representing different fragments of a SQL
	 *        	statement. Please refer to {@link CDbCommand::__construct}
	 *        	for more details about how to pass an array as the query. If
	 *        	this parameter is not given,
	 *        	you will have to call query builder methods of {@link
	 ...
	 * 
	 * @return CDbTransaction the currently active transaction. Null if no
	 *         active transaction.
	 */
	 ...
	 * 
	 * @throws CDbException if CDbConnection does not support reading schema for
	 ...
	 * If the table name contains schema prefix, the prefix will also be
	 * properly quoted.
	 * 
	 * @param string $name
	 *        	table name
	 ...
	 * If the column name contains prefix, the prefix will also be properly
	 * quoted.
	 * 
	 * @param string $name
	 *        	column name
	 ...
	 * 
	 * @return boolean whether creating or updating a DB record will be
	 ...
	 * 
	 * @param boolean $value
	 *        	whether creating or updating a DB record will be automatically
	 *        	committed.
	 */
	 ...
	 * connection.
	 * 
	 * @return array attributes (name=>value) that are previously explicitly set
	 ...
	 * 
	 * @return array the first element indicates the number of SQL statements
	 *         executed,
	 *         and the second element the total time spent in SQL execution.
	 */
