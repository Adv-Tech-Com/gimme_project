 ...
 * (such as SELECT), use {@link query} or its convenient versions {@link
 * queryRow},
 ...
 * @property CDbConnection $connection The connection associated with this
 *           command.
 * @property PDOStatement $pdoStatement The underlying PDOStatement for this
 *           command
 *           It could be null if the statement is not prepared yet.
 * @property string $select The SELECT part (without 'SELECT') in the query.
 * @property boolean $distinct A value indicating whether SELECT DISTINCT should
 *           be used.
 ...
 * @property mixed $join The join part in the query. This can be an array
 *           representing
 *           multiple join fragments, or a string representing a single join
 *           fragment.
 *           Each join fragment will contain the proper join operator (e.g. LEFT
 *           JOIN).
 ...
 *           This can be either a string or an array representing multiple union
 *           parts.
 *          
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db
...
class CDbCommand extends CComponent {
	 ...
	 *        	a string representing a SQL statement, or an array whose
	 *        	name-value pairs
	 *        	will be used to set the corresponding properties of the
	 *        	created command object.
	 *        	
	 *        	For example, you can pass in either <code>'SELECT * FROM
	 *        	tbl_user'</code>
	 *        	or <code>array('select'=>'*', 'from'=>'tbl_user')</code>. They
	 *        	are equivalent
	 *        	in terms of the final query result.
	 *        	
	 *        	When passing the query as an array, the following properties
	 *        	are commonly set:
	 *        	{@link select}, {@link distinct}, {@link from}, {@link where},
	 *        	{@link join},
	 *        	{@link group}, {@link having}, {@link order}, {@link limit},
	 *        	{@link offset} and
	 *        	{@link union}. Please refer to the setter of each of these
	 *        	properties for details
	 *        	about valid property values. This feature has been available
	 *        	since version 1.1.6.
	 *        	
	 *        	Since 1.1.7 it is possible to use a specific mode of data
	 *        	fetching by setting
	 ...
	 * Calling this method will clean up all internal states of the command
	 * object.
	 * 
	 ...
	 *        	placeholders, this will be the 1-indexed position of the
	 *        	parameter.
	 * @param mixed $value
	 *        	Name of the PHP variable to bind to the SQL statement
	 *        	parameter
	 * @param integer $dataType
	 *        	SQL data type of the parameter. If null, the type is
	 *        	determined by the PHP type of the value.
	 * @param integer $length
	 *        	length of the data type
	 * @param mixed $driverOptions
	 *        	the driver-specific options (this is available since version
	 *        	1.1.6)
	 ...
	 * This is similar to {@link bindValue} except that it binds multiple
	 * values.
	 * Note that the SQL data type of each value is determined by its PHP type.
	 * 
	 * @param array $values
	 *        	the values to be bound. This must be given in terms of an
	 *        	associative
	 *        	array with array keys being the parameter names, and array
	 ...
	 * 
	 * @param array $params
	 *        	input parameters (name=>value) for the SQL execution. This is
	 *        	an alternative
	 *        	to {@link bindParam} and {@link bindValue}. If you have
	 *        	multiple input parameters, passing
	 *        	them in this way can improve the performance. Note that if you
	 *        	pass parameters in this way,
	 *        	you cannot bind parameters or values using {@link bindParam}
	 *        	or {@link bindValue}, and vice versa.
	 *        	Please also note that all values are treated as strings in
	 *        	this case, if you need them to be handled as
	 *        	their real data types, you have to use {@link bindParam} or
	 ...
	 *        	whether each row should be returned as an associated array
	 *        	with
	 *        	column names as the keys or the array keys are column indexes
	 *        	(0-based).
	 * @param array $params
	 *        	input parameters (name=>value) for the SQL execution. This is
	 *        	an alternative
	 ...
	 *        	their real data types, you have to use {@link bindParam} or
	 *        	{@link bindValue} instead.
	 * @return array all rows of the query result. Each array element is an
	 ...
	 * This is a convenient method of {@link query} when only the first row of
	 * data is needed.
	 * 
	 * @param boolean $fetchAssociative
	 ...
	 *        	their real data types, you have to use {@link bindParam} or
	 *        	{@link bindValue} instead.
	 * @return mixed the first row (in terms of an array) of the query result,
	 *         false if no result.
	 * @throws CException execution failed
	 */
	 ...
	 *        	their real data types, you have to use {@link bindParam} or
	 *        	{@link bindValue} instead.
	 * @return mixed the value of the first column in the first row of the query
	 *         result. False is returned if there is no value.
	 * @throws CException execution failed
	 */
	 ...
	 * This is a convenient method of {@link query} when only the first column
	 * of data is needed.
	 * Note, the column returned will contain the first element in each row of
	 * result.
	 * 
	 * @param array $params
	 *        	input parameters (name=>value) for the SQL execution. This is
	 ...
	 *        	their real data types, you have to use {@link bindParam} or
	 *        	{@link bindValue} instead.
	 * @return array the first column of the query result. Empty array if no
	 *         result.
	 * @throws CException execution failed
	 */
	 ...
	 *        	query options are supported: {@link select}, {@link distinct},
	 *        	{@link from},
	 *        	{@link where}, {@link join}, {@link group}, {@link having},
	 *        	{@link order},
	 *        	{@link limit}, {@link offset} and {@link union}.
	 * @throws CDbException if "from" key is not present in given query
	 *         parameter
	 ...
	 * 
	 * @param mixed $columns
	 *        	the columns to be selected. Defaults to '*', meaning all
	 *        	columns.
	 *        	Columns can be specified in either a string (e.g. "id, name")
	 *        	or an array (e.g. array('id', 'name')).
	 *        	Columns can contain table prefixes (e.g. "tbl_user.id") and/or
	 *        	column aliases (e.g. "tbl_user.id AS user_id").
	 *        	The method will automatically quote the column names unless a
	 *        	column contains some parenthesis
	 *        	(which means the column contains a DB expression).
	 * @param string $option
	 *        	additional option that should be appended to the 'SELECT'
	 *        	keyword. For example,
	 *        	in MySQL, the option 'SQL_CALC_FOUND_ROWS' can be used. This
	 *        	parameter is supported since version 1.1.8.
	 ...
	 * 
	 * @param mixed $value
	 *        	the data to be selected. Please refer to {@link select()} for
	 *        	details
	 ...
	 * This is the same as {@link select} except that the DISTINCT flag is
	 * turned on.
	 * 
	 * @param mixed $columns
	 *        	the columns to be selected. See {@link select} for more
	 *        	details.
	 * @return CDbCommand the command object itself
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @return boolean a value indicating whether SELECT DISTINCT should be
	 *         used.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param mixed $tables
	 *        	the table(s) to be selected from. This can be either a string
	 *        	(e.g. 'tbl_user')
	 *        	or an array (e.g. array('tbl_user', 'tbl_profile')) specifying
	 *        	one or several table names.
	 *        	Table names can contain schema prefixes (e.g.
	 *        	'public.tbl_user') and/or table aliases (e.g. 'tbl_user u').
	 *        	The method will automatically quote the table names unless it
	 *        	contains some parenthesis
	 *        	(which means the table is given as a sub-query or DB
	 *        	expression).
	 * @return CDbCommand the command object itself
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param mixed $value
	 *        	the tables to be selected from. Please refer to {@link from()}
	 *        	for details
	 *        	on how to specify this parameter.
	 * @since 1.1.6
	 */
	 ...
	 * array.
	 * If the latter, it must be of the format <code>array(operator, operand1,
	 * operand2, ...)</code>,
	 * where the operator can be one of the followings, and the possible
	 * operands depend on the corresponding
	 * operator:
	 * <ul>
	 * <li><code>and</code>: the operands should be concatenated together using
	 * AND. For example,
	 * array('and', 'id=1', 'id=2') will generate 'id=1 AND id=2'. If an operand
	 * is an array,
	 * it will be converted into a string using the same rules described here.
	 * For example,
	 * array('and', 'type=1', array('or', 'id=1', 'id=2')) will generate 'type=1
	 * AND (id=1 OR id=2)'.
	 * The method will NOT do any quoting or escaping.</li>
	 * <li><code>or</code>: similar as the <code>and</code> operator except that
	 * the operands are concatenated using OR.</li>
	 * <li><code>in</code>: operand 1 should be a column or DB expression, and
	 * operand 2 be an array representing
	 * the range of the values that the column or DB expression should be in.
	 * For example,
	 * array('in', 'id', array(1,2,3)) will generate 'id IN (1,2,3)'.
	 * The method will properly quote the column name and escape values in the
	 * range.</li>
	 * <li><code>not in</code>: similar as the <code>in</code> operator except
	 * that IN is replaced with NOT IN in the generated condition.</li>
	 * <li><code>like</code>: operand 1 should be a column or DB expression, and
	 * operand 2 be a string or an array representing
	 * the values that the column or DB expression should be like.
	 * For example, array('like', 'name', '%tester%') will generate "name LIKE
	 * '%tester%'".
	 * When the value range is given as an array, multiple LIKE predicates will
	 * be generated and concatenated using AND.
	 * For example, array('like', 'name', array('%test%', '%sample%')) will
	 * generate
	 * "name LIKE '%test%' AND name LIKE '%sample%'".
	 * The method will properly quote the column name and escape values in the
	 * range.</li>
	 * <li><code>not like</code>: similar as the <code>like</code> operator
	 * except that LIKE is replaced with NOT LIKE in the generated
	 * condition.</li>
	 * <li><code>or like</code>: similar as the <code>like</code> operator
	 * except that OR is used to concatenated the LIKE predicates.</li>
	 * <li><code>or not like</code>: similar as the <code>not like</code>
	 * operator except that OR is used to concatenated the NOT LIKE
	 * predicates.</li>
	 * </ul>
	 * 
	...
	/**
	 * Appends given condition to the existing WHERE part of the query with
	 * 'AND' operator.
	 *
	 * This method works almost the same way as {@link where} except the fact
	 * that it appends condition
	 * with 'AND' operator, but not replaces it with the new one. For more
	 ...
	 * This method works almost the same way as {@link where} except the fact
	 * that it appends condition
	 * with 'OR' operator, but not replaces it with the new one. For more
	 * information on parameters
	 * of this method refer to the {@link where} documentation.
	 *
	 * @param mixed $conditions
	 ...
	 *        	Table name can contain schema prefix (e.g. 'public.tbl_user')
	 *        	and/or table alias (e.g. 'tbl_user u').
	 *        	The method will automatically quote the table name unless it
	 *        	contains some parenthesis
	 *        	(which means the table is given as a sub-query or DB
	 *        	expression).
	 * @param mixed $conditions
	 ...
	 * 
	 * @return mixed the join part in the query. This can be an array
	 *         representing
	 *         multiple join fragments, or a string representing a single jojin
	 *         fragment.
	 *         Each join fragment will contain the proper join operator (e.g.
	 *         LEFT JOIN).
	 * @since 1.1.6
	 */
	 ...
	 *        	an array representing multiple join parts in the query. Each
	 *        	part must contain
	 *        	the proper join operator (e.g. 'LEFT JOIN tbl_profile ON
	 ...
	 *        	Columns can be specified in either a string (e.g. "id ASC,
	 *        	name DESC") or an array (e.g. array('id ASC', 'name DESC')).
	 *        	The method will automatically quote the column names unless a
	 *        	column contains some parenthesis
	 *        	(which means the column contains a DB expression).
	 *        	
	 ...
	 *         This can be either a string or an array representing multiple
	 *         union parts.
	 * @since 1.1.6
	 */
	 ...
	 * The method will properly escape the column names, and bind the values to
	 * be inserted.
	 * 
	 * @param string $table
	 ...
	 *        	Do not use column names as parameter names here. They are
	 *        	reserved for <code>$columns</code> parameter.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.6
	 */
	 ...
	 * (e.g. 'name'=>'string'),
	 * where name stands for a column name which will be properly quoted by the
	 * method, and definition
	 * stands for the column type which can contain an abstract DB type.
	 * The {@link getColumnType} method will be invoked to convert any abstract
	 * type into a physical one.
	 *
	 * If a column is specified with definition only (e.g. 'PRIMARY KEY (name,
	 * type)'), it will be directly
	 * inserted into the generated SQL.
	 *
	 * @param string $table
	 *        	the name of the table to be created. The name will be properly
	 *        	quoted by the method.
	 * @param array $columns
	 *        	the columns (name=>definition) in the new table.
	 * @param string $options
	 *        	additional SQL fragment that will be appended to the generated
	 *        	SQL.
	 * @return integer 0 is always returned. See {@link
	 *         http://php.net/manual/en/pdostatement.rowcount.php} for more
	 *         information.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param string $table
	 *        	the table to be renamed. The name will be properly quoted by
	 *        	the method.
	 * @param string $newName
	 *        	the new table name. The name will be properly quoted by the
	 *        	method.
	 * @return integer 0 is always returned. See {@link
	 *         http://php.net/manual/en/pdostatement.rowcount.php} for more
	 *         information.
	 ...
	 * 
	 * @param string $table
	 *        	the table to be dropped. The name will be properly quoted by
	 *        	the method.
	 * @return integer 0 is always returned. See {@link
	 *         http://php.net/manual/en/pdostatement.rowcount.php} for more
	 *         information.
	 ...
	 * 
	 * @param string $table
	 *        	the table to be truncated. The name will be properly quoted by
	 *        	the method.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param string $table
	 *        	the table that the new column will be added to. The table name
	 *        	will be properly quoted by the method.
	 * @param string $column
	 *        	the name of the new column. The name will be properly quoted
	 *        	by the method.
	 * @param string $type
	 *        	the column type. The {@link getColumnType} method will be
	 *        	invoked to convert abstract column type (if any)
	 *        	into the physical one. Anything that is not recognized as
	 ...
	 * 
	 * @param string $table
	 *        	the table whose column is to be dropped. The name will be
	 *        	properly quoted by the method.
	 * @param string $column
	 *        	the name of the column to be dropped. The name will be
	 *        	properly quoted by the method.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param string $table
	 *        	the table whose column is to be renamed. The name will be
	 *        	properly quoted by the method.
	 * @param string $name
	 *        	the old name of the column. The name will be properly quoted
	 *        	by the method.
	 * @param string $newName
	 *        	the new name of the column. The name will be properly quoted
	 *        	by the method.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param string $table
	 *        	the table whose column is to be changed. The table name will
	 *        	be properly quoted by the method.
	 * @param string $column
	 *        	the name of the column to be changed. The name will be
	 *        	properly quoted by the method.
	 * @param string $type
	 *        	the new column type. The {@link getColumnType} method will be
	 *        	invoked to convert abstract column type (if any)
	 *        	into the physical one. Anything that is not recognized as
	 *        	abstract type will be kept in the generated SQL.
	 *        	For example, 'string' will be turned into 'varchar(255)',
	 ...
	 *        	the name of the column to that the constraint will be added
	 ...
	 *        	the name of the column that the foreign key references to. If
	 *        	there are multiple columns, separate them with commas.
	 * @param string $delete
	 *        	the ON DELETE option. Most DBMS support these options:
	 *        	RESTRICT, CASCADE, NO ACTION, SET DEFAULT, SET NULL
	 * @param string $update
	 *        	the ON UPDATE option. Most DBMS support these options:
	 *        	RESTRICT, CASCADE, NO ACTION, SET DEFAULT, SET NULL
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param string $name
	 *        	the name of the foreign key constraint to be dropped. The name
	 *        	will be properly quoted by the method.
	 * @param string $table
	 *        	the table whose foreign is to be dropped. The name will be
	 *        	properly quoted by the method.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param string $name
	 *        	the name of the index. The name will be properly quoted by the
	 *        	method.
	 * @param string $table
	 *        	the table that the new index will be created for. The table
	 *        	name will be properly quoted by the method.
	 * @param string $column
	 *        	the column(s) that should be included in the index. If there
	 *        	are multiple columns, please separate them
	 *        	by commas. The column names will be properly quoted by the
	 *        	method.
	 ...
	 * 
	 * @param string $name
	 *        	the name of the index to be dropped. The name will be properly
	 *        	quoted by the method.
	 * @param string $table
	 *        	the table whose index is to be dropped. The name will be
	 *        	properly quoted by the method.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.6
	 */
	 ...
	 * 
	 * @param string $type
	 *        	the join type ('join', 'left join', 'right join', 'cross
	 *        	join', 'natural join')
	 * @param string $table
	 *        	the table to be joined.
	 *        	Table name can contain schema prefix (e.g. 'public.tbl_user')
	 ...
	 * 
	 * @param string $name
	 *        	the name of the primary key constraint to be created. The name
	 *        	will be properly quoted by the method.
	 * @param string $table
	 *        	the table who will be inheriting the primary key. The name
	 *        	will be properly quoted by the method.
	 * @param string $columns
	 *        	the column/s where the primary key will be effected. The name
	 *        	will be properly quoted by the method.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.13
	 */
	 ...
	 * 
	 * @param string $name
	 *        	the name of the primary key constraint to be dropped. The name
	 *        	will be properly quoted by the method.
	 * @param string $table
	 *        	the table that owns the primary key. The name will be properly
	 *        	quoted by the method.
	 * @return integer number of rows affected by the execution.
	 * @since 1.1.13
	 */
