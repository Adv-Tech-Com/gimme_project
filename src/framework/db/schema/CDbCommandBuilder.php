...
class CDbCommandBuilder extends CComponent {
	 ...
	 * 
	 * @param mixed $table
	 *        	the table schema ({@link CDbTableSchema}) or the table name
	 *        	(string).
	...
	public function createCountCommand($table, $criteria, $alias = 't') {
		...
		// Suppress binding of parameters belonging to the ORDER clause. Issue
		// #1407.
	 ...
	 *        	(string).
	 * @param array $data
	 *        	data to be inserted (column name=>column value). If a key is
	 *        	not a valid column name, the corresponding value will be
	 *        	ignored.
	 ...
	 * This method could be used to achieve better performance during insertion
	 * of the large
	 ...
	 *        	(string).
	 * @param array[] $data
	 *        	list data to be inserted, each value should be an array in
	 *        	format (column name=>column value).
	 *        	If a key is not a valid column name, the corresponding value
	 *        	will be ignored.
	...
	/**
	 * Creates a multiple INSERT command.
	 * This method compose the SQL expression via given part templates,
	 * providing ability to adjust
	 ...
	 *        	counters to be updated (counter increments/decrements indexed
	 *        	by column names.)
	 * @param CDbCriteria $criteria
	 *        	the query criteria
	 * @throws CDbException if no columns are being updated for the given table
	 ...
	 *        	database command
	 * @param array $values
	 *        	values for binding (integer-indexed array for question mark
	 ...
	 *        	If a string, it is treated as query condition (the WHERE
	 *        	clause);
	 *        	If an array, it is treated as the initial values for
	 *        	constructing a {@link CDbCriteria} object;
	 *        	Otherwise, it should be an instance of {@link CDbCriteria}.
	 * @param array $params
	 *        	parameters to be bound to an SQL statement.
	 *        	This is only used when the first parameter is a string (query
	 *        	condition).
	 *        	In other cases, please use {@link CDbCriteria::params} to set
	 *        	parameters.
	 ...
	 *        	(string).
	 * @param mixed $pk
	 *        	primary key value(s). Use array for multiple primary keys. For
	 *        	composite key, each key value must be an array (column
	 *        	name=>column value).
	 * @param mixed $condition
	 *        	query condition or criteria.
	 *        	If a string, it is treated as query condition;
	 *        	If an array, it is treated as the initial values for
	 *        	constructing a {@link CDbCriteria};
	 *        	Otherwise, it should be an instance of {@link CDbCriteria}.
	 * @param array $params
	 *        	parameters to be bound to an SQL statement.
	 *        	This is only used when the second parameter is a string (query
	 *        	condition).
	 *        	In other cases, please use {@link CDbCriteria::params} to set
	 *        	parameters.
	 * @param string $prefix
	 *        	column prefix (ended with dot). If null, it will be the table
	 *        	name
	 * @return CDbCriteria the created query criteria
	 */
	public function createPkCriteria($table, $pk, $condition = '', $params = array(), $prefix = null) {
		...
		if (is_array ( $table->primaryKey ) && ! isset ( $pk [0] ) && $pk !== array ()) // single
		                                                                    // composite
		                                                                    // key
			$pk = array (
					$pk 
	 ...
	 *        	(string).
	 * @param array $columns
	 *        	column values that should be matched in the query
	 *        	(name=>value)
	 * @param mixed $condition
	 *        	query condition or criteria.
	 *        	If a string, it is treated as query condition;
	 ...
	 * @param array $params
	 *        	parameters to be bound to an SQL statement.
	 *        	This is only used when the third parameter is a string (query
	 *        	condition).
	 *        	In other cases, please use {@link CDbCriteria::params} to set
	 *        	parameters.
	 * @param string $prefix
	...
	/**
	 * Generates the expression for searching the specified keywords within a
	 * list of columns.
	 * The search expression is generated using the 'LIKE' SQL syntax.
	 * Every word in the keywords must be present and appear in at least one of
	 * the columns.
	 * 
	 * @param mixed $table
	 *        	the table schema ({@link CDbTableSchema}) or the table name
	 ...
	 *        	search keywords. This can be either a string with
	 *        	space-separated keywords or an array of keywords.
	 * @param string $prefix
	 *        	optional column prefix (with dot at the end). If null, the
	 *        	table name will be used as the prefix.
	 * @param boolean $caseSensitive
	 *        	whether the search is case-sensitive. Defaults to true.
	 * @throws CDbException if specified column is not found in given table
	 * @return string SQL search condition matching on a set of columns. An
	 ...
	 *        	the column name(s). It can be either a string indicating a
	 *        	single column
	 *        	or an array of column names. If the latter, it stands for a
	 *        	composite key.
	 * @param array $values
	...
	public function createInCondition($table, $columnName, $values, $prefix = null) {
		...
		} elseif (is_array ( $columnName )) 		// composite key:
		                              // $values=array(array('pk1'=>'v1','pk2'=>'v2'),array(...))
		{
	 ...
	 *        	If this refers to a valid table name, this parameter will be
	 *        	returned with the corresponding table schema.
