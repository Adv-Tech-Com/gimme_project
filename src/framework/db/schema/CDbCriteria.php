...
class CDbCriteria extends CComponent {
	 ...
	 *      in an SQL
	 *      statement. The property can be either a string (column names
	 *      separated by commas)
	...
	/**
	 *
	 * @var mixed the relational query criteria. This is used for fetching
	 *      related objects in eager loading fashion.
	 *      This property is effective only when the criteria is passed as a
	 *      parameter to the following methods of CActiveRecord:
	 *      <ul>
	 ...
	 *      </ul>
	 *      The property value will be used as the parameter to the {@link
	 *      CActiveRecord::with()} method
	 *      to perform the eager loading. Please refer to {@link
	...
	/**
	 *
	 * @var string the alias name of the table. If not set, it means the alias
	 *      is 't'.
	 */
	public $alias;
	/**
	 *
	 * @var boolean whether the foreign tables should be joined with the primary
	 *      table in a single SQL.
	 *      This property is only used in relational AR queries for HAS_MANY and
	 *      MANY_MANY relations.
	 *     
	 *      When this property is set true, only a single SQL will be executed
	 *      for a relational AR query,
	 *      even if the primary table is limited and the relationship between a
	 *      foreign table and the primary
	 *      table is many-to-one.
	 *     
	 *      When this property is set false, a SQL statement will be executed
	 *      for each HAS_MANY relation.
	 *     
	...
	/**
	 *
	 * @var string the name of the AR attribute whose value should be used as
	 *      index of the query result array.
	 *      Defaults to null, meaning the result array will be zero-based
	 *      integers.
	 * @since 1.1.5
	 */
	public $index;
	 ...
	 *      <li>Multiple scopes:
	 *      $criteria->scopes=array('scopeName1','scopeName2');</li>
	 *      <li>Scope with parameters:
	 *      $criteria->scopes=array('scopeName'=>array($params));</li>
	 ...
	 *        	the new condition. It can be either a string or an array of
	 *        	strings.
	 ...
	 *        	the search keyword. This interpretation of the keyword is
	 *        	affected by the next parameter.
	 * @param boolean $escape
	 *        	whether the keyword should be escaped if it contains
	 *        	characters % or _.
	 *        	When this parameter is true (default), the special characters
	 *        	% (matches 0 or more characters)
	 *        	and _ (matches a single character) will be escaped, and the
	 *        	keyword will be surrounded with a %
	 *        	character on both ends. When this parameter is false, the
	 *        	keyword will be directly used for
	 *        	matching without any change.
	 * @param string $operator
	 *        	the operator used to concatenate the new condition with the
	 *        	existing one.
	 *        	Defaults to 'AND'.
	 * @param string $like
	 *        	the LIKE operator. Defaults to 'LIKE'. You may also set this
	 *        	to be 'NOT LIKE'.
	 * @return CDbCriteria the criteria object itself
	 */
	...
	public function addInCondition($column, $values, $operator = 'AND') {
		if (($n = count ( $values )) < 1)
			$condition = '0=1'; // 0=1 is used because in MSSQL value alone can't be
			                  // used in WHERE
		elseif ($n === 1) {
			$value = reset ( $values );
			if ($value === null)
	 ...
	 *        	the operator to concatenate multiple column matching
	 ...
	 *
	 * The comparison operator is intelligently determined based on the first
	 * few
	 ...
	 * <li><code>&gt;</code>: the column must be greater than the given
	 * value.</li>
	 * <li><code>&lt;=</code>: the column must be less than or equal to the
	 * given value.</li>
	 * <li><code>&gt;=</code>: the column must be greater than or equal to the
	 * given value.</li>
	 * <li><code>&lt;&gt;</code>: the column must not be the same as the given
	 * value.
	 * Note that when $partialMatch is true, this would mean the value must not
	 * be a substring
	 * of the column.</li>
	 * <li><code>=</code>: the column must be equal to the given value.</li>
	 * <li>none of the above: the column must be equal to the given value. Note
	 * that when $partialMatch
	 * is true, this would mean the value must be the same as the given value or
	 * be a substring of it.</li>
	 * </ul>
	 *
	 * Note that any surrounding white spaces will be removed from the value
	 * before comparison.
	 ...
	 * @param mixed $value
	 *        	the column value to be compared with. If the value is a
	 *        	string, the aforementioned
	 *        	intelligent comparison will be conducted. If the value is an
	 *        	array, the comparison is done
	 *        	by exact match of any of the value in the array. If the string
	 *        	or the array is empty,
	 *        	the existing search condition will not be modified.
	 * @param boolean $partialMatch
	 *        	whether the value should consider partial text match (using
	 ...
	 *        	Defaults to 'AND'.
	 * @param boolean $escape
	 *        	whether the value should be escaped if $partialMatch is true
	 *        	and
	 *        	the value contains characters % or _. When this parameter is
	 *        	true (default),
	 *        	the special characters % (matches 0 or more characters)
	 *        	and _ (matches a single character) will be escaped, and the
	 *        	value will be surrounded with a %
	 *        	character on both ends. When this parameter is false, the
	 *        	value will be directly used for
	 *        	matching without any change.
	 * @return CDbCriteria the criteria object itself
	 * @since 1.1.1
	 */
	 ...
	 *        	the criteria to be merged with. Either an array or
	 *        	CDbCriteria.
	 * @param string|boolean $operator
	 *        	the operator used to concatenate where and having conditions.
	 *        	Defaults to 'AND'.
