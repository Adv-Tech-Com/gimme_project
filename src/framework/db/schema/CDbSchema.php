 ...
 *
 * @property CDbConnection $dbConnection Database connection. The connection is
 *           active.
 * @property array $tables The metadata for all tables in the database.
 *           Each array element is an instance of {@link CDbTableSchema} (or its
 *           child class).
 *           The array keys are table names.
 * @property array $tableNames All table names in the database.
 * @property CDbCommandBuilder $commandBuilder The SQL command builder for this
 *           connection.
 *          
 * @author Qiang Xue <qiang.xue@gmail.com>
...
abstract class CDbSchema extends CComponent {
	 ...
	 * @param string $name
	 *        	table name
	 * @return CDbTableSchema driver dependent table metadata, null if the table
	 *         does not exist.
	 */
	 ...
	 * @return CDbTableSchema table metadata. Null if the named table does not
	 *         exist.
	 */
	 ...
	 * 
	 * @param string $schema
	 *        	the schema of the tables. Defaults to empty string, meaning
	 *        	the current or default schema.
	 * @return array the metadata for all tables in the database.
	 *         Each array element is an instance of {@link CDbTableSchema} (or
	 *         its child class).
	 *         The array keys are table names.
	 */
	 ...
	 *        	the schema of the tables. Defaults to empty string, meaning
	 *        	the current or default schema.
	 *        	If not empty, the returned table names will be prefixed with
	 *        	the schema name.
	 ...
	 * inserted
	 * will have the specified value or max value of a primary key plus one
	 * (i.e. sequence trimming).
	 * 
	 ...
	 *        	If this is not set, the next new row's primary key will have
	 *        	the max value of a primary
	...
	/**
	 * Returns all table names in the database.
	 * This method should be overridden by child classes in order to support
	 * this feature
	 ...
	 *        	If not empty, the returned table names will be prefixed with
	 *        	the schema name.
	 * @throws CDbException if current schema does not support fetching all
	 *         table names
	 * @return array all table names in the database.
	 */
	 ...
	 * columnTypes}.
	 * These abstract column types are supported (using MySQL as example to
	 * explain the corresponding
	 * physical types):
	 * <ul>
	 * <li>pk: an auto-incremental primary key type, will be converted into
	 * "int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY"</li>
	 ...
	 * "string NOT NULL"), then only
	 * the first part will be converted, and the rest of the parts will be
	 * appended to the conversion result.
	 ...
	 *        	RESTRICT, CASCADE, NO ACTION, SET DEFAULT, SET NULL
	 * @return string the SQL statement for adding a foreign key constraint to
	 *         an existing table.
	 * @since 1.1.6
	 */
	 ...
	 *        	by commas. Each column name will be properly quoted by the
	 ...
	 *        	comma separated string or array of columns that the primary
	 *        	key will consist of.
	 *        	Array value can be passed since 1.1.14.
	 * @return string the SQL statement for adding a primary key constraint to
	 *         an existing table.
	 * @since 1.1.13
	 */
	...
	/**
	 * Builds a SQL statement for removing a primary key constraint to an
	 * existing table.
	 * 
	 * @param string $name
	 *        	the name of the primary key constraint to be removed.
	 * @param string $table
	 *        	the table that the primary key constraint will be removed
	 *        	from.
	 * @return string the SQL statement for removing a primary key constraint
