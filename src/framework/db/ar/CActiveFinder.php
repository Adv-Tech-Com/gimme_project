...
class CActiveFinder extends CComponent {
	...
	/**
	 * Constructor.
	 * A join tree is built up based on the declared relationships between
	 ...
	 * This method is internally invoked by {@link CActiveRecord} to support
	 * lazy loading.
	 * 
	...
	/**
	 * Builds up the join tree representing the relationships involved in this
	 * query.
	 * 
	 * @param CJoinElement $parent
	 *        	the parent tree node
	 * @param mixed $with
	 *        	the names of the related objects relative to the parent tree
	 *        	node
	 * @param array $options
	 *        	additional query options to be merged with the relation
	 * @throws CDbException if given parent tree node is an instance of {@link
	 *         CStatElement}
	 *         or relation is not defined in the given parent's tree node model
	 *         class
	 */
	private function buildJoinTree($parent, $with, $options = null) {
		...
		if (is_string ( $with )) {
			...
			if (! empty ( $relation->scopes ))
				$scopes = array_merge ( $scopes, ( array ) $relation->scopes ); // no need
				                                                       // for complex
				                                                       // merging
			
			if (! empty ( $options ['scopes'] ))
				$scopes = array_merge ( $scopes, ( array ) $options ['scopes'] ); // no need
				                                                        // for complex
				                                                        // merging
			...
			// select has a special meaning in stat relation, so we need to
			// ignore select from scope or model criteria
...
/**
 * CJoinElement represents a tree node in the join tree created by {@link
 * CActiveFinder}.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.db.ar
...
class CJoinElement {
	...
	/**
	 *
	 * @var array list of active records found by the queries. They are indexed
	 ...
	 *        	or the model (if the third parameter is null) associated with
	 *        	this tree node.
	 * @param CJoinElement $parent
	 *        	the parent tree node
	 * @param integer $id
	 *        	the ID of this tree node that is unique among all the tree
	 *        	nodes
	 */
	...
	/**
	 * Removes references to child elements and finder to avoid circular
	 * references.
	...
	public function find($criteria = null) {
		if ($this->_parent === null) 		// root element
		{
			...
			$this->runQuery ( $query );
		} elseif (! $this->_joined && ! empty ( $this->_parent->records )) 		// not joined
		                                                           // before
		{
	 ...
	 *        	the active record whose related object is to be fetched.
	 * @throws CDbException if relation in active record class is not specified
	 *         correctly
	 */
	 ...
	 * @param CDbCriteria $criteria
	 *        	the query criteria
	 * @return string number of primary records. Note: type is string to keep
	 *         max. precision.
	 */
	...
	private function populateRecord($query, $row) {
		...
		foreach ( $this->children as $child ) {
			...
			{
				// need to double check to avoid adding duplicated related
				// objects
	...
	/**
	 *
	 * @return string the table name and the table alias (if any). This can be
	 *         used directly in SQL query without escaping.
	 */
	 ...
	 * Columns will be properly aliased and primary keys will be added to
	 * selection if they are not specified.
	 * 
	 * @param mixed $select
	 *        	columns to be selected. Defaults to '*', indicating all
	 *        	columns.
	 * @throws CDbException if active record class is trying to select an
	 *         invalid column
	...
	public function getColumnSelect($select = '*') {
		...
		if ($select === '*') {
			...
			foreach ( $select as $name ) {
				...
				} elseif (preg_match ( '/^(.*?)\s+AS\s+(\w+)$/im', $name, $matches )) 				// if
				                                                              // the column
				                                                              // is already
				                                                              // aliased
				{
	...
	/**
	 *
	 * @return string the condition that specifies only the rows with the
...
class CStatElement {
	...
	/*
	 * @param string $joinTableName jointablename @param string $keys keys
	 */
