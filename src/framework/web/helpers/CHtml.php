...
class CHtml {
	...
	/**
	 *
	 * @var string the CSS class for displaying error summaries (see {@link
	 *      errorSummary}).
	 */
	public static $errorSummaryCss = 'errorSummary';
	/**
	 *
	 * @var string the CSS class for displaying error messages (see {@link
	 *      error}).
	 */
	public static $errorMessageCss = 'errorMessage';
	/**
	 *
	 * @var string the CSS class for highlighting error inputs. Form inputs will
	 *      be appended
	 ...
	 * Event handlers are attached directly to the DOM element, that must
	 * already exist
	 * on the page. Elements injected into the page at a later time will not be
	 * processed.
	 *
	 * You can override this setting for a particular element by setting the
	 * htmlOptions delegate attribute
	 * (see {@link clientChange}).
	 *
	 * For more information about attaching jQuery event handler see {@link
	...
	/**
	 *
	 * @var boolean whether to close single tags. Defaults to true. Can be set
	 *      to false for HTML5.
	 * @since 1.1.13
	 */
	...
	/**
	 *
	 * @var callback the generator used in the {@link CHtml::modelName()}
	 *      method.
	 * @since 1.1.14
	 */
	 ...
	 * The {@link CApplication::charset application charset} will be used for
	 * encoding.
	 * 
	 * @param string $text
	 *        	data to be encoded
	 ...
	 *        	the element attributes. The values will be HTML-encoded using
	 *        	{@link encode()}.
	 *        	If an 'encode' attribute is given and its value is false,
	 *        	the rest of the attribute values will NOT be HTML-encoded.
	 *        	Since version 1.1.5, attributes whose value is null will not
	 *        	be rendered.
	 * @param mixed $content
	 *        	the content to be enclosed between open and close element
	 *        	tags. It will not be HTML-encoded.
	...
	/**
	 * Generates a meta tag that can be inserted in the head section of HTML
	 * page.
	 * 
	 * @param string $content
	 *        	content attribute of the meta tag
	 * @param string $name
	 *        	name attribute of the meta tag. If null, the attribute will
	 *        	not be generated
	 * @param string $httpEquiv
	 *        	http-equiv attribute of the meta tag. If null, the attribute
	...
	/**
	 * Generates a link tag that can be inserted in the head section of HTML
	 * page.
	 * Do not confuse this method with {@link link()}. The latter generates a
	 * hyperlink.
	 * 
	 * @param string $relation
	 *        	rel attribute of the link tag. If null, the attribute will not
	 *        	be generated.
	 * @param string $type
	 *        	type attribute of the link tag. If null, the attribute will
	 *        	not be generated.
	 * @param string $href
	 *        	href attribute of the link tag. If null, the attribute will
	 *        	not be generated.
	 * @param string $media
	 *        	media attribute of the link tag. If null, the attribute will
	 *        	not be generated.
	 * @param array $options
	 ...
	 * This method can be invoked anywhere in a view. It will register a
	 * 'refresh'
	 ...
	 * @param string $url
	 *        	the URL to which the page should be redirected to. If empty,
	 *        	it means the current page.
	 * @since 1.1.1
	 */
	 ...
	 * 
	 * @param mixed $action
	 *        	the form action URL (see {@link normalizeUrl} for details
	...
	/**
	 * Generates an opening form tag.
	 * Note, only the open tag is generated. A close tag should be placed
	 * manually
	 ...
	 * A stateful form tag is similar to {@link form} except that it renders an
	 * additional
	 * hidden field for storing persistent page states. You should use this
	 * method to generate
	 * a form tag if you want to access persistent page states when the form is
	 * submitted.
	 * 
	 * @param mixed $action
	 *        	the form action URL (see {@link normalizeUrl} for details
	 ...
	 * 
	 * @param string $text
	 *        	link body. It will NOT be HTML-encoded. Therefore you can pass
	 *        	in HTML code such as an image tag.
	 * @param mixed $url
	 *        	a URL or an action route that can be used to create a URL.
	 *        	See {@link normalizeUrl} for more details about how to specify
	 *        	this parameter.
	 * @param array $htmlOptions
	 *        	additional HTML attributes. Besides normal HTML attributes, a
	 *        	few special
	 *        	attributes are also recognized (see {@link clientChange} and
	 *        	{@link tag} for more details.)
	 ...
	 *        	in HTML code such as an image tag.
	 * @param string $email
	 *        	email address. If this is empty, the first parameter (link
	 *        	body) will be treated as the email address.
	 * @param array $htmlOptions
	 *        	additional HTML attributes. Besides normal HTML attributes, a
	 *        	few special
	 *        	attributes are also recognized (see {@link clientChange} and
	 ...
	 * This method is similar to {@link button} except that it generates a
	 * 'button'
	 * tag instead of 'input' tag.
	 * 
	 * @param string $label
	 *        	the button label. Note that this value will be directly
	 *        	inserted in the button element
	 ...
	 *        	If this is false, the 'for' attribute for the label tag will
	 *        	not be rendered.
	 * @param array $htmlOptions
	 *        	additional HTML attributes.
	 *        	The following HTML option is recognized:
	 *        	<ul>
	 *        	<li>required: if this is set and is true, the label will be
	 *        	styled
	 *        	with CSS class 'required' (customizable with
	 *        	CHtml::$requiredCss),
	 ...
	 * Note, you have to set the enclosing form's 'enctype' attribute to be
	 * 'multipart/form-data'.
	 * After the form is submitted, the uploaded file information can be
	 * obtained via $_FILES[$name] (see
	 * PHP documentation).
	 * 
	 * @param string $name
	 *        	the input name
	 ...
	 *        	attributes are also recognized (see {@link clientChange} and
	 *        	{@link tag} for more details.)
	 *        	Since version 1.1.2, a special option named 'uncheckValue' is
	 *        	available that can be used to specify
	 *        	the value returned when the radio button is not checked. When
	 *        	set, a hidden field is rendered so that
	 *        	when the radio button is not checked, we can still obtain the
	 *        	posted uncheck value.
	 *        	If 'uncheckValue' is not set or set to NULL, the hidden field
	...
	public static function radioButton($name, $checked = false, $htmlOptions = array()) {
		...
		if ($uncheck !== null) {
			// add a hidden field so that if the radio button is not selected,
	 ...
	 *        	Since version 1.1.2, a special option named 'uncheckValue' is
	 *        	available that can be used to specify
	 *        	the value returned when the checkbox is not checked. When set,
	 *        	a hidden field is rendered so that
	 *        	when the checkbox is not checked, we can still obtain the
	 *        	posted uncheck value.
	 *        	If 'uncheckValue' is not set or set to NULL, the hidden field
	 *        	will not be rendered.
	...
	public static function checkBox($name, $checked = false, $htmlOptions = array()) {
		...
		if ($uncheck !== null) {
			// add a hidden field so that if the check box is not checked, it
			// still submits a value
			if (isset ( $htmlOptions ['id'] ) && $htmlOptions ['id'] !== false)
				$uncheckOptions = array (
						'id' => self::ID_PREFIX . $htmlOptions ['id'] 
				);
	 ...
	 *        	Please refer to {@link listOptions} on how this data is used
	 *        	to generate the list options.
	 *        	Note, the values and labels will be automatically HTML-encoded
	 *        	by this method.
	 * @param array $htmlOptions
	 *        	additional HTML attributes. Besides normal HTML attributes, a
	 *        	few special
	 *        	attributes are recognized. See {@link clientChange} and {@link
	 *        	tag} for more details.
	 *        	In addition, the following options are also supported
	 *        	specifically for dropdown list:
	 *        	<ul>
	 *        	<li>encode: boolean, specifies whether to encode the values.
	 *        	Defaults to true.</li>
	 *        	<li>prompt: string, specifies the prompt text shown as the
	 *        	first list option. Its value is empty. Note, the prompt text
	 *        	will NOT be HTML-encoded.</li>
	 *        	<li>empty: string, specifies the text corresponding to empty
	 *        	selection. Its value is empty.
	 *        	The 'empty' option can also be an array of value-label pairs.
	 *        	Each pair will be used to render a list option at the
	 *        	beginning. Note, the text label will NOT be HTML-encoded.</li>
	 *        	<li>options: array, specifies additional attributes for each
	 *        	OPTION tag.
	 *        	The array keys must be the option values, and the array values
	 *        	are the extra
	 ...
	 *        	</li>
	 *        	</ul>
	 *        	Since 1.1.13, a special option named 'unselectValue' is
	 *        	available. It can be used to set the value
	 *        	that will be returned when no option is selected in multiple
	 *        	mode. When set, a hidden field is
	 *        	rendered so that if no option is selected in multiple mode, we
	 *        	can still obtain the posted
	 *        	unselect value. If 'unselectValue' is not set or set to NULL,
	 *        	the hidden field will not be rendered.
	...
	public static function dropDownList($name, $select, $data, $htmlOptions = array()) {
		...
		// add a hidden field so that if the option is not selected, it still
		// submits a value
	 ...
	 *        	the input name
	 * @param mixed $select
	 *        	the selected value(s). This can be either a string for single
	 *        	selection or an array for multiple selections.
	 * @param array $data
	 *        	data for generating the list options (value=>display)
	 *        	You may use {@link listData} to generate this data.
	 *        	Please refer to {@link listOptions} on how this data is used
	 ...
	 *        	Note, the values will be automatically HTML-encoded, while the
	 *        	labels will not.
	 * @param array $htmlOptions
	 *        	additional HTML options. The options will be applied to
	 *        	each checkbox input. The following special options are
	 *        	recognized:
	 *        	<ul>
	 *        	<li>template: string, specifies how each checkbox is rendered.
	 *        	Defaults
	 *        	to "{input} {label}", where "{input}" will be replaced by the
	 *        	generated
	 *        	check box input tag while "{label}" be replaced by the
	 *        	corresponding check box label,
	 *        	{beginLabel} will be replaced by &lt;label&gt; with
	 *        	labelOptions, {labelTitle} will be replaced
	 *        	by the corresponding check box label title and {endLabel} will
	 *        	be replaced by &lt;/label&gt;</li>
	 *        	<li>separator: string, specifies the string that separates the
	 *        	generated check boxes.</li>
	 *        	<li>checkAll: string, specifies the label for the "check all"
	 *        	checkbox.
	 *        	If this option is specified, a 'check all' checkbox will be
	 *        	displayed. Clicking on
	 *        	this checkbox will cause all checkboxes checked or
	 *        	unchecked.</li>
	 *        	<li>checkAllLast: boolean, specifies whether the 'check all'
	 *        	checkbox should be
	 *        	displayed at the end of the checkbox list. If this option is
	 *        	not set (default)
	 *        	or is false, the 'check all' checkbox will be displayed at the
	 *        	beginning of
	 *        	the checkbox list.</li>
	 *        	<li>labelOptions: array, specifies the additional HTML
	 *        	attributes to be rendered
	 *        	for every label tag in the list.</li>
	 *        	<li>container: string, specifies the checkboxes enclosing tag.
	 *        	Defaults to 'span'.
	 *        	If the value is an empty string, no enclosing tag will be
	 *        	generated</li>
	 *        	<li>baseID: string, specifies the base ID prefix to be used
	 *        	for checkboxes in the list.
...
EOD;
	 ...
	 * A radio button list is like a {@link checkBoxList check box list}, except
	 * that
	 * it only allows single selection.
	 * 
	 * @param string $name
	 *        	name of the radio button list. You can use this name to
	 *        	retrieve
	 *        	the selected value(s) once the form is submitted.
	 * @param string $select
	 ...
	 * @param array $htmlOptions
	 *        	additional HTML options. The options will be applied to
	 *        	each radio button input. The following special options are
	 *        	recognized:
	 *        	<ul>
	 *        	<li>template: string, specifies how each radio button is
	 *        	rendered. Defaults
	 *        	to "{input} {label}", where "{input}" will be replaced by the
	 *        	generated
	 *        	radio button input tag while "{label}" will be replaced by the
	 *        	corresponding radio button label,
	 *        	{beginLabel} will be replaced by &lt;label&gt; with
	 *        	labelOptions, {labelTitle} will be replaced
	 *        	by the corresponding radio button label title and {endLabel}
	 *        	will be replaced by &lt;/label&gt;</li>
	 *        	<li>separator: string, specifies the string that separates the
	 *        	generated radio buttons. Defaults to new line (<br/>).</li>
	 *        	<li>labelOptions: array, specifies the additional HTML
	 *        	attributes to be rendered
	 *        	for every label tag in the list.</li>
	 *        	<li>container: string, specifies the radio buttons enclosing
	 *        	tag. Defaults to 'span'.
	 *        	If the value is an empty string, no enclosing tag will be
	 *        	generated</li>
	 *        	<li>baseID: string, specifies the base ID prefix to be used
	 *        	for radio buttons in the list.
	 *        	This option is available since version 1.1.13.</li>
	 *        	<li>empty: string, specifies the text corresponding to empty
	 *        	selection. Its value is empty.
	 *        	The 'empty' option can also be an array of value-label pairs.
	 *        	Each pair will be used to render a radio button at the
	 *        	beginning. Note, the text label will NOT be HTML-encoded.
	 ...
	 * @param mixed $url
	 *        	the URL for the AJAX request. If empty, it is assumed to be
	 *        	the current URL. See {@link normalizeUrl} for more details.
	 ...
	 * 
	 * @param array $options
	 *        	AJAX options. The valid options are used in the form of
	 *        	jQuery.ajax([settings])
	 *        	as specified in the jQuery AJAX documentation.
	 *        	The following special options are added for convenience:
	 *        	<ul>
	 *        	<li>update: string, specifies the selector whose HTML content
	 *        	should be replaced
	 *        	by the AJAX request result.</li>
	 *        	<li>replace: string, specifies the selector whose target
	 *        	should be replaced
	 *        	by the AJAX request result.</li>
	 *        	</ul>
	 *        	Note, if you specify the 'success' option, the above options
	 *        	will be ignored.
	 ...
	 *        	whether the published directory should be named as the hashed
	 *        	basename.
	 *        	If false, the name will be the hashed dirname of the path
	 *        	being published.
	 *        	Defaults to false. Set true if the path being published is
	 *        	shared among
	 ...
	 *
	 * If the input parameter is an empty string, the currently requested URL
	 * will be returned.
	 *
	 * If the input parameter is a non-empty string, it is treated as a valid
	 * URL and will
	 * be returned without any change.
	 *
	 * If the input parameter is an array, it is treated as a controller route
	 * and a list of
	 * GET parameters, and the {@link CController::createUrl} method will be
	 * invoked to
	 * create a URL. In this case, the first array element refers to the
	 * controller route,
	 * and the rest key-value pairs refer to the additional GET parameters for
	 * the URL.
	 * For example, <code>array('post/list', 'page'=>3)</code> may be used to
	 * generate the URL
	 ...
	 * This method generates an input HTML tag based on the given input name and
	 * value.
	 * 
	 * @param string $type
	 ...
	 * If the attribute has input error, the label's CSS class will be appended
	 * with {@link errorCss}.
	 * 
	 * @param CModel $model
	 *        	the data model
	 ...
	 *        	the attribute
	 * @param array $htmlOptions
	 *        	additional HTML attributes. The following special options are
	 *        	recognized:
	 *        	<ul>
	 *        	<li>required: if this is set and is true, the label will be
	 *        	styled
	 ...
	 *        	and be decorated with {@link CHtml::beforeRequiredLabel} and
	 *        	{@link CHtml::afterRequiredLabel}.</li>
	 *        	<li>label: this specifies the label to be displayed. If this
	 *        	is not set,
	 *        	{@link CModel::getAttributeLabel} will be called to get the
	 *        	label for display.
	 *        	If the label is specified as false, no label will be
	 *        	rendered.</li>
	 *        	</ul>
	 * @return string the generated label tag
	 */
	...
	/**
	 * Generates a label tag for a model attribute.
	 * This is an enhanced version of {@link activeLabel}. It will render
	 * additional
	...
	public static function activeFileField($model, $attribute, $htmlOptions = array()) {
		...
		// still use isset($_POST[$modelClass]) to detect if the input is
		// submitted
		$hiddenOptions = isset ( $htmlOptions ['id'] ) ? array (
				'id' => self::ID_PREFIX . $htmlOptions ['id'] 
		) : array (
	 ...
	 *        	attributes are also recognized (see {@link clientChange} and
	 *        	{@link tag} for more details.)
	 *        	A special option named 'uncheckValue' is available that can be
	 *        	used to specify
	 *        	the value returned when the radio button is not checked. By
	 *        	default, this value is '0'.
	 *        	Internally, a hidden field is rendered so that when the radio
	 *        	button is not checked,
	 *        	we can still obtain the posted uncheck value.
	 *        	If 'uncheckValue' is set as NULL, the hidden field will not be
	 *        	rendered.
	 * @return string the generated radio button
	 * @see clientChange
	 * @see activeInputField
	...
	public static function activeRadioButton($model, $attribute, $htmlOptions = array()) {
		...
		// add a hidden field so that if the radio button is not selected, it
		// still submits a value
	 ...
	 *        	A special option named 'uncheckValue' is available that can be
	 *        	used to specify
	 *        	the value returned when the checkbox is not checked. By
	 *        	default, this value is '0'.
	 *        	Internally, a hidden field is rendered so that when the
	 ...
	 * If the attribute has input error, the input field's CSS class will
	 * be appended with {@link errorCss}.
	 * Note that a check box list allows multiple selection, like {@link
	 * listBox}.
	 * As a result, the corresponding POST value is an array. In case no
	 * selection
	 ...
	 *        	to "{input} {label}", where "{input}" will be replaced by the
	 *        	generated
	 *        	check box input tag while "{label}" will be replaced by the
	 *        	corresponding check box label.</li>
	 *        	<li>separator: string, specifies the string that separates the
	 *        	generated check boxes.</li>
	 *        	<li>checkAll: string, specifies the label for the "check all"
	 *        	checkbox.
	 ...
	 *        	beginning of
	 *        	the checkbox list.</li>
	 *        	<li>encode: boolean, specifies whether to encode HTML-encode
	 *        	tag attributes and values. Defaults to true.</li>
	 *        	</ul>
	 *        	Since 1.1.7, a special option named 'uncheckValue' is
	 *        	available. It can be used to set the value
	 *        	that will be returned when the checkbox is not checked. By
	 *        	default, this value is ''.
	 *        	Internally, a hidden field is rendered so when the checkbox is
	 *        	not checked, we can still
	 *        	obtain the value. If 'uncheckValue' is set to NULL, there will
	 *        	be no hidden field rendered.
	 * @return string the generated check box list
	 * @see checkBoxList
	 */
	 ...
	 *        	generated
	 *        	radio button input tag while "{label}" will be replaced by the
	 *        	corresponding radio button label.</li>
	 *        	<li>separator: string, specifies the string that separates the
	 *        	generated radio buttons. Defaults to new line (<br/>).</li>
	 *        	<li>encode: boolean, specifies whether to encode HTML-encode
	 *        	tag attributes and values. Defaults to true.</li>
	 *        	</ul>
	 *        	Since version 1.1.7, a special option named 'uncheckValue' is
	 *        	available that can be used to specify the value
	 *        	returned when the radio button is not checked. By default,
	 *        	this value is ''. Internally, a hidden field is
	 *        	rendered so that when the radio button is not checked, we can
	 *        	still obtain the posted uncheck value.
	 *        	If 'uncheckValue' is set as NULL, the hidden field will not be
	 *        	rendered.
	 * @return string the generated radio button list
	 * @see radioButtonList
	 ...
	 * 
	 * @param mixed $model
	 *        	the models whose input errors are to be displayed. This can be
	 *        	either
	 ...
	 * @param array $htmlOptions
	 *        	additional HTML attributes to be rendered in the container div
	 *        	tag.
	 *        	A special option named 'firstError' is recognized, which when
	 *        	set true, will
	 *        	make the error summary to show only the first error message of
	 *        	each attribute.
	 *        	If this is not set or is false, all error messages will be
	 *        	displayed.
	 ...
	 *        	the attribute name
	 * @param array $htmlOptions
	 *        	additional HTML attributes to be rendered in the container
	 *        	tag.
	 ...
	 * The generated data can be used in {@link dropDownList}, {@link listBox},
	 * {@link checkBoxList},
	 * {@link radioButtonList}, and their active-versions (such as {@link
	 * activeDropDownList}).
	 * Note, this method does not HTML-encode the generated data. You may call
	 * {@link encodeArray} to
	 * encode it if needed.
	 * Please refer to the {@link value} method on how to specify value field,
	 * text field and group field.
	 * You can also pass anonymous functions as second, third and fourth
	 * arguments which calculates
	 * text field value (PHP 5.3+ only) since 1.1.13. Your anonymous function
	 ...
	 *        	can also be an array of associative arrays (e.g. results of
	 *        	{@link CDbCommand::queryAll}).
	 * @param mixed $valueField
	 *        	the attribute name or anonymous function (PHP 5.3+) for list
	 *        	option values
	 ...
	 *        	the attribute name or anonymous function (PHP 5.3+) for list
	 *        	option group names. If empty, no group will be generated.
	 * @return array the list data that can be used in {@link dropDownList},
	 *         {@link listBox}, etc.
	 */
	 ...
	 * The attribute name can be given in a dot syntax. For example, if the
	 * attribute
	 * is "author.firstName", this method will return the value of
	 * "$model->author->firstName".
	 * A default value (passed as the last parameter) will be returned if the
	 * attribute does
	 * not exist or is broken in the middle (e.g. $model->author is null).
	 * The model can be either an object or an array. If the latter, the
	 * attribute is treated
	 * as a key of the array. For the example of "author.firstName", if would
	 * mean the array value
	 * "$model['author']['firstName']".
	 *
	 * Anonymous function could also be used for attribute calculation since
	 * 1.1.13
	 ...
	 * });
	 * </pre>
	 * Your anonymous function should receive one argument, which is the model,
	 * the current
	 * value is calculated from. This feature could be used together with the
	 * {@link listData}.
	 ...
	 *        	the attribute name (use dot to concatenate multiple
	 *        	attributes)
	 *        	or anonymous function (PHP 5.3+). Remember that functions
	 *        	created by "create_function"
	 *        	are not supported by this method. Also note that numeric value
	 *        	is meaningless when
	 ...
	 *        	the new generator, the model or class name will be passed to
	 *        	the this callback
	 ...
	 * Unlike {@link resolveName}, this method does NOT modify the attribute
	 * name.
	 * 
	 * @param CModel $model
	 *        	the data model
	 ...
	 * This method generates an input HTML tag based on the given data model and
	 * attribute.
	 * If the attribute has input error, the input field's CSS class will
	 * be appended with {@link errorCss}.
	 ...
	 * @param array $htmlOptions
	 *        	additional HTML attributes. The following two special
	 ...
	 *        	</pre>
	 *        	</li>
	 *        	<li>key: string, specifies the name of key attribute of the
	 *        	selection object(s).
	 *        	This is used when the selection is represented in terms of
	 *        	objects. In this case,
	 *        	the property named by the key option of the objects will be
	 *        	treated as the actual selection value.
	 *        	This option defaults to 'primaryKey', meaning using the
	 *        	'primaryKey' property value of the objects in the selection.
	 ...
	 * @param array $htmlOptions
	 *        	HTML attributes which may contain the following special
	 *        	attributes
	 *        	specifying the client change behaviors:
	 *        	<ul>
	 *        	<li>submit: string, specifies the URL to submit to. If the
	 *        	current element has a parent form, that form will be
	 *        	submitted, and if 'submit' is non-empty its value will replace
	 *        	the form's URL. If there is no parent form the
	 *        	data listed in 'params' will be submitted instead (via POST
	 *        	method), to the URL in 'submit' or the currently
	 *        	requested URL if 'submit' is empty. Please note that if the
	 *        	'csrf' setting is true, the CSRF token will be
	 *        	included in the params too.</li>
	 *        	<li>params: array, name-value pairs that should be submitted
	 *        	together with the form. This is only used when 'submit' option
	 *        	is specified.</li>
	 *        	<li>csrf: boolean, whether a CSRF token should be
	 *        	automatically included in 'params' when {@link
	 *        	CHttpRequest::enableCsrfValidation} is true. Defaults to
	 *        	false.
	 *        	You may want to set this to be true if there is no enclosing
	 *        	form around this element.
	 *        	This option is meaningful only when 'submit' option is
	 *        	set.</li>
	 *        	<li>return: boolean, the return value of the javascript.
	 *        	Defaults to false, meaning that the execution of
	 *        	javascript would not cause the default behavior of the
	 *        	event.</li>
	 *        	<li>confirm: string, specifies the message that should show in
	 *        	a pop-up confirmation dialog.</li>
	 *        	<li>ajax: array, specifies the AJAX options (see {@link
	 *        	ajax}).</li>
	 *        	<li>live: boolean, whether the event handler should be
	 *        	delegated or directly bound.
	 *        	If not set, {@link liveEvents} will be used. This option has
	 *        	been available since version 1.1.11.</li>
	 *        	</ul>
	 ...
	 * This method will update the HTML options by setting appropriate 'name'
	 * and 'id' attributes.
	 ...
	 * Note, the attribute name may be modified after calling this method if the
	 * name
	 * contains square brackets (mainly used in tabular input) before the real
	 * attribute name.
	 * 
	 * @param CModel $model
	 *        	the data model
	 ...
	 * Special attributes, such as 'checked', 'disabled', 'readonly', will be
	 * rendered
