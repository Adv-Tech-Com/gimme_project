 ...
 *
 * When calling {@link CBaseController::createWidget}, {@link
 * CBaseController::widget}
 * or {@link CBaseController::beginWidget}, if the "widgetFactory" component is
 * installed,
 * it will be used to create the requested widget. To install the
 ...
 * according to the widget class name and the widget {@link CWidget::skin}
 * property.
 * If a skin is found, it will be merged with the initial properties passed via
 * {@link createWidget}. Then the merged initial properties will be used to
 * initialize
 ...
 * Skins are stored in PHP scripts like other configurations. Each script file
 * stores the skins
 * for a particular widget type and is named as the widget class name (e.g.
 * CLinkPager.php).
 * Each widget type may have one or several skins, identified by the skin name
 * set via
 * {@link CWidget::skin} property. If the {@link CWidget::skin} property is not
 * set for a given
 * widget, it means the default skin would be used. The following shows the
 * possible skins for
 ...
 * indexed by the string "default".
 * Note that {@link CWidget::skin} defaults to "default". Therefore, this is the
 * skin that will be applied
 * if we do not explicitly specify the {@link CWidget::skin} property.
 * The second one is named as the "short" skin which will be used only when we
 * set {@link CWidget::skin}
 * to be "short".
 *
 * By default, CWidgetFactory looks for the skin of a widget under the "skins"
 * directory
 * of the current application's {@link CWebApplication::viewPath} (e.g.
 * protected/views/skins).
 * If a theme is being used, it will look for the skin under the "skins"
 * directory of
 * the theme's {@link CTheme::viewPath} (as well as the aforementioned skin
 * directory).
...
class CWidgetFactory extends CApplicationComponent implements IWidgetFactory {
	 ...
	 *      represents the initial property values for a single widget class,
	 *      with
	 *      the array key being the widget class name, and array value being the
	 *      initial
	 ...
	 *      They may also be overridden by widget skins, if {@link enableSkin}
	 *      is true.
	 * @since 1.1.3
	 */
	 ...
	 *      Because skinning widgets has performance impact, you may want to
	 *      specify this property
	 *      to limit skinning only to specific widgets. Any widgets that are not
	 *      in this list
	 *      will not be skinned. Defaults to null, meaning all widgets can be
	 *      skinned.
	 * @since 1.1.3
	 */
	...
	/**
	 *
	 * @var string the directory containing all the skin files. Defaults to
	 *      null,
	 *      meaning using the "skins" directory under the current application's
	 *      {@link CWebApplication::viewPath}.
	 */
	public $skinPath;
	 ...
	 * This method overrides the parent implementation by resolving the skin
	 * path.
	 */
