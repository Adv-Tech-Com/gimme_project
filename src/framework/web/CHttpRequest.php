 ...
 *
 * CHttpRequest is a default application component loaded by {@link
 * CWebApplication}. It can be
 * accessed via {@link CWebApplication::getRequest()}.
 *
 * @property string $url Part of the request URL after the host info.
 * @property string $hostInfo Schema and hostname part (with port number if
 *           needed) of the request URL (e.g. http://www.yiiframework.com).
 * @property string $baseUrl The relative URL for the application.
 * @property string $scriptUrl The relative URL of the entry script.
 * @property string $pathInfo Part of the request URL that is after the entry
 *           script and before the question mark.
 ...
 * @property string $requestUri The request URI portion for the currently
 *           requested URL.
 * @property string $queryString Part of the request URL that is after the
 *           question mark.
 * @property boolean $isSecureConnection If the request is sent via secure
 *           channel (https).
 * @property string $requestType Request type, such as GET, POST, HEAD, PUT,
 *           DELETE.
 ...
 * @property boolean $isAjaxRequest Whether this is an AJAX (XMLHttpRequest)
 *           request.
 * @property boolean $isFlashRequest Whether this is an Adobe Flash or Adobe
 *           Flex request.
 ...
 * @property string $scriptFile Entry script file path (processed w/
 *           realpath()).
 ...
 * @property array $preferredAcceptType The user preferred accept type as an
 *           array map, e.g. array('type' => 'application', 'subType' =>
 *           'xhtml', 'baseType' => 'xml', 'params' => array('q' => 0.9)).
 * @property array $preferredAcceptTypes An array of all user accepted types (as
 *           array maps like array('type' => 'application', 'subType' =>
 *           'xhtml', 'baseType' => 'xml', 'params' => array('q' => 0.9)) ) in
 *           order of preference.
 * @property string $preferredLanguage The user preferred language.
 * @property array $preferredLanguages An array of all user accepted languages
...
class CHttpRequest extends CApplicationComponent {
	/**
	 *
	 * @var boolean whether cookies should be validated to ensure they are not
	 *      tampered. Defaults to false.
	 */
	public $enableCookieValidation = false;
	/**
	 *
	 * @var boolean whether to enable CSRF (Cross-Site Request Forgery)
	 *      validation. Defaults to false.
	 *      By setting this property to true, forms submitted to an Yii Web
	 *      application must be originated
	 *      from the same application. If not, a 400 HTTP exception will be
	 *      raised.
	 *      Note, this feature requires that the user client accepts cookie.
	 *      You also need to use {@link CHtml::form} or {@link
	 *      CHtml::statefulForm} to generate
	 ...
	 *      'YII_CSRF_TOKEN'.
	 *      This property is effectively only when {@link enableCsrfValidation}
	 *      is true.
	 */
	public $csrfTokenName = 'YII_CSRF_TOKEN';
	/**
	 *
	 * @var array the property values (in name-value pairs) used to initialize
	 *      the CSRF cookie.
	 *      Any property of {@link CHttpCookie} may be initialized.
	 *      This property is effective only when {@link enableCsrfValidation} is
	 *      true.
	 */
	public $csrfCookie;
	private $_requestUri;
	 ...
	 * This method strips off slashes in request data if get_magic_quotes_gpc()
	 * returns true.
	 ...
	 * If the GET or POST parameter does not exist, the second parameter to this
	 * method will be returned.
	 * If both GET and POST contains such a named parameter, the GET parameter
	 * takes precedence.
	 * 
	 * @param string $name
	 *        	the GET parameter name
	 * @param mixed $defaultValue
	 *        	the default parameter value if the GET parameter does not
	 *        	exist.
	 ...
	 * If the GET parameter does not exist, the second parameter to this method
	 * will be returned.
	 * 
	 * @param string $name
	 *        	the GET parameter name
	 ...
	 * If the POST parameter does not exist, the second parameter to this method
	 * will be returned.
	 * 
	 * @param string $name
	 *        	the POST parameter name
	 * @param mixed $defaultValue
	 *        	the default parameter value if the POST parameter does not
	 *        	exist.
	 ...
	 * If the DELETE parameter does not exist or if the current request is not a
	 * DELETE request,
	 * the second parameter to this method will be returned.
	 * If the DELETE request was tunneled through POST via _method parameter,
	 * the POST parameter
	 ...
	 * @param mixed $defaultValue
	 *        	the default parameter value if the DELETE parameter does not
	 *        	exist.
	 ...
	 * If the PUT parameter does not exist or if the current request is not a
	 * PUT request,
	 * the second parameter to this method will be returned.
	 * If the PUT request was tunneled through POST via _method parameter, the
	 * POST parameter
	 * will be returned instead (available since version 1.1.11).
	 * 
	 * @param string $name
	 ...
	 * @param mixed $defaultValue
	 *        	the default parameter value if the PUT parameter does not
	 *        	exist.
	 ...
	 * You may explicitly specify it by setting the {@link setHostInfo hostInfo}
	 * property.
	 * 
	 * @param string $schema
	 *        	schema to use (e.g. http, https). If empty, the schema used
	 *        	for the current request will be used.
	 * @return string schema and hostname part (with port number if needed) of
	 *         the request URL (e.g. http://www.yiiframework.com)
	 * @see setHostInfo
	 */
	 ...
	 * This setter is provided in case the schema and hostname cannot be
	 * determined
	 ...
	 * it does not have the script file name, and the ending slashes are
	 * stripped off.
	 * 
	 * @param boolean $absolute
	 *        	whether to return an absolute URL. Defaults to false, meaning
	 *        	returning a relative one.
	 ...
	 * The implementation of this method referenced Zend_Controller_Request_Http
	 * in Zend Framework.
	 * 
	 ...
	 * This refers to the part that is after the entry script and before the
	 * question mark.
	 ...
	 *         Note, the returned pathinfo is decoded starting from 1.1.4.
	 *         Prior to 1.1.4, whether it is decoded or not depends on the
	 *         server configuration
	 *         (in most cases it is not decoded).
	 * @throws CException if the request URI cannot be determined due to
	 ...
	 * This method is an improved variant of the native urldecode() function and
	 * used in {@link getPathInfo getPathInfo()} to
	 * decode the path part of the request URI. You may override this method to
	 * change the way the path info is being decoded.
	 * 
	 ...
	 * This refers to the portion that is after the {@link hostInfo host info}
	 * part.
	 ...
	 * _method. Useful
	 * for RESTful request from older browsers which do not support PUT or
	 * DELETE
	 ...
	 *        	the user agent to be analyzed. Defaults to null, meaning using
	 *        	the
	 ...
	 * request is insecure.
	 * You may explicitly specify it by setting the {@link setPort port}
	 * property.
	 * 
	 ...
	 * request is secure.
	 * You may explicitly specify it by setting the {@link setSecurePort
	 * securePort} property.
	 * 
	 ...
	 * The result can be used like an associative array. Adding {@link
	 * CHttpCookie} objects
	 * to the collection will send the cookies to the client; and removing the
	 * objects
	 ...
	 *        	absolute (not starting with "/") it will be relative to
	 *        	current request URL.
	...
	/**
	 * Parses an HTTP Accept header, returning an array map with all parts of
	 * each entry.
	 * Each array entry consists of a map with the type, subType, baseType and
	 * params, an array map of key-value parameters,
	 * obligatorily including a `q` value (i.e. preference ranking) as a double.
	 * For example, an Accept header value of
	 * <code>'application/xhtml+xml;q=0.9;level=1'</code> would give an array
	 * entry of
	 * <pre>
	 * array(
	 * 'type' => 'application',
	 ...
	 *
	 * <b>Please note:</b>
	 * To avoid great complexity, there are no steps taken to ensure that quoted
	 * strings are treated properly.
	 * If the header text includes quoted strings containing space or the , or ;
	 * characters then the results may not be correct!
	 *
	 * See also {@link http://tools.ietf.org/html/rfc2616#section-14.1} for
	 * details on Accept header.
	 * 
	 * @param string $header
		...
		if (count ( $matches ) === 6) {
			...
			while ( $i < $itemLen ) {
				...
				if ($matches [3] [$i] !== null && $matches [3] [$i] !== '')
					...
					// continue looping while there is no new content type, to
				// fill in all accompanying params
				for($i ++; $i < $itemLen; $i ++) {
	...
	/**
	 * Compare function for determining the preference of accepted MIME type
	 * array maps
	 ...
	 * @param array $b
	 *        	user accepted MIME type as an array map
	 * @return integer -1, 0 or 1 if $a has respectively greater preference,
	 *         equal preference or less preference than $b (higher preference
	 *         comes first).
	 */
	 ...
	 * 
	 * @return array the user accepted MIME types, as array maps, in the order
	 *         of preference.
	 */
	 ...
	 * The MIME type is returned as an array map (see {@link
	 * parseAcceptHeader()}).
	 * 
	 * @return array the user preferred accept MIME type or false if the user
	 *         does not have any.
	 */
	 ...
	 * The returned language IDs will NOT be canonicalized using {@link
	 ...
	 * The returned language ID will be canonicalized using {@link
	 * CLocale::getCanonicalID}.
	 * 
	 * @return string the user preferred language or false if the user does not
	 *         have any.
	 */
	 ...
	 *        	mime type of the content. If null, it will be guessed
	...
	public function sendFile($fileName, $content, $mimeType = null, $terminate = true) {
		...
		if (isset ( $_SERVER ['HTTP_RANGE'] )) {
			...
			/*
			 * Check the range and make sure it's treated according to the
			 * specs. http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html
			 */
			// End bytes can not be larger than $end.
			$contentEnd = ($contentEnd > $fileSize) ? $fileSize - 1 : $contentEnd;
			
			// Validate the requested range and return an error if it's not
			// correct.
	 ...
	 * Sends existing file to a browser as a download using x-sendfile.
	 *
	 * X-Sendfile is a feature allowing a web application to redirect the
	 * request for a file to the webserver
	 * that in turn processes the request, this way eliminating the need to
	 * perform tasks like reading the file
	 * and sending it to the user. When dealing with a lot of files (or very big
	 * files) this can lead to a great
	 * increase in performance as the web application is allowed to terminate
	 * earlier while the webserver is
	 * handling the request.
	 *
	 * The request is sent to the server through a special non-standard
	 * HTTP-header.
	 * When the web server encounters the presence of such header it will
	 * discard all output and send the file
	 * specified by that header using web server internals including all
	 * optimizations like caching-headers.
	 *
	 * As this header directive is non-standard different directives exists for
	 * different web servers applications:
	 * <ul>
	 * <li>Apache: {@link http://tn123.org/mod_xsendfile X-Sendfile}</li>
	 * <li>Lighttpd v1.4: {@link
	 * http://redmine.lighttpd.net/projects/lighttpd/wiki/X-LIGHTTPD-send-file
	 * X-LIGHTTPD-send-file}</li>
	 * <li>Lighttpd v1.5: {@link
	 * http://redmine.lighttpd.net/projects/lighttpd/wiki/X-LIGHTTPD-send-file
	 * X-Sendfile}</li>
	 * <li>Nginx: {@link http://wiki.nginx.org/XSendfile X-Accel-Redirect}</li>
	 * <li>Cherokee: {@link
	 * http://www.cherokee-project.com/doc/other_goodies.html#x-sendfile
	 * X-Sendfile and X-Accel-Redirect}</li>
	 * </ul>
	 * So for this method to work the X-SENDFILE option/module should be enabled
	 * by the web server and
	 * a proper xHeader should be sent.
	 *
	 * <b>Note:</b>
	 * This option allows to download files that are not under web folders, and
	 * even files that are otherwise protected (deny from all) like .htaccess
	 *
	 * <b>Side effects</b>:
	 * If this option is disabled by the web server, when this method is called
	 * a download configuration dialog
	 * will open but the downloaded file will have 0 bytes.
	 *
	 * <b>Known issues</b>:
	 * There is a Bug with Internet Explorer 6, 7 and 8 when X-SENDFILE is used
	 * over an SSL connection, it will show
	 * an error message like this: "Internet Explorer was not able to open this
	 * Internet site. The requested site is either unavailable or cannot be
	 * found.".
	 * You can work around this problem by removing the
	 * <code>Pragma</code>-header.
	 *
	 * <b>Example</b>:
	 * <pre>
	 * <?php
	 ...
	 *        	additional options:
	 *        	<ul>
	 *        	<li>saveName: file name shown to the user, if not set real
	 *        	file name will be used</li>
	 *        	<li>mimeType: mime type of the file, if not set it will be
	 *        	guessed automatically based on the file name, if set to null
	 *        	no content-type header will be sent.</li>
	 *        	<li>xHeader: appropriate x-sendfile header, defaults to
	 *        	"X-Sendfile"</li>
	 *        	<li>terminate: whether to terminate the current application
	 *        	after calling this method, defaults to true</li>
	 *        	<li>forceDownload: specifies whether the file will be
	 *        	downloaded or shown inline, defaults to true. (Since version
	 *        	1.1.9.)</li>
	 *        	<li>addHeaders: an array of additional http headers in
	 *        	header-value pairs (available since version 1.1.10)</li>
	 *        	</ul>
	 */
	 ...
	 * This is the event handler responding to {@link
	 * CApplication::onBeginRequest}.
	 * The default implementation will compare the CSRF token obtained
	 * from a cookie and from a POST field. If they are different, a CSRF attack
	 * is detected.
	 * 
	 * @param CEvent $event
	 *        	event parameter
...
class CCookieCollection extends CMap {
	 ...
	 *        	Cookie object.
	 * @throws CException if the item to be inserted is not a CHttpCookie
	 *         object.
	 */
	 ...
	 * Since version 1.1.11, the second parameter is available that can be used
	 * to specify
	 * the options of the CHttpCookie being removed. For example, this may be
	 * useful when dealing
	 * with ".domain.tld" where multiple subdomains are expected to be able to
	 * manage cookies:
	 *
	 * <pre>
	 ...
	 *        	Cookie name.
	 * @param array $options
	 *        	Cookie configuration array consisting of name-value pairs,
	 *        	available since 1.1.11.
