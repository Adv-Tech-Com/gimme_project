 ...
 * <li>An {@link IUserIdentity identity instance} is created with the
 * user-provided information.</li>
 * <li>Call {@link IUserIdentity::authenticate} to check if the identity is
 * valid.</li>
 ...
 * @property mixed $id The unique identifier for the user. If null, it means the
 *           user is a guest.
 * @property string $name The user name. If the user is not logged in, this will
 *           be {@link guestName}.
 * @property string $returnUrl The URL that the user should be redirected to
 *           after login.
 * @property string $stateKeyPrefix A prefix for the name of the session
 *           variables storing user session data.
...
class CWebUser extends CApplicationComponent implements IWebUser {
	 ...
	 *      This is used by {@link getName} when the current user is a guest
	 *      (not authenticated).
	 */
	public $guestName = 'Guest';
	/**
	 *
	 * @var string array URL for login. If using array, the first element should
	 *      be
	 *      the route to the login action, and the rest name-value pairs are GET
	 *      parameters
	 *      to construct the login URL (e.g. array('/site/login')). If this
	 *      property is null,
	...
	/**
	 *
	 * @var integer timeout in seconds after which user is logged out if
	 *      inactive.
	 *      If this property is not set, the user will be logged out after the
	...
	/**
	 *
	 * @var integer timeout in seconds after which user is logged out regardless
	 *      of activity.
	 * @since 1.1.14
	 */
	...
	/**
	 *
	 * @var boolean whether to automatically renew the identity cookie each time
	 *      a page is requested.
	 *      Defaults to false. This property is effective only when {@link
	 *      allowAutoLogin} is true.
	 *      When this is false, the identity cookie will expire after the
	 *      specified duration since the user
	 *      is initially logged in. When this is true, the identity cookie will
	...
	/**
	 *
	 * @var boolean whether to automatically update the validity of flash
	 *      messages.
	 *      Defaults to true, meaning flash messages will be valid only in the
	 *      current and the next requests.
	 *      If this is set false, you will be responsible for ensuring a flash
	 *      message is deleted after usage.
	 *      (This can be achieved by calling {@link getFlash} with the 3rd
	...
	/**
	 *
	 * @var string value that will be echoed in case that user session has
	 *      expired during an ajax call.
	 *      When a request is made and user session has expired, {@link
	 *      loginRequired} redirects to {@link loginUrl} for login.
	 *      If that happens during an ajax call, the complete HTML login page is
	 *      returned as the result of that ajax call. That could be
	 *      a problem if the ajax call expects the result to be a json array or
	 *      a predefined string, as the login page is ignored in that case.
	 *      To solve this, set this property to the desired return value.
	 *     
	 *      If this property is set, this value will be returned as the result
	 *      of the ajax call in case that the user session has expired.
	 * @since 1.1.9
	 * @see loginRequired
	 */
	...
	/**
	 * PHP magic method.
	 * This method is overriden so that persistent states can be accessed like
	 * properties.
	 * 
	 * @param string $name
	 *        	property name
	...
	/**
	 * PHP magic method.
	 * This method is overriden so that persistent states can be set like
	 * properties.
	 * 
	 * @param string $name
	 *        	property name
	...
	/**
	 * PHP magic method.
	 * This method is overriden so that persistent states can also be checked
	 * for null value.
	 * 
	 * @param string $name
	 *        	property name
	 ...
	 * performing cookie-based authentication if enabled, and updating the flash
	 * variables.
	 */
	 ...
	 * if you want to allow user to be authenticated based on the cookie
	 * information.
	 *
	 * @param IUserIdentity $identity
	 *        	the user identity (which should already be authenticated)
	 * @param integer $duration
	 *        	number of seconds that the user can remain in logged-in
	 *        	status. Defaults to 0, meaning login till the user closes the
	 *        	browser.
	 *        	If greater than 0, cookie-based login will be used. In this
	 *        	case, {@link allowAutoLogin}
	 ...
	 *        	whether to destroy the whole session. Defaults to true. If
	 *        	false,
	 *        	then {@link clearStates} will be called, which removes only
	 *        	the data stored via {@link setState}.
	 */
	 ...
	 *
	 * @param mixed $value
	 *        	the unique identifier for the user. If null, it means the user
	 *        	is a guest.
	 */
	 ...
	 * 
	 * @return string the user name. If the user is not logged in, this will be
	 *         {@link guestName}.
	 */
	...
	/**
	 * Returns the URL that the user should be redirected to after successful
	 * login.
	 ...
	 * to the current page after successful login. Make sure you set {@link
	 * loginUrl}
	 * so that the user browser can be redirected to the specified login URL
	 * after
	 * calling this method.
	 * After calling this method, the current request processing will be
	 * terminated.
	 */
	 ...
	 * @param array $states
	 *        	a set of name-value pairs that are provided by the user
	 *        	identity.
	...
	/**
	 * Populates the current user object with the information obtained from
	 * cookie.
	 * This method is used when automatic login ({@link allowAutoLogin}) is
	 * enabled.
	 * The user identity information is recovered from cookie.
	 * Sufficient security measures are used to prevent cookie data from being
	 * tampered.
	 * 
	 * @see saveToCookie
	 */
	 ...
	 * This method is used when automatic login ({@link allowAutoLogin}) is
	 * enabled.
	 * This method saves user ID, username, other identity states and a
	 * validation key to cookie.
	 * These information are used to do authentication next time when user
	...
	/**
	 *
	 * @return string a prefix for the name of the session variables storing
	 *         user session data.
	 */
	 ...
	 *
	 * @param string $value
	 *        	a prefix for the name of the session variables storing user
	 *        	session data.
	 */
	 ...
	 *        	default value
	 * @return mixed the value of the variable. If it doesn't exist in the
	 *         session,
	 ...
	 *        	whether to delete the flash messages after calling this
	 *        	method.
	 ...
	 *        	flash message
	 * @param mixed $defaultValue
	 *        	if this value is the same as the flash message, the flash
	 *        	message
	 *        	will be removed. (Therefore, you can use setFlash('key',null)
	 ...
	 * If the user has been inactive for {@link authTimeout} seconds, or {link
	 * absoluteAuthTimeout} has passed,
	 ...
	 *        	this array, which holds the value of
	 *        	{@link getId()} when {@link CDbAuthManager} or {@link
	 *        	CPhpAuthManager} is used.
	 ...
	 *        	is effective
	 *        	only within the same request and only works when
