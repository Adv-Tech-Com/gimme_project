 ...
 * @property string $savePath The current session save path, defaults to {@link
 *           http://php.net/session.save_path}.
 * @property array $cookieParams The session cookie parameters.
 * @property string $cookieMode How to use cookie to store session ID. Defaults
 *           to 'Allow'.
 * @property float $gCProbability The probability (percentage) that the gc
 *           (garbage collection) process is started on every session
 *           initialization, defaults to 1 meaning 1% chance.
 * @property boolean $useTransparentSessionID Whether transparent sid support is
 *           enabled or not, defaults to false.
 * @property integer $timeout The number of seconds after which data will be
 *           seen as 'garbage' and cleaned up, defaults to 1440 seconds.
 * @property CHttpSessionIterator $iterator An iterator for traversing the
 *           session variables.
...
class CHttpSession extends CApplicationComponent implements IteratorAggregate, ArrayAccess, Countable {
	/**
	 *
	 * @var boolean whether the session should be automatically started when the
	 ...
	 * If returning true, make sure the methods {@link openSession}, {@link
	 * closeSession}, {@link readSession},
	 * {@link writeSession}, {@link destroySession}, and {@link gcSession} are
	 * overridden in child
	 ...
	 *
	 * Please refer to {@link http://php.net/session_regenerate_id} for more
	 * details.
	 * 
	 ...
	 *
	 * @param string $value
	 *        	the session name for the current session, must be an
	...
	/**
	 *
	 * @return string the current session save path, defaults to {@link
	 *         http://php.net/session.save_path}.
	 */
	...
	/**
	 *
	 * @return string how to use cookie to store session ID. Defaults to
	 *         'Allow'.
	 */
	 ...
	 *
	 * @param string $value
	 *        	how to use cookie to store session ID. Valid values include
	 *        	'none', 'allow' and 'only'.
	 */
	...
	/**
	 *
	 * @return float the probability (percentage) that the gc (garbage
	 ...
	 *
	 * @param float $value
	 *        	the probability (percentage) that the gc (garbage collection)
	...
	/**
	 *
	 * @return boolean whether transparent sid support is enabled or not,
	 *         defaults to false.
	 */
	...
	/**
	 *
	 * @return integer the number of seconds after which data will be seen as
	 ...
	 * 
	 * @return CHttpSessionIterator an iterator for traversing the session
	 *         variables.
	 */
	 ...
	 * except that it will return $defaultValue if the session variable does not
	 * exist.
	 * 
	 * @param mixed $key
	 *        	the session variable name
	 * @param mixed $defaultValue
	 *        	the default value to be returned when the session variable
	 *        	does not exist.
	 * @return mixed the session variable value, or $defaultValue if the session
	 ...
	 * Note, if the specified name already exists, the old value will be removed
	 * first.
	 * 
	 * @param mixed $key
