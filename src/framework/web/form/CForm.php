 ...
 * that specify each individual form inputs, and those that decorate the form
 * inputs.
 * A CForm object represents the former part. It relies on the rendering process
 * to
 * accomplish form input decoration. Reusability is mainly achieved in the
 * rendering process.
 * That is, a rendering process can be reused to render different CForm objects.
 *
 * A form can be rendered in different ways. One can call the {@link render}
 * method
 * to get a quick form rendering without writing any HTML code; one can also
 * override
 * {@link render} to render the form in a different layout; and one can use an
 * external
 * view template to render each form element explicitly. In these ways, the
 * {@link render}
 * method can be applied to all kinds of forms and thus achieves maximum
 * reusability;
 * while the external view template keeps maximum flexibility in rendering
 * complex forms.
 *
 ...
 * button elements ({@link CFormButtonElement}). When a CForm object is embedded
 * in the
 * {@link elements} collection, it is called a sub-form which can have its own
 * {@link elements}
 ...
 * also
 * a good way to partition a lengthy form into shorter ones, even though all
 * inputs
 * may belong to the same model.
 *
 * Form input specifications are given in terms of a configuration array which
 * is
 * used to initialize the property values of a CForm object. The {@link
 * elements} and
 * {@link buttons} properties need special attention as they are the main
 * properties
 * to be configured. To configure {@link elements}, we should give it an array
 * like
 * the following:
 * <pre>
 * 'elements'=>array(
 ...
 * the model
 * object must have exactly the same attributes 'username' and 'password'. Each
 * element
 * has a type which specifies what kind of input should be used. The rest of the
 * array elements
 * (e.g. 'maxlength') in an input specification are rendered as HTML element
 * attributes
 * when the input field is rendered. The {@link buttons} property is configured
 * similarly.
 *
 * If you're going to use AJAX and/or client form validation with the enabled
 * error summary
 * you have to set {@link $showErrors} property to true. Please refer to it's
 * documentation
 * for more details.
 *
 * For more details about configuring form elements, please refer to {@link
 * CFormInputElement}
 * and {@link CFormButtonElement}.
 *
 * @property CForm $root The top-level form object.
 * @property CActiveForm $activeFormWidget The active form widget associated
 *           with this form.
 *           This method will return the active form widget as specified by
 *           {@link activeForm}.
 * @property CBaseController $owner The owner of this form. This refers to
 *           either a controller or a widget
 *           by which the form is created and rendered.
 * @property CModel $model The model associated with this form. If this form
 *           does not have a model,
 *           it will look for a model in its ancestors.
 * @property array $models The models that are associated with this form or its
 *           sub-forms.
...
class CForm extends CFormElement implements ArrayAccess {
	/**
	 *
	 * @var string the title for this form. By default, if this is set, a
	 *      fieldset may be rendered
	 *      around the form body using the title as its legend. Defaults to
	 *      null.
	 */
	public $title;
	/**
	...
	/**
	 *
	 * @var array HTML attribute values for the form tag. When the form is
	 *      embedded within another form,
	 *      this property will be used to render the HTML attribute values for
	 *      the fieldset enclosing the child form.
	 */
	...
	/**
	 *
	 * @var boolean null error elements of the form attributes should be
	 *      rendered. There are three possible
	 *      valid values: null, true and false.
	 *     
	 *      Defaults to null meaning that {@link $showErrorSummary} will be used
	 *      as value. This is done mainly to keep
	 *      backward compatibility with existing applications. If you want to
	 *      use error summary with AJAX and/or client
	 *      validation you have to set this property to true (recall that {@link
	 *      CActiveForm::error()} should be called
	 *      for each attribute that is going to be AJAX and/or client
	 *      validated).
	 *     
	 *      False value means that the error elements of the form attributes
	 *      shall not be displayed. True value means that
	 ...
	 *      The widget will be used to render the form tag and the error
	 *      messages.
	 *      The 'class' option is required, which specifies the class of the
	 *      widget.
	 *      The rest of the options will be passed to {@link
	 *      CBaseController::beginWidget()} call.
	 ...
	 * 
	 * @param mixed $config
	 *        	the configuration for this form. It can be a configuration
	 *        	array
	 *        	or the path alias of a PHP script file that returns a
	 *        	configuration array.
	 *        	The configuration array consists of name-value pairs that are
	 *        	used to initialize
	 ...
	 * @param mixed $parent
	 *        	the direct parent of this form. This could be either a {@link
	 *        	CBaseController}
	 *        	object (a controller or a widget), or a {@link CForm} object.
	 *        	If the former, it means the form is a top-level form; if the
	 *        	latter, it means this form is a sub-form.
	 */
	 ...
	 * You may override this method to provide customized initialization (such
	 * as
	 ...
	 *        	whether to call {@link loadData} if the form is submitted so
	 *        	that
	 ...
	 * the validation. You may use {@link CModel::getErrors()} to retrieve the
	 * validation
	 * error messages.
	 * 
	 ...
	 * This method will go through all models associated with this form and its
	 * sub-forms
	...
	/**
	 *
	 * @return CBaseController the owner of this form. This refers to either a
	 ...
	 *        	whether to return parent's model if this form doesn't have
	 *        	model by itself.
	 * @return CModel the model associated with this form. If this form does not
	 *         have a model,
	 *         it will look for a model in its ancestors.
	 */
	 ...
	 * 
	 * @return array the models that are associated with this form or its
	 *         sub-forms.
	 */
	 ...
	 * Note that the returned result is a {@link CFormElementCollection} object,
	 * which
	 ...
	 * The configuration must be an array of input configuration array indexed
	 * by input name.
	 * Each input configuration array consists of name-value pairs that are used
	 * to initialize
	 * a {@link CFormStringElement} object (when 'type' is 'string'), a {@link
	 * CFormElement} object
	 * (when 'type' is a string ending with 'Form'), or a {@link
	 * CFormInputElement} object in
	 * all other cases.
	 * 
	 ...
	 * The configuration must be an array of button configuration array indexed
	 * by button name.
	 * Each button configuration array consists of name-value pairs that are
	 * used to initialize
	...
	/**
	 * Renders the form.
	 * The default implementation simply calls {@link renderBegin}, {@link
	 * renderBody} and {@link renderEnd}.
	 * 
	 ...
	 * If {@link title} or {@link description} is specified, they will be
	 * rendered as well.
	 * And if the associated model contains error, the error summary may also be
	 * displayed.
	 * The form tag will not be rendered. Please call {@link renderBegin} and
	 * {@link renderEnd}
	 ...
	 * 
	 * @param mixed $element
	 *        	the form element to be rendered. This can be either a {@link
	 *        	CFormElement} instance
	...
	/**
	 * This method is called after an element is added to the element
	 * collection.
	 * 
	 * @param string $name
	 ...
	 *        	whether the element is added to the {@link buttons}
	 *        	collection.
	 *        	If false, it means the element is added to the {@link
	 *        	elements} collection.
	 */
	 ...
	 * @param boolean $forButtons
	 *        	whether the element is removed from the {@link buttons}
	 *        	collection
	 *        	If false, it means the element is removed from the {@link
	 *        	elements} collection.
	 */
	 ...
	 * If any one of them is visible, the form is considered as visible.
	 ...
	 * @return mixed the element at the offset, null if no element is found at
	 *         the offset
	 */
