 ...
 * 2. Class-based action: create an instance of class 'XYZ' if the class is
 * found in the action class map
 * (specified via {@link actions()}, and execute the action;
 * 3. Call {@link missingAction()}, which by default will raise a 404 HTTP
 * exception.
 *
 ...
 * actions.
 * Filters preprocess/postprocess the user request/response and may quit
 * executing actions
 * if needed. They are executed in the order they are specified. If during the
 * execution,
 * any of the filters returns true, the rest filters and the action will no
 * longer get executed.
 *
 * Filters can be individual objects, or methods defined in the controller
 * class.
 * They are specified by overriding {@link filters()} method. The following is
 * an example
 ...
 * COutputCache. The first two
 * are method-based filters (defined in CController), which refer to filtering
 * methods in the controller class;
 * while the last refers to an object-based filter whose class is
 * 'system.web.widgets.COutputCache' and
 * the 'duration' property is initialized as 300 (s).
 *
 * For method-based filters, a method named 'filterXYZ($filterChain)' in the
 * controller class
 * will be executed, where 'XYZ' stands for the filter name as specified in
 * {@link filters()}.
 * Note, inside the filter method, you must call
 * <code>$filterChain->run()</code> if the action should
 ...
 * actions.
 * For method-based filters, this is done by using '+' and '-' operators in the
 * filter specification.
 * The '+' operator means the filter runs only when the specified actions are
 * requested;
 * while the '-' operator means the filter runs only when the requested action
 * is not among those actions.
 * For object-based filters, the '+' and '-' operators are following the class
 * name.
 *
 * @property array $actionParams The request parameters to be used for action
 *           parameter binding.
 * @property CAction $action The action currently being executed, null if no
 *           active action.
 * @property string $id ID of the controller.
 * @property string $uniqueId The controller ID that is prefixed with the module
 *           ID (if any).
 * @property string $route The route (module ID, controller ID and action ID) of
 *           the current request.
 * @property CWebModule $module The module that this controller belongs to. It
 *           returns null
 *           if the controller does not belong to any module.
 * @property string $viewPath The directory containing the view files for this
 *           controller. Defaults to 'protected/views/ControllerID'.
 * @property CMap $clips The list of clips.
 * @property string $pageTitle The page title. Defaults to the controller name
 *           and the action name.
...
class CController extends CBaseController {
	...
	/**
	 *
	 * @var mixed the name of the layout to be applied to this controller's
	 *      views.
	 *      Defaults to null, meaning the {@link CWebApplication::layout
	 *      application layout}
	 *      is used. If it is false, no layout will be applied.
	 *      The {@link CWebModule::layout module layout} will be used
	 *      if the controller belongs to a module and this layout property is
	 *      null.
	 */
	public $layout;
	/**
	 ...
	 * This method is called by the application before the controller starts to
	 * execute.
	 * You may override this method to perform the needed initialization for the
	 * controller.
	 */
	 ...
	 *
	 * By overriding this method, child classes can specify filters to be
	 * applied to actions.
	 *
	 ...
	 * where the '+' ('-') operators describe which actions should be (should
	 * not be) applied with the filter.
	 *
	 ...
	 * )
	 * </pre>
	 * where the name-value pairs will be used to initialize the properties of
	 * the filter.
	 *
	 * Note, in order to inherit filters defined in the parent class, a child
	 * class needs to
	 * merge the parent filters with child filters using functions like
	 * array_merge().
	 *
	 ...
	 * action class in dot syntax (e.g.
	 * 'edit'=>'application.controllers.article.EditArticle')
	 * or arrays representing the configuration of the actions, such as the
	 * following,
	 * <pre>
	 * return array(
	 ...
	 *
	 * Note, in order to inherit actions defined in the parent class, a child
	 * class needs to
	 * merge the parent actions with child actions using functions like
	 * array_merge().
	 *
	 ...
	 * behavior names. Each behavior configuration can be either a string
	 * specifying
	 ...
	 * </pre>
	 *
	 * Note, the behavior classes must implement {@link IBehavior} or extend
	 * from
	 ...
	 * 
	 * @return array the behavior configurations (behavior name=>behavior
	 *         configuration)
	 */
	 ...
	 * Override this method if you use the {@link filterAccessControl
	 * accessControl} filter.
	 * 
	 * @return array list of access rules. See {@link CAccessControlFilter} for
	 ...
	 *        	action ID
	 * @throws CHttpException if the action does not exist or the action name is
	 *         not proper.
	 * @see filters
	 * @see createAction
	 * @see runAction
	 ...
	 * This method is invoked by {@link runActionWithFilters} after all possible
	 ...
	 * binding.
	 * By default, this method will return $_GET. You may override this method
	 * if you
	 * want to use other request parameters (e.g. $_GET+$_POST).
	 * 
	 * @return array the request parameters to be used for action parameter
	 *         binding
	 * @since 1.1.7
	 */
	 ...
	 * requirement of the specified action.
	 ...
	 * This method is invoked at the end of {@link render()} and {@link
	 * renderText()}.
	 * If there are registered client scripts, this method will insert them into
	 * the output
	 * at appropriate places. If there are dynamic contents, they will also be
	 * inserted.
	 ...
	 * The latter is created by looking up the action map specified in {@link
	 * actions}.
	 * 
	 * @param string $actionID
	 *        	ID of the action. If empty, the {@link defaultAction default
	 *        	action} will be used.
	...
	public function createAction($actionID) {
		...
		if (method_exists ( $this, 'action' . $actionID ) && strcasecmp ( $actionID, 's' )) // we
		                                                                         // have
		                                                                         // actions
		                                                                         // method
	 ...
	 * @param array $config
	 *        	the action configuration that should be applied on top of the
	 ...
	 * This method is invoked when the controller cannot find the requested
	 * action.
	...
	/**
	 *
	 * @return CAction the action currently being executed, null if no active
	 *         action.
	 */
	...
	/**
	 *
	 * @return string the controller ID that is prefixed with the module ID (if
	 *         any).
	 */
	...
	/**
	 *
	 * @return string the route (module ID, controller ID and action ID) of the
	 *         current request.
	 * @since 1.1.0
	 */
	...
	/**
	 *
	 * @return CWebModule the module that this controller belongs to. It returns
	 *         null
	 *         if the controller does not belong to any module
	 */
	 ...
	 * is the {@link CWebModule::getViewPath module view path} appended with the
	 * controller ID.
	 * 
	 * @return string the directory containing the view files for this
	 *         controller. Defaults to 'protected/views/ControllerID'.
	 */
	 ...
	 *
	 * When a theme is currently active, this method will call {@link
	 * CTheme::getViewFile} to determine
	 * which view file should be returned.
	 *
	 * Otherwise, this method will return the corresponding view file based on
	 * the following criteria:
	 * <ul>
	 * <li>absolute view within a module: the view name starts with a single
	 * slash '/'.
	 * In this case, the view will be searched for under the currently active
	 * module's view path.
	 * If there is no active module, the view will be searched for under the
	 * application's view path.</li>
	 * <li>absolute view within the application: the view name starts with
	 * double slashes '//'.
	 * In this case, the view will be searched for under the application's view
	 * path.
	 ...
	 * The view file is determined by calling {@link YiiBase::getPathOfAlias()}.
	 * Note that aliased views
	 * cannot be themed because they can refer to a view file located at
	 * arbitrary places.</li>
	 * <li>relative view: otherwise. Relative views will be searched for under
	 * the currently active
	 * controller's view path.</li>
	 * </ul>
	 *
	 * After the view file is identified, this method may further call {@link
	 ...
	 * <li>layout is null: the currently active module's layout will be used. If
	 * there is no active module,
	 * the application's layout will be used.</li>
	 * <li>a regular view name.</li>
	 * </ul>
	 *
	 * The resolution of the view file based on the layout view is similar to
	 * that in {@link getViewFile}.
	 * In particular, the following rules are followed:
	 *
	 * Otherwise, this method will return the corresponding view file based on
	 * the following criteria:
	 * <ul>
	 * <li>When a theme is currently active, this method will call {@link
	 * CTheme::getLayoutFile} to determine
	 ...
	 * <li>relative view: otherwise. Relative views will be searched for under
	 * the currently active
	 * module's layout path. In case when there is no active module, the view
	 ...
	 *        	layout name
	 * @return string the view file for the layout. False if the view file
	 *         cannot be found
	 */
	 ...
	 * controller's view path.</li>
	 * </ul>
	 * For absolute view and relative view, the corresponding view file is a PHP
	 * file
	 * whose name is the same as the view name. The file is located under a
	 * specified directory.
	 * This method will call {@link CApplication::findLocalizedFile} to search
	 * for a localized file, if any.
	 * 
	 * @param string $viewName
	 *        	the view name
	 ...
	 *        	the directory that is used to search for an absolute view name
	 *        	under the application
	 * @param string $moduleViewPath
	 *        	the directory that is used to search for an absolute view name
	 *        	under the current module.
	 *        	If this is not set, the application base view path will be
	 *        	used.
	 ...
	 * This is like {@link redirect}, but the user browser's URL remains
	 * unchanged.
	 * In most cases, you should call {@link redirect} instead of this method.
	 * 
	 * @param string $route
	 *        	the route of the new controller action. This can be an action
	 *        	ID, or a complete route
	 *        	with module ID (optional in the current module), controller ID
	 *        	and action ID. If the former, the action is assumed
	 *        	to be located within the current controller.
	 * @param boolean $exit
	 *        	whether to end the application after this call. Defaults to
	 *        	true.
	 * @since 1.1.0
	 */
	 ...
	 *
	 * This method first calls {@link renderPartial} to render the view (called
	 * content view).
	 * It then renders the layout view which may embed the content view at
	 * appropriate place.
	 * In the layout view, the content view rendering result can be accessed via
	 * variable
	 * <code>$content</code>. At the end, it calls {@link processOutput} to
	 * insert scripts
	 ...
	 *
	 * @param string $view
	 *        	name of the view to be rendered. See {@link getViewFile} for
	 *        	details
	 *        	about how the view script is resolved.
	 * @param array $data
	 *        	data to be extracted into PHP variables and made available to
	 *        	the view script
	 * @param boolean $return
	 *        	whether the rendering result should be returned instead of
	 *        	being displayed to end users.
	 * @return string the rendering result. Null if the rendering result is not
	 *         required.
	 * @see renderPartial
	 * @see getLayoutFile
	 */
	 ...
	 * You may override this method to do some preprocessing when rendering a
	 * view.
	 * 
	 * @param string $view
	 ...
	 * You may override this method to do some postprocessing for the view
	 * rendering.
	 * 
	 * @param string $view
	 *        	the view that has been rendered
	 * @param string $output
	 *        	the rendering result of the view. Note that this parameter is
	 *        	passed
	 *        	as a reference. That means you can modify it within this
	 *        	method.
	 * @since 1.1.5
	 */
	 ...
	 * The string will be inserted in the current controller layout and returned
	 * back.
	 * 
	 * @param string $text
	 ...
	 * will always show as their latest state even if the content surrounding
	 * them is being cached.
	 * This is especially useful when caching pages that are mostly static but
	 * contain some small
	 * dynamic regions, such as username or current time.
	 * We can use this method to render these dynamic regions to ensure they are
	 * always up-to-date.
	 *
	 * The first parameter to this method should be a valid PHP callback, while
	 * the rest parameters
	 * will be passed to the callback.
	 *
	 * Note, the callback and its parameter values will be serialized and saved
	 * in cache.
	 * Make sure they are serializable.
	 *
	 * @param callback $callback
	 *        	a PHP callback which returns the needed dynamic content.
	 *        	When the callback is specified as a string, it will be first
	 *        	assumed to be a method of the current
	 *        	controller class. If the method does not exist, it is assumed
	 *        	to be a global PHP function.
	 *        	Note, the callback should return the dynamic content instead
	 *        	of echoing it.
	 */
	 ...
	 *        	If the ControllerID is not present, the current controller ID
	 *        	will be prefixed to the route.
	 *        	If the route is empty, it is assumed to be the current action.
	 *        	If the controller belongs to a module, the {@link
	 *        	CWebModule::getId module ID}
	 *        	will be prefixed to the route. (If you do not want the module
	 *        	ID prefix, the route should start with a slash '/'.)
	 * @param array $params
	 *        	additional GET parameters (name=>value). Both the name and
	 *        	value will be URL-encoded.
	 *        	If the name is '#', the corresponding value will be treated as
	 *        	an anchor
	 ...
	 *        	the URL route. This should be in the format of
	 *        	'ControllerID/ActionID'.
	 *        	If the ControllerPath is not present, the current controller
	 *        	ID will be prefixed to the route.
	 *        	If the route is empty, it is assumed to be the current action.
	 * @param array $params
	 *        	additional GET parameters (name=>value). Both the name and
	 *        	value will be URL-encoded.
	 * @param string $schema
	 *        	schema to use (e.g. http, https). If empty, the schema used
	 *        	for the current request will be used.
	 * @param string $ampersand
	 *        	the token separating name-value pairs in the URL.
	 * @return string the constructed URL
	 */
	...
	/**
	 *
	 * @return string the page title. Defaults to the controller name and the
	 *         action name.
	 */
	 ...
	 *        	the first element must be a route to a controller action and
	 *        	the rest
	 *        	are GET parameters in name-value pairs.
	 * @param boolean $terminate
	 *        	whether to terminate the current application after calling
	 *        	this method. Defaults to true.
	 * @param integer $statusCode
	 *        	the HTTP status code. Defaults to 302. See {@link
	 ...
	 *        	Defaults to empty. Make sure the anchor starts with '#' if you
	 *        	want to specify it.
	 */
	 ...
	 *        	whose method is being called. If empty it means the controller
	 *        	itself.
	 * @param string $method
	 *        	the method name
	 * @param array $params
	...
	public function recordCachingAction($context, $method, $params) {
		if ($this->_cachingStack) 		// record only when there is an active output
		                         // cache
		{
	 ...
	 *        	whether to create a stack if it does not exist yet. Defaults
	 *        	to true.
	 ...
	 * 
	 * @return boolean whether the caching stack is empty. If not empty, it
	 *         means currently there are
	 *         some output cache in effect. Note, the return result of this
	 *         method may change when it is
	 *         called in different output regions, depending on the partition of
	 *         output caches.
	 */
	 ...
	 * This filter throws an exception (CHttpException with code 400) if the
	 ...
	 * A page state is a variable that is persistent across POST requests of the
	 * same page.
	 ...
	 *        	the given value, the state will be removed from persistent
	 *        	storage.
	 * @see getPageState
	 * @see CHtml::statefulForm
	 */
