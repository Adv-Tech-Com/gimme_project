 ...
 * doc
 * comment block containing the '@soap' tag. In the doc comment, the type and
 * name
 * of every input parameter and the type of the return value should be declared
 * using
 ...
 * CWsdlGenerator will look for its property declarations. Only public
 * properties
 * are considered, and they each must be associated with a doc comment block
 * containg
 * the '@soap' tag. The doc comment block should declare the type of the
 * property.
 *
 ...
 * 'Member' is not
 * a primitive type, CWsdlGenerator will look further to find the definition of
 * 'Member'.
 *
 ...
 * </ul>
 *
 * Additionally, each complex data type can have assigned a soap indicator flag
 * declaring special usage for such a data type.
 * A soap indicator must be declared in the doc comment block with the
 * '@soap-indicator' tag.
 * Following soap indicators are currently supported:
 * <ul>
 * <li>all - (default) allows any sorting order of child nodes</li>
 * <li>sequence - all child nodes in WSDL XML file will be expected in
 * predefined order</li>
 * <li>choice - supplied can be either of the child elements</li>
 * </ul>
 * The Group indicators can be also injected via custom soap definitions as XML
 * node into WSDL structure.
 *
 ...
 * }
 * </pre>
 * For more on soap indicators, see See {@link
 ...
 * string into generated WSDL file.
 * If such a block of the code is found in class's comment block, then it will
 * be used instead of parsing and generating standard attributes within the
 * class.
 * This gives virtually unlimited flexibility in defining data structures of any
 * complexity.
 ...
 * file.
 * Each complex type is described in a separate HTML table and recognizes also
 * the '@example' PHPDoc tag. See {@link buildHtmlDocs()}.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class CWsdlGenerator extends CComponent {
	...
	/**
	 *
	 * @var array List of recognized SOAP operations that will become remotely
	 *      available.
	 *      All methods with declared @soap parameter will be included here in
	 ...
	 *      If an SOAP operation defines complex input or output type, all
	 *      objects are included here containing all sub-parameters.
	 *      For instance, if an SOAP operation "createUser" requires complex
	 *      input object "User", then the object "User" will be included here
	 *      with declared subparameters such as "firstname", "lastname", etc..
	 */
	protected $types;
	...
	protected function processType($type) {
		...
		elseif (($pos = strpos ( $type, '[]' )) !== false) { // array of types
			...
			// extract soap indicator flag, if defined, e.g. @soap-indicator
			// sequence
				...
				if ($property->isPublic () && strpos ( $comment, '@soap' ) !== false) {
					if (preg_match ( '/@var\s+([\w\.]+(\[\s*\])?)\s*?(.*)$/mi', $comment, $matches )) {
						...
						$this->types [$type] ['properties'] [$property->getName ()] = array (
								...
								$example 
						); // name => type, doc, nillable, minOccurs, maxOccurs,
						   // example
					}
				}
			}
	...
	/**
	 * Import custom XML source node into WSDL document under specified target
	 * node
	 * 
...
<definitions name=\"{$this->serviceName}\" targetNamespace=\"{$this->namespace}\"
	 ...
	 *
	 * @param DOMDocument $dom
	 *        	Represents an entire HTML or XML document; serves as the root
	...
	protected function addTypes($dom) {
		...
		foreach ( $this->types as $phpType => $xmlType ) {
			...
			if (is_string ( $xmlType )) {
				...
				$arrayType = ($dppos = strpos ( $xmlType, ':' )) !== false ? substr ( $xmlType, $dppos + 1 ) : $xmlType; // strip
				                                                                                            // namespace,
				                                                                                            // if
				                                                                                            // any
	 ...
	 * This method can be invoked either by inserting URL parameter "&makedoc"
	 * into URL link, e.g. "http://www.mydomain.com/soap/create?makedoc", or
	 * simply by calling from another script with argument $return=true.
	 *
	 ...
	 * <li>Type - attribute type, e.g. integer, date,
	 * tns:SoapPovCalculationResultArray</li>
	 ...
	 * <li>Example - Attribute example value if provided via PHPDoc property
	 * @example.</li>
	 * <ul>
	 *
	 * @param bool $return
	 *        	If true, generated HTML output will be returned rather than
	 *        	directly sent to output buffer
	 */
