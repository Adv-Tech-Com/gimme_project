...
class CClientScript extends CApplicationComponent {
	/**
	 * The script is rendered in the head section right before the title
	 * element.
	 */
	const POS_HEAD = 0;
	/**
	...
	/**
	 *
	 * @var array the mapping between script file names and the corresponding
	 *      script URLs.
	 *      The array keys are script file names (without directory part) and
	 *      the array values are the corresponding URLs.
	 *      If an array value is false, the corresponding script file will not
	 *      be rendered.
	 *      If an array key is '*.js' or '*.css', the corresponding URL will
	 *      replace all
	 *      JavaScript files or CSS files, respectively.
	 *     
	 *      This property is mainly used to optimize the generated HTML pages
	 *      by merging different scripts files into fewer and optimized script
	 *      files.
	 */
	 ...
	 *      This property keeps a list of named script packages, each of which
	 *      can contain
	 *      a set of CSS and/or JavaScript script files, and their dependent
	 *      package names.
	 *      By calling {@link registerPackage}, one can register a whole package
	 *      of client
	 *      scripts together with their dependent packages and render them in
	 *      the HTML output.
	 *     
	 ...
	 *      For example, if 'basePath' is 'application.assets', a script named
	 *      'comments.js'
	 *      will refer to the file 'protected/assets/comments.js'.
	 *     
	 *      When a script is being rendered in HTML, it will be prefixed with
	 *      'baseUrl'.
	 *      For example, if 'baseUrl' is '/assets', the 'comments.js' script
	 *      will be rendered
	 *      using URL '/assets/comments.js'.
	 *     
	 *      If 'baseUrl' does not start with '/', the relative URL of the
	 *      application entry
	 *      script will be inserted at the beginning. For example, if 'baseUrl'
	 *      is 'assets'
	 *      and the current application runs with the URL
	 *      'http://localhost/demo/index.php',
	 *      then the 'comments.js' script will be rendered using URL
	 *      '/demo/assets/comments.js'.
	 *     
	 *      If 'baseUrl' is not set, the script will be published by {@link
	 *      CAssetManager}
	 *      and the corresponding published URL will be used.
	 *     
	 *      When calling {@link registerPackage} to register a script package,
	 *      this property will be checked first followed by {@link
	 *      corePackages}.
	 ...
	 *      You may configure this property to customize the core script
	 *      packages.
	 *     
	 *      When calling {@link registerPackage} to register a script package,
	...
	/**
	 *
	 * @var array the registered head meta tags. Each array element represents
	 *      an option array
	...
	/**
	 *
	 * @var array the registered head link tags. Each array element represents
	 *      an option array
	...
	/**
	 *
	 * @var array the registered css code blocks (key => array(CSS code, media
	 *      type)).
	 * @since 1.1.3
	 */
	protected $css = array ();
	...
	/**
	 *
	 * @var integer Where the scripts registered using {@link
	 *      registerCoreScript} or {@link registerPackage}
	 *      will be inserted in the page. This can be one of the
	 *      CClientScript::POS_* constants.
	 ...
	 *
	 * @var integer Where the scripts registered using {@link
	 *      registerScriptFile} will be inserted in the page.
	...
	/**
	 *
	 * @var integer Where the scripts registered using {@link registerScript}
	 ...
	 * If the base URL is not explicitly set, this method will publish the whole
	 * directory
	 ...
	 * This setter is provided in case when core javascript files are manually
	 * published
	 * to a pre-specified location. This may save asset publishing time for
	 ...
	 * If needed, this method may publish the assets of the package and returns
	 * the published base URL.
	 * 
	 * @param string $name
	 *        	the package name
	 * @return string the base URL for the named package. False is returned if
	 ...
	 * @return CClientScript the CClientScript object itself (to support method
	 *         chaining, available since version 1.1.5).
	 * @since 1.1.7
	 ...
	 *        	URL of the CSS file
	 * @param string $media
	 *        	media that the CSS file should be applied to. If empty, it
	 *        	means all media types.
	 * @return CClientScript the CClientScript object itself (to support method
	 *         chaining, available since version 1.1.5).
	 */
	 ...
	 *        	the CSS code
	 * @param string $media
	 *        	media that the CSS code should be applied to. If empty, it
	 *        	means all media types.
	 * @return CClientScript the CClientScript object itself (to support method
	 *         chaining, available since version 1.1.5).
	 */
	 ...
	 *        	the position of the JavaScript code. Valid values include the
	 *        	following:
	 *        	<ul>
	 *        	<li>CClientScript::POS_HEAD : the script is inserted in the
	 *        	head section right before the title element.</li>
	 *        	<li>CClientScript::POS_BEGIN : the script is inserted at the
	 *        	beginning of the body section.</li>
	 *        	<li>CClientScript::POS_END : the script is inserted at the end
	 *        	of the body section.</li>
	 *        	</ul>
	 ...
	 *        	<li>CClientScript::POS_END : the script is inserted at the end
	 *        	of the body section.</li>
	 *        	<li>CClientScript::POS_LOAD : the script is inserted in the
	 *        	window.onload() function.</li>
	 *        	<li>CClientScript::POS_READY : the script is inserted in the
	 *        	jQuery's ready function.</li>
	 *        	</ul>
	 * @param array $htmlOptions
	 *        	additional HTML attributes
	 *        	Note: HTML attributes are not allowed for script positions
	 *        	"CClientScript::POS_LOAD" and "CClientScript::POS_READY".
	 * @return CClientScript the CClientScript object itself (to support method
	 *         chaining, available since version 1.1.5).
	 */
	 ...
	 *
	 * <b>Note:</b>
	 * Each call of this method will cause a rendering of new meta tag, even if
	...
	/**
	 * Registers a link tag that will be inserted in the head section (right
	 * before the title element) of the resulting page.
	 * 
	 ...
	 * @return CClientScript the CClientScript object itself (to support method
	 *         chaining, available since version 1.1.10).
	 *        
	 * @since 1.1.9
	 */
