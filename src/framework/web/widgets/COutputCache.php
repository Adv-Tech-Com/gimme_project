 ...
 * For the latter, the shortcuts {@link CBaseController::beginCache()} and
 * {@link CBaseController::endCache()}
 ...
 * {@link cacheID}.
 * If the cache application component is not available, COutputCache will be
 * disabled.
 *
 ...
 * any subsequent display of the form should not be cached because it contains
 * user input.
 * We can set {@link requestTypes} to be <code>array('GET')</code> to accomplish
 * this task.
 *
 ...
 * and uses the corresponding values to determine the version of the cached
 * content.</li>
 ...
 *
 * @property boolean $isContentCached Whether the content can be found from
 *           cache.
 *          
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class COutputCache extends CFilterWidget {
	...
	/**
	 *
	 * @var integer number of seconds that the data can remain in cache.
	 *      Defaults to 60 seconds.
	 *      If it is 0, existing cached content would be removed from the cache.
	 *      If it is a negative value, the cache will be disabled (any existing
	 *      cached content will
	 *      remain in the cache.)
	 *     
	...
	/**
	 *
	 * @var boolean whether the content being cached should be differentiated
	 *      according to route.
	 ...
	 *
	 * @var boolean whether the content being cached should be differentiated
	 *      according to user sessions. Defaults to false.
	 */
	public $varyBySession = false;
	/**
	 *
	 * @var array list of GET parameters that should participate in cache key
	 *      calculation.
	 *      By setting this property, the output cache will use different cached
	 *      data
	...
	/**
	 *
	 * @var string a PHP expression whose result is used in the cache key
	 *      calculation.
	 *      By setting this property, the output cache will use different cached
	 *      data
	...
	/**
	 *
	 * @var array list of request types (e.g. GET, POST) for which the cache
	 ...
	 *      This can be either an object implementing {@link ICacheDependency}
	 ...
	 *      )
	 *      </pre>
	 *      would make the output cache depends on the last modified time of all
	 *      posts.
	 *      If any post has its modification time changed, the cached content
	 ...
	 * This method is meant to be overridden by child classes if begin-filtering
	 * is needed.
	 * 
	 * @param CFilterChain $filterChain
	 *        	list of filters being applied to an action
	 * @return boolean whether the filtering process should stop after this
	 ...
	 * The key is calculated based on {@link getBaseCacheKey} and other factors,
	 * including
	 * {@link varyByRoute}, {@link varyByParam}, {@link varyBySession} and
	 * {@link varyByLanguage}.
	 * 
	 ...
	 *        	a property name of the controller. The property should refer
	 *        	to an object
	 *        	whose method is being recorded. If empty it means the
	 *        	controller itself.
	 * @param string $method
	 *        	the method name
	 * @param array $params
