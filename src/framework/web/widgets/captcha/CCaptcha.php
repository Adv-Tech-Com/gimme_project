 ...
 *
 * CCaptcha is used together with {@link CCaptchaAction} to provide {@link
...
class CCaptcha extends CWidget {
	/**
	 *
	 * @var string the ID of the action that should provide CAPTCHA image.
	 *      Defaults to 'captcha',
	 *      meaning the 'captcha' action of the current controller. This
	 *      property may also
	 *      be in the format of 'ControllerID/ActionID'. Underneath, this
	 *      property is used
	 *      by {@link CController::createUrl} to create the URL that would serve
	 *      the CAPTCHA image.
	...
	/**
	 *
	 * @var boolean whether to display a button next to the CAPTCHA image.
	 *      Clicking on the button
	 *      will cause the CAPTCHA image to be changed to a new one. Defaults to
	 *      true.
	 */
	public $showRefreshButton = true;
	/**
	 *
	 * @var boolean whether to allow clicking on the CAPTCHA image to refresh
	 *      the CAPTCHA letters.
	...
	/**
	 *
	 * @var string the label for the refresh button. Defaults to 'Get a new
	 *      code'.
	 */
	public $buttonLabel;
	/**
	 *
	 * @var string the type of the refresh button. This should be either 'link'
	 *      or 'button'.
	 *      The former refers to hyperlink button while the latter a normal push
	 *      button.
	 *      Defaults to 'link'.
	 */
	...
	/**
	 *
	 * @var array HTML attributes to be applied to the rendered refresh button
	 *      element.
	 */
	...
	public function registerClientScript() {
		...
		if ($this->showRefreshButton) {
			// reserve a place in the registered script so that any enclosing
			// button js code appears after the captcha js
...
";
	 ...
	 *        	name to be checked. Possible values are 'gd', 'imagick' and
	 *        	null.
	 *        	Default value is null meaning that both extensions will be
	 *        	checked. This parameter
	 *        	is available since 1.1.13.
	 * @return boolean true if ImageMagick extension with PNG support or GD with
	 *         FreeType support is loaded,
	 *         otherwise false
	 * @since 1.1.5
	 */
