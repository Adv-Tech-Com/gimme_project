 ...
 * The 'beginWidget' and 'endWidget' call of CActiveForm widget will render
 * the open and close form tags. Most other methods of CActiveForm are wrappers
 * of the corresponding 'active' methods in {@link CHtml}. Calling them in
 * between
 * the 'beginWidget' and 'endWidget' calls will render text labels, input
 * fields,
 * etc. For example, calling {@link CActiveForm::textField}
 * would generate an input field for a specified model attribute.
 *
 ...
 * <ul>
 * <li>server-side validation: the validation is performed at server side after
 * the whole page containing the form is submitted. If there is any validation
 * error,
 * CActiveForm will render the error in the page back to user.</li>
 * <li>AJAX-based validation: when the user enters data into an input field,
 * an AJAX request is triggered which requires server-side validation. The
 * validation
 * result is sent back in AJAX response and the input field changes its
 * appearance
 * accordingly.</li>
 * <li>client-side validation (available since version 1.1.7):
 * when the user enters data into an input field,
 * validation is performed on the client side using JavaScript. No server
 * contact
 * will be made, which reduces the workload on the server.</li>
 * </ul>
 *
 ...
 *
 * To ensure data validity, server-side validation is always performed.
 * By setting {@link enableAjaxValidation} to true, one can enable AJAX-based
 * validation;
 * and by setting {@link enableClientValidation} to true, one can enable
 * client-side validation.
 * Note that in order to make the latter two validations work, the user's
 * browser
 * must has its JavaScript enabled. If not, only the server-side validation will
 * be performed.
 *
 * The AJAX-based validation and client-side validation may be used together
 * or separately. For example, in a user registration form, one may use
 * AJAX-based
 ...
 *
 * The AJAX-based validation has a few limitations. First, it does not work
 * with file upload fields. Second, it should not be used to perform validations
 * that
 * may cause server-side state changes. Third, it is not designed
 * to work with tabular data input for the moment.
 *
 ...
 * validator
 * will support client-side validation only if it implements {@link
 ...
 * automatically assigns different CSS classes for the last three states
 * to the HTML element containing the input field.
 * By default, these CSS classes are named as 'validating', 'error' and
 * 'success',
 * respectively. We may customize these CSS classes by configuring the
 * {@link clientOptions} property or specifying in the {@link error} method.
 *
...
class CActiveForm extends CWidget {
	/**
	 *
	 * @var mixed the form action URL (see {@link CHtml::normalizeUrl} for
	...
	/**
	 *
	 * @var string the form submission method. This should be either 'post' or
	 *      'get'.
	 *      Defaults to 'post'.
	 */
	public $method = 'post';
	 ...
	 *      CHtml::statefulForm}). Defaults to false.
	 */
	public $stateful = false;
	/**
	 *
	 * @var string the CSS class name for error messages.
	 *      Since 1.1.14 this defaults to 'errorMessage' defined in {@link
	 *      CHtml::$errorMessageCss}.
	 *      Individual {@link error} call may override this value by specifying
	 *      the 'class' HTML option.
	 */
	public $errorMessageCssClass;
	/**
	 *
	 * @var array additional HTML attributes that should be rendered for the
	 *      form tag.
	 */
	 ...
	 *      <ul>
	 *      <li>ajaxVar: string, the name of the parameter indicating the
	 *      request is an AJAX request.
	 *      When the AJAX validation is triggered, a parameter named as this
	 *      property will be sent
	 *      together with the other form data to the server. The parameter value
	 *      is the form ID.
	 *      The server side can then detect who triggers the AJAX validation and
	 *      react accordingly.
	 *      Defaults to 'ajax'.</li>
	 *      <li>validationUrl: string, the URL that performs the AJAX
	 *      validations.
	 *      If not set, it will take the value of {@link action}.</li>
	 *      <li>validationDelay: integer, the number of milliseconds that an
	 *      AJAX validation should be
	 *      delayed after an input is changed. A value 0 means the validation
	 *      will be triggered immediately
	 *      when an input is changed. A value greater than 0 means changing
	 *      several inputs may only
	 *      trigger a single validation if they happen fast enough, which may
	 *      help reduce the server load.
	 *      Defaults to 200 (0.2 second).</li>
	 *      <li>validateOnSubmit: boolean, whether to perform AJAX validation
	 *      when the form is being submitted.
	 *      If there are any validation errors, the form submission will be
	 *      stopped.
	 ...
	 *      this off
	 *      if it causes too much performance impact, because each AJAX
	 *      validation request
	 *      will submit the data of the whole form. Defaults to true.</li>
	 *      <li>validateOnType: boolean, whether to trigger an AJAX validation
	 *      each time when the user
	 *      presses a key. When setting this property to be true, you should
	 *      tune up the 'validationDelay'
	 *      option to avoid triggering too many AJAX validations. Defaults to
	 *      false.</li>
	 *      <li>hideErrorMessage: boolean, whether to hide the error message
	 *      even if there is an error.
	 *      Defaults to false, which means the error message will show up
	 *      whenever the input has an error.</li>
	 *      <li>inputContainer: string, the jQuery selector for the HTML element
	 *      containing the input field.
	 *      During the validation process, CActiveForm will set different CSS
	 *      class for the container element
	 *      to indicate the state change. If not set, it means the closest 'div'
	 *      element that contains the input field.</li>
	 *      <li>errorCssClass: string, the CSS class to be assigned to the
	 *      container whose associated input
	 *      has AJAX validation error. Defaults to 'error'.</li>
	 *      <li>successCssClass: string, the CSS class to be assigned to the
	 *      container whose associated input
	 *      passes AJAX validation without any error. Defaults to
	 *      'success'.</li>
	 *      <li>validatingCssClass: string, the CSS class to be assigned to the
	 *      container whose associated input
	 *      is currently being validated via AJAX. Defaults to
	 *      'validating'.</li>
	 *      <li>errorMessageCssClass: string, the CSS class assigned to the
	 *      error messages returned
	 *      by AJAX validations. Defaults to 'errorMessage'.</li>
	 *      <li>beforeValidate: function, the function that will be invoked
	 *      before performing ajax-based validation
	 *      triggered by form submission action (available only when
	 *      validateOnSubmit is set true).
	 *      The expected function signature should be <code>beforeValidate(form)
	 *      {...}</code>, where 'form' is
	 *      the jquery representation of the form object. If the return value of
	 *      this function is NOT true, the validation
	 *      will be cancelled.
	 *     
	 *      Note that because this option refers to a js function, you should
	 *      wrap the value with {@link CJavaScriptExpression} to prevent it
	 *      from being encoded as a string. This option has been available since
	 *      version 1.1.3.</li>
	 *      <li>afterValidate: function, the function that will be invoked after
	 *      performing ajax-based validation
	 *      triggered by form submission action (available only when
	 *      validateOnSubmit is set true).
	 *      The expected function signature should be <code>afterValidate(form,
	 *      data, hasError) {...}</code>, where 'form' is
	 *      the jquery representation of the form object; 'data' is the JSON
	 *      response from the server-side validation; 'hasError'
	 *      is a boolean value indicating whether there is any validation error.
	 *      If the return value of this function is NOT true,
	 ...
	 *      from being encoded as a string. This option has been available since
	 *      version 1.1.3.</li>
	 *      <li>beforeValidateAttribute: function, the function that will be
	 *      invoked before performing ajax-based validation
	 *      triggered by a single attribute input change. The expected function
	 *      signature should be
	 *      <code>beforeValidateAttribute(form, attribute) {...}</code>, where
	 *      'form' is the jquery representation of the form object
	 *      and 'attribute' refers to the js options for the triggering
	 *      attribute (see {@link error}).
	 *      If the return value of this function is NOT true, the validation
	 *      will be cancelled.
	 *     
	 *      Note that because this option refers to a js function, you should
	 *      wrap the value with {@link CJavaScriptExpression} to prevent it
	 *      from being encoded as a string. This option has been available since
	 *      version 1.1.3.</li>
	 *      <li>afterValidateAttribute: function, the function that will be
	 *      invoked after performing ajax-based validation
	 *      triggered by a single attribute input change. The expected function
	 *      signature should be
	 *      <code>afterValidateAttribute(form, attribute, data, hasError)
	 *      {...}</code>, where 'form' is the jquery
	 *      representation of the form object; 'attribute' refers to the js
	 *      options for the triggering attribute (see {@link error});
	 *      'data' is the JSON response from the server-side validation;
	 *      'hasError' is a boolean value indicating whether
	 ...
	 *      hideErrorMessage,
	 *      inputContainer, errorCssClass, successCssClass, validatingCssClass,
	 *      beforeValidateAttribute, afterValidateAttribute.
	 */
	public $clientOptions = array ();
	/**
	 *
	 * @var boolean whether to enable data validation via AJAX. Defaults to
	 *      false.
	 *      When this property is set true, you should respond to the AJAX
	 *      validation request on the server side as shown below:
	 *      <pre>
	 *      public function actionCreate()
	 *      {
	 *      $model=new User;
	...
	/**
	 *
	 * @var boolean whether to enable client-side data validation. Defaults to
	 *      false.
	 *     
	 *      When this property is set true, client-side validation will be
	 *      performed by validators
	 *      that support it (see {@link CValidator::enableClientValidation} and
	 *      {@link CValidator::clientValidateAttribute}).
	 *     
	 * @see error
	 * @since 1.1.7
	 */
	 ...
	 *      If set as array, first element should be model and second element
	 *      should be the attribute.
	 *      If set as string any jQuery selector can be used
	 *     
	 *      Example - set input focus on page load to:
	 *      <ul>
	 *      <li>'focus'=>array($model,'username') - $model->username input
	 *      filed</li>
	 *      <li>'focus'=>'#'.CHtml::activeId($model,'username') -
	 *      $model->username input field</li>
	 *      <li>'focus'=>'#LoginForm_username' - input field with ID
	 *      LoginForm_username</li>
	 *      <li>'focus'=>'input[type="text"]:first' - first input element of
	 *      type text</li>
	 *      <li>'focus'=>'input:visible:enabled:first' - first visible and
	 *      enabled input element</li>
	...
	/**
	 *
	 * @var array the javascript options for model attributes (input ID =>
	 *      options)
	 * @see error
	 * @since 1.1.7
	 */
	 ...
	 *    <?php/**
	 * Displays the first validation error for a model attribute.
	 * This is similar to {@link CHtml::error} except that it registers the
	 * model attribute
	 * so that if its value is changed by users, an AJAX validation may be
	 * triggered.
	 * 
	 * @param CModel $model
	 *        	the data model
	 ...
	 *        	tag.
	 *        	Besides all those options available in {@link CHtml::error},
	 ...
	 *        	<li>afterValidateAttribute</li>
	 *        	</ul>
	 *        	These options override the corresponding options as declared
	 *        	in {@link options} for this
	 *        	particular model attribute. For more details about these
	 *        	options, please refer to {@link clientOptions}.
	 *        	Note that these options are only used when {@link
	 *        	enableAjaxValidation} or {@link enableClientValidation}
	 *        	is set true.
	 *        	<ul>
	 *        	<li>inputID</li>
	 *        	</ul>
	 *        	When an CActiveForm input field uses a custom ID, for
	 *        	ajax/client validation to work properly
	 *        	inputID should be set to the same ID
	 *        	
	 *        	Example:
	 *        	<pre>
	 ...
	 *        	</pre>
	 *        	
	 *        	When client-side validation is enabled, an option named
	 *        	"clientValidation" is also recognized.
	 *        	This option should take a piece of JavaScript code to perform
	 *        	client-side validation. In the code,
	 *        	the variables are predefined:
	 *        	<ul>
	 *        	<li>value: the current input value associated with this
	 *        	attribute.</li>
	 *        	<li>messages: an array that may be appended with new error
	 *        	messages for the attribute.</li>
	 *        	<li>attribute: a data structure keeping all client-side
	 *        	options for the attribute</li>
	 *        	</ul>
	 *        	This should NOT be a function but just the code, Yii will
	 *        	enclose the code you provide inside the
	 *        	actual JS function.
	 * @param boolean $enableAjaxValidation
	 *        	whether to enable AJAX validation for the specified attribute.
	 *        	Note that in order to enable AJAX validation, both {@link
	 *        	enableAjaxValidation} and this parameter
	 *        	must be true.
	 * @param boolean $enableClientValidation
	 *        	whether to enable client-side validation for the specified
	 *        	attribute.
	 *        	Note that in order to enable client-side validation, both
	 *        	{@link enableClientValidation} and this parameter
	 *        	must be true. This parameter has been available since version
	 *        	1.1.7.
	 ...
	 * This method is very similar to {@link CHtml::errorSummary} except that it
	 * also works
	 ...
	 * Please check {@link CHtml::activeRadioButtonList} for detailed
	 * information
	 * about the parameters for this method.
	 * 
	 * @param CModel $model
	 ...
	 * This is a helper method that simplifies the way of writing AJAX
	 * validation code.
	 * 
	 * @param mixed $models
	 ...
	 *        	meaning any attribute listed in the applicable validation
	 *        	rules of the models should be
	 *        	validated. If this parameter is given as a list of attributes,
	 *        	only
	 *        	the listed attributes will be validated.
	 * @param boolean $loadInput
	 *        	whether to load the data from $_POST array in this method.
	 *        	If this is true, the model will be populated from
	 *        	<code>$_POST[ModelClass]</code>.
	...
	/**
	 * Validates an array of model instances and returns the results in JSON
	 * format.
	 * This is a helper method that simplifies the way of writing AJAX
	 * validation code for tabular input.
	 * 
	 * @param mixed $models
