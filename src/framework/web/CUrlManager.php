 ...
 *
 * It provides URL construction ({@link createUrl()}) as well as parsing ({@link
 * parseUrl()}) functionality.
 *
 ...
 * </ul>
 *
 * When using 'path' format, CUrlManager uses a set of {@link setRules rules}
 * to:
 * <ul>
 * <li>parse the requested URL into a route ('ControllerID/ActionID') and GET
 * parameters;</li>
 * <li>create URLs based on the given route and GET parameters.</li>
 * </ul>
 *
 * A rule consists of a route and a pattern. The latter is used by CUrlManager
 * to determine
 * which rule is used for parsing/creating URLs. A pattern is meant to match the
 * path info
 * part of a URL. It may contain named parameters using the syntax
 * '&lt;ParamName:RegExp&gt;'.
 *
 * When parsing a URL, a matching rule will extract the named parameters from
 * the path info
 * and put them into the $_GET variable; when creating a URL, a matching rule
 * will extract
 * the named parameters from $_GET and put them into the path info part of the
 * created URL.
 *
 * If a pattern ends with '/*', it means additional GET parameters may be
 * appended to the path
 * info part of the URL; otherwise, the GET parameters can only appear in the
 * query string part.
 *
 * To specify URL rules, set the {@link setRules rules} property as an array of
 * rules (pattern=>route).
 * For example,
 * <pre>
 * array(
 ...
 * <ul>
 * <li>The first rule says that if the user requests the URL
 * '/path/to/index.php/articles',
 * it should be treated as '/path/to/index.php/article/list'; and vice versa
 * applies
 * when constructing such a URL.</li>
 * <li>The second rule contains a named parameter 'id' which is specified using
 * the &lt;ParamName:RegExp&gt; syntax. It says that if the user requests the
 * URL
 * '/path/to/index.php/article/13', it should be treated as
 * '/path/to/index.php/article/read?id=13';
 * and vice versa applies when constructing such a URL.</li>
 * </ul>
 *
 * The route part may contain references to named parameters defined in the
 * pattern part.
 * This allows a rule to be applied to different routes based on matching
 * criteria.
 * For example,
 * <pre>
 * array(
 ...
 * part. The '<_c>'
 * parameter matches either 'post' or 'comment', while the '<_a>' parameter
 * matches an action ID.
 *
 ...
 * And given the route 'post/list' and GET parameter 'page' being 2, we should
 * get a URL
 ...
 * For example, the URL <code>http://admin.example.com/en/profile</code> may be
 * parsed into GET parameters
 * <code>user=admin</code> and <code>lang=en</code>. On the other hand, rules
 * with hostname may also be used to
 ...
 * them for one or several URL rules.
 * For example,
 * <pre>
 * array(
 * // a standard rule
 ...
 * )
 * </pre>
 * Please note that the custom URL rule class should extend from {@link
 * CBaseUrlRule} and
 ...
 *
 * @property string $baseUrl The base URL of the application (the part after
 *           host name and before query string).
 *           If {@link showScriptName} is true, it will include the script name
 *           part.
 *           Otherwise, it will not, and the ending slashes are stripped off.
 * @property string $urlFormat The URL format. Defaults to 'path'. Valid values
 *           include 'path' and 'get'.
 *           Please refer to the guide for more details about the difference
...
class CUrlManager extends CApplicationComponent {
	 ...
	 *      For example, ".html" can be used so that the URL looks like pointing
	 *      to a static HTML page. Defaults to empty.
	 */
	public $urlSuffix = '';
	/**
	 *
	 * @var boolean whether to show entry script name in the constructed URL.
	 *      Defaults to true.
	 */
	public $showScriptName = true;
	/**
	 *
	 * @var boolean whether to append GET parameters to the path info part.
	 *      Defaults to true.
	 *      This property is only effective when {@link urlFormat} is 'path' and
	 *      is mainly used when
	 *      creating URLs. When it is true, GET parameters will be appended to
	 *      the path info and
	 *      separate from each other using slashes. If this is false, GET
	 *      parameters will be in query part.
	 */
	...
	/**
	 *
	 * @var boolean whether routes are case-sensitive. Defaults to true. By
	 *      setting this to false,
	 *      the route in the incoming request will be turned to lower case first
	 *      before further processing.
	 *      As a result, you should follow the convention that you use lower
	 *      case when specifying
	 *      controller mapping ({@link CWebApplication::controllerMap}) and
	 *      action mapping
	 *      ({@link CController::actions}). Also, the directory names for
	 *      organizing controllers should
	 *      be in lower case.
	 */
	 ...
	 *      If it is set true, then an incoming URL must match one of the {@link
	 *      rules URL rules}.
	 *      Otherwise, it will be treated as an invalid request and trigger a
	 *      404 HTTP exception.
	 *      Defaults to false.
	 */
	...
	/**
	 *
	 * @var string the class name or path alias for the URL rule instances.
	 *      Defaults to 'CUrlRule'.
	 *      If you change this to something else, please make sure that the new
	 *      class must extend from
	 *      {@link CBaseUrlRule} and have the same constructor signature as
	 *      {@link CUrlRule}.
	 ...
	 *        	whether the new URL rules should be appended to the existing
	 *        	ones. If false,
	 ...
	 * @param array $params
	 *        	list of GET parameters (name=>value). Both the name and value
	 *        	will be URL-encoded.
	 ...
	 * 
	 * @return string the base URL of the application (the part after host name
	 *         and before query string).
	 *         If {@link showScriptName} is true, it will include the script
	 *         name part.
	 *         Otherwise, it will not, and the ending slashes are stripped off.
	 */
	 ...
	 * query string).
	 * This method is provided in case the {@link baseUrl} cannot be determined
	 * automatically.
	 * The ending slashes should be stripped off. And you are also responsible
	 * to remove the script name
	 ...
	 * 
	 * @return string the URL format. Defaults to 'path'. Valid values include
	 *         'path' and 'get'.
	 *         Please refer to the guide for more details about the difference
	 *         between these two formats.
	 */
...
abstract class CBaseUrlRule extends CComponent {
	 ...
	 *        	the request object
	 * @param string $pathInfo
	 *        	path info part of the URL (URL suffix is already removed based
...
class CUrlRule extends CBaseUrlRule {
	 ...
	 *      Defaults to null, meaning using the value of {@link
