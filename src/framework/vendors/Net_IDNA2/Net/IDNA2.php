 ...
 * worldwide
 * to be translated between their original (localized) form and their encoded
 * form
...
class Net_IDNA2 {
	 ...
	 * default is false.]
	 * [strict - true: strict mode, good for registration purposes - Causes
	 * errors
	 * on failures; false: loose mode, ideal for "wildlife" applications
	 * by silently ignoring errors and returning the original input instead]
	 *
	 * @param mixed $option
	 *        	Parameter to set (string: single parameter; array of Parameter
	 *        	=> Value pairs)
	 * @param string $value
	...
	public function decode($input, $one_time_encoding = false) {
		...
		} elseif (preg_match ( '![:\./]!', $input )) { // Or a complete domain name
		...
		// The output is UTF-8 by default, other output formats need conversion
		// here
		// If one time encoding is given, use this, else the objects property
		switch (($one_time_encoding) ? $one_time_encoding : $this->_api_encoding) {
			case 'utf8' :
	...
	private function _encode($decoded) {
		...
		while ( $codecount < $deco_len ) {
			...
			// Scan input again and encode all characters whose code point is
			// $cur_code
			for($i = 0; $i < $deco_len; $i ++) {
	...
	private function _utf8_to_ucs4($input) {
		...
		for($k = 0; $k < $inp_len; ++ $k) {
			...
			if ('next' == $mode) { // Try to find the next start byte; determine
			                       // the width of the Unicode char
				$start_byte = $v;
				$mode = 'add';
				$test = 'range';
				if ($v >> 5 == 6) { // &110xxxxx 10xxxxx
					$next_byte = 0; // Tells, how many times subsequent bitmasks must
					                // rotate 6bits to the left
					$v = ($v - 192) << 6;
					...
					$next_byte = 2;
					$v = ($v - 240) << 18;
				} elseif ($v >> 2 == 62) { // &111110xx 10xxxxxx 10xxxxxx 10xxxxxx
				                           // 10xxxxxx
					$next_byte = 3;
					$v = ($v - 248) << 24;
				} elseif ($v >> 1 == 126) { // &1111110x 10xxxxxx 10xxxxxx 10xxxxxx
				                            // 10xxxxxx 10xxxxxx
					$next_byte = 4;
					$v = ($v - 252) << 30;
				} else {
	...
	/**
	 * Gives you a bit representation of given Byte (8 bits), Word (16 bits) or
	 * DWord (32 bits)
