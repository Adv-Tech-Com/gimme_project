...
if (! defined ( 'HL_NUMBERS_LI' )) {
	...
	/**
	 * Use 2-column table with line numbers in left column and code in right
	 * column.
	 */
 ...
 * - 'use_language' - class names will be prefixed with language, like
 * "php-reserved" or "css-code"
 *
...
class Text_Highlighter_Renderer_Html extends Text_Highlighter_Renderer_Array {
	 ...
	 *        
	 *        
	 *         Descendents of Text_Highlighter call this method from the
	 *         constructor,
	...
	function reset() {
		...
		if (isset ( $this->_options ['doclinks'] ) && is_array ( $this->_options ['doclinks'] ) && ! empty ( $this->_options ['doclinks'] ['url'] )) {
			...
			$this->_doclinks = $this->_options ['doclinks']; // keys: url, target,
			                                                // elements array
	...
	function finalize() {
		...
		foreach ( $output as $token ) {
			...
			// print "<pre> token = ".var_export($token, true)." -- span = " .
			// htmlentities($span). "-- deco = ".$decorated_output."</pre>\n";
	 ...
	 * @param string $class
	 *        	Class name
	 *        	@paran boolean $span_tag Whether or not to return styling
	 *        	attributes in a <code>&gt;span&lt;</code> tag
	 * @return string <code>span</code> tag or just a <code>class</code> and/or
