...
class Markdown_Parser {
	...
	public function formParagraphs($text) {
		...
		foreach ( $grafs as $key => $value ) {
			if (! preg_match ( '/^B\x1A[0-9]+B$/', $value )) {
				...
				// # that initialization code should be pulled into its own sub,
				// though.
	...
	public function _initDetab() {
		...
		// function that will loosely count the number of UTF-8 characters with
		// a
		// regular expression.
		//
...
class MarkdownExtra_Parser extends Markdown_Parser {
	...
	public function hashHTMLBlocks($text) {
		...
		// _HashHTMLBlocks_InMarkdown to handle the Markdown syntax within the
		// tag.
	...
	public function _hashHTMLBlocks_inMarkdown($text, $indent = 0, $enclosing_tag_re = '', $span = false) {
		...
		//
		// * If $enclosing_tag_re is not empty, stops at the first unmatched
		// closing
		// tag with that name. Nested tags supported.
		//
		// * If $span is true, text inside must treated as span. So any double
		// newline will be replaced by a single newline so that it does not
		// create
		// paragraphs.
		//
		...
		//
		              // Loop through every tag until we find the closing tag of
		              // the parent
		              // or loop until reaching the end of text if no parent tag
		              // specified.
		              //
		do {
			//
			...
			//
				...
				// Make sure it stays outside of any paragraph by adding
				// newlines.
				$parsed .= "\n\n$block_text\n\n";
			} 			//
...
preg_match ( '{^</?(?:' . $enclosing_tag_re . ')\b}', $tag )) {
	...
	public function _hashHTMLBlocks_inHTML($text, $hash_method, $md_attr) {
		...
		//
		               ...
		               // (This pattern makes $base_tag_name_re safe without
		               // quoting.)
		               //
		...
		do {
			...
			if (count ( $parts ) < 3) {
				...
				// first character as filtered to prevent an infinite loop in
				// the
				// parent function.
				//
				return array (
			...
			if (preg_match ( '{^</?(?:' . $this->auto_close_tags_re . ')\b}', $tag ) || $tag {1} == '!' || $tag {1} == '?') {
				...
				if ($md_attr && preg_match ( $markdown_attr_re, $tag, $attr_m ) && preg_match ( '/^1|block|span$/', $attr_m [2] . $attr_m [3] )) {
					...
					// Check if text inside this tag must be parsed in span
					// mode.
	...
	public function hashClean($text) {
		//
		// Called whenever a tag must be hashed when a function insert a "clean"
		// tag
	...
	public function doFootnotes($text) {
		...
		// which will be replaced by the actual footnote marker in
		// appendFootnotes.
		//
		if (! $this->in_anchor) {
	...
	public function appendFootnotes($text) {
		...
		if (! empty ( $this->footnotes_ordered )) {
			...
			while ( ! empty ( $this->footnotes_ordered ) ) {
				...
				// Add backlink to last paragraph; create new paragraph if
				// needed.
