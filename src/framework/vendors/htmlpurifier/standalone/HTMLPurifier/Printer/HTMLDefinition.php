...
class HTMLPurifier_Printer_HTMLDefinition extends HTMLPurifier_Printer {
	 ...
	 * 
	 * @param $def HTMLPurifier_ChildDef
	 *        	of pertinent element
	 */
	protected function renderChildren($def) {
		...
		if ($def->type == 'chameleon') {
			...
			$ret .= $this->element ( 'td', '<em>Block</em>: ' . $this->escape ( $this->listifyTagLookup ( $def->block->elements ) ), 0, 0 );
			$ret .= $this->end ( 'tr' );
			$ret .= $this->start ( 'tr' );
			$ret .= $this->element ( 'td', '<em>Inline</em>: ' . $this->escape ( $this->listifyTagLookup ( $def->inline->elements ) ), 0, 0 );
		} elseif ($def->type == 'custom') {
			
			$ret .= $this->element ( 'td', '<em>' . ucfirst ( $def->type ) . '</em>: ' . $def->dtd_regex );
		} else {
			$ret .= $this->element ( 'td', '<em>' . ucfirst ( $def->type ) . '</em>: ' . $this->escape ( $this->listifyTagLookup ( $elements ) ), 0, 0 );
		}
		$ret .= $this->end ( 'tr' );
		return $ret;
	}
	 ...
	 * 
	 * @param $array Tag
	 *        	lookup array in form of array('tagname' => true)
	 */
	 ...
	 * 
	 * @param $array Array
	 *        	hash in form of array('attrname' => HTMLPurifier_AttrDef)
	 */
