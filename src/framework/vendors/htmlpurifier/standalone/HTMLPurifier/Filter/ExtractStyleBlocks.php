...
class HTMLPurifier_Filter_ExtractStyleBlocks extends HTMLPurifier_Filter {
	...
	public function preFilter($html, $config, $context) {
		...
		$context->register ( 'StyleBlocks', $style_blocks ); // $context must not be
		                                                  // reused
		if ($this->_tidy) {
			...
			foreach ( $decls as $selector => $style ) {
				...
				if ($selector === '')
					continue; // should not happen
					                                ...
					                                // selector | S+ [ combinator? selector ]?
					                                // ]?
						                                // combinator
						                                // : '+' S*
						                                // : '>' S*
						                                // simple_selector
						                                // : element_name [ HASH | class
					                                // | attrib | pseudo ]*
						                                // | [ HASH | class | attrib |
					                                // pseudo ]+
						                                // element_name
					                                ...
					                                // \'([^\n\r\f\\"]|\\{nl}|{escape})*\'
						                                //
						                                // We'll implement a subset (in
					                                // order to reduce attack
						                                // surface); in particular:
						                                //
						                                // - No Unicode support
						                                // - No escapes support
						                                // - No string support (by proxy
					                                // no attrib support)
						                                // - element_name is matched
					                                // against allowed
						                                // elements (some people might
					                                // find this
						                                // annoying...)
						                                // - Pseudo-elements one of
					                                // :first-child, :link,
