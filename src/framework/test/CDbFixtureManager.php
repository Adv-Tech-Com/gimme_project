 ...
 * using a fixture means that at the beginning of the method, the table has and
 * only
 ...
 * the database. If this file is not found, all available fixtures will be
 * loaded
 * into the database.
 *
 * @property CDbConnection $dbConnection The database connection.
 * @property array $fixtures The information of the available fixtures (table
 *           name => fixture file).
 *          
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.test
...
class CDbFixtureManager extends CApplicationComponent {
	/**
	 *
	 * @var string the name of the initialization script that would be executed
	 *      before the whole test set runs.
	 *      Defaults to 'init.php'. If the script does not exist, every table
	 *      with a fixture file will be reset.
	 */
	public $initScript = 'init.php';
	/**
	 *
	 * @var string the suffix for fixture initialization scripts.
	 *      If a table is associated with such a script whose name is TableName
	 *      suffixed this property value,
	 *      then the script will be executed each time before the table is
	 *      reset.
	 */
	 ...
	 *      Note, data in this database may be deleted or modified during
	 *      testing.
	...
	/**
	 *
	 * @var array list of database schemas that the test tables may reside in.
	 *      Defaults to
	 *      array(''), meaning using the default schema (an empty string refers
	 *      to the
	 *      default schema). This property is mainly used when turning on and
	 *      off integrity checks
	 *      so that fixture data can be populated into the database without
	 *      causing problem.
	 */
	 ...
	 * 
	 * @throws CException if {@link connectionID} application component is
	 *         invalid
	 ...
	 * This method is invoked in {@link init}. It executes the database init
	 * script
	 ...
	 * Otherwise, {@link truncateTable} will be invoked to delete all rows in
	 * the table
	 ...
	 * This method will insert rows given in the fixture into the corresponding
	 * table.
	 * The loaded rows will be returned by this method.
	 * If the table has auto-incremental primary key, each row will contain
	 * updated primary key value.
	 ...
	 * If a file's name is the same as a table name, it is considered to be the
	 * fixture data for that table.
	 * 
	 * @return array the information of the available fixtures (table name =>
	 *         fixture file)
	 */
	 ...
	 * This method may be used to temporarily turn off foreign constraints
	 * check.
	 * 
	 * @param boolean $check
	 ...
	 * sequence, if any.
	 * You may need to call {@link checkIntegrity} to turn off integrity check
	 * temporarily
	 ...
	 * 
	 * @param string $schema
	 *        	the schema name. Defaults to empty string, meaning the default
	 *        	database schema.
	 * @see truncateTable
	 */
	 ...
	 * and {@link getRecord}.
	 * Note, if a table does not have fixture data, {@link resetTable} will
	 * still
	 ...
	 *        	If table names, they must begin with a colon character (e.g.
	 *        	'Post'
	 ...
	 * The rows will have updated primary key values if the primary key is
	 * auto-incremental.
	 * 
	 * @param string $name
	 *        	the fixture name
	 * @return array the fixture data rows. False is returned if there is no
	 *         such fixture data.
	 */
	 ...
	 * @return CActiveRecord the ActiveRecord instance. False is returned if
	 *         there is no such fixture row.
	 */
