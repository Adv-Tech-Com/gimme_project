 ...
 * viewed.
 * It can generate HMAC and encrypt the data. The private key used to generate
 * HMAC
 * is set by {@link setValidationKey ValidationKey}. The key used to encrypt
 * data is
 * specified by {@link setEncryptionKey EncryptionKey}. If the above keys are
 * not
 * explicitly set, random keys will be generated and used.
 *
 * To protected data with HMAC, call {@link hashData()}; and to check if the
 * data
 ...
 *           If the key is not explicitly set, a random one is generated and
 *           returned.
...
class CSecurityManager extends CApplicationComponent {
	...
	/**
	 *
	 * @var string the name of the hashing algorithm to be used by {@link
	 *      computeHMAC}.
	 *      See {@link http://php.net/manual/en/function.hash-algos.php
	 *      hash-algos} for the list of possible
	 *      hash algorithms. Note that if you are using PHP 5.1.1 or below, you
	 *      can only use 'sha1' or 'md5'.
	 *     
	...
	/**
	 *
	 * @var mixed the name of the crypt algorithm to be used by {@link encrypt}
	 *      and {@link decrypt}.
	 *      This will be passed as the first parameter to {@link
	 *      http://php.net/manual/en/function.mcrypt-module-open.php
	 *      mcrypt_module_open}.
	 *     
	 *      This property can also be configured as an array. In this case, the
	 *      array elements will be passed in order
	 *      as parameters to mcrypt_module_open. For example,
	 *      <code>array('rijndael-256', '', 'ofb', '')</code>.
	 *     
	 ...
	 * @deprecated in favor of {@link generateRandomString()} since 1.1.14.
	 ...
	 * @param string $key
	 *        	the decryption key. This defaults to null, meaning using
	 *        	{@link getEncryptionKey EncryptionKey}.
	...
	/**
	 * Opens the mcrypt module with the configuration specified in {@link
	 * cryptAlgorithm}.
	 * 
	 * @throws CException if failed to initialize the mcrypt module or PHP
	 *         mcrypt extension
	 ...
	 *        	data to be hashed.
	 * @param string $key
	 *        	the private key to be used for generating HMAC. Defaults to
	 *        	null, meaning using {@link validationKey}.
	...
	/**
	 * Computes the HMAC for the data with {@link getValidationKey
	 * validationKey}.
	 ...
	 *        	See {@link http://php.net/manual/en/function.hash-algos.php
	 *        	hash-algos} for the list of possible
	 *        	hash algorithms. Note that if you are using PHP 5.1.1 or
	 *        	below, you can only use 'sha1' or 'md5'.
	 ...
	 * @return string boolean string or false in case it cannot be generated.
	 * @since 1.1.14
	 */
	 ...
	 *        	result cannot be generated. The method attempts to read from a
	 ...
	 *        	Wikipedia}.
	 *        	However, in some runtime environments, PHP has no access to a
	 *        	CS-PRNG, in which case
	 *        	the method returns false if $cryptographicallyStrong is true.
	 *        	When $cryptographicallyStrong is false,
	 *        	the method always returns a pseudorandom result but may fall
	 *        	back to using {@link generatePseudoRandomBlock}.
	 *        	This method does not guarantee that entropy, from sources
	 *        	external to the CS-PRNG, was mixed into
	 *        	the CS-PRNG state between each successive call. The caller can
	 *        	therefore expect non-blocking
	 *        	behavior, unlike, for example, reading from /dev/random on
	 *        	Linux, see
	 *        	{@link http://eprint.iacr.org/2006/086.pdf Gutterman et al
	 *        	2006}.
	 * @return boolean string random binary string or false on failure.
	 * @since 1.1.14
	 */
	 ...
	 * better than PHP's {@link mt_rand} built-in function, which is not really
	 * random.
	 * 
	...
	public function generatePseudoRandomBlock() {
		...
		for($i = 0; $i < 32; ++ $i)
			...
			// On UNIX and UNIX-like operating systems the numerical values in
		// `ps`, `uptime` and `iostat`
			// ought to be fairly unpredictable. Gather the non-zero digits from
		// those.
		foreach ( array (
				'ps',
				'uptime',
		...
		// Gather the current time's microsecond part. Note: this is only a
		// source of entropy on
		// the first call! If multiple calls are made, the entropy is only as
		// much as the
		// randomness in the time between calls.
		$bytes .= $this->substr ( microtime (), 2, 6 );
		
		// Concatenate everything gathered, mix it with sha512. hash() is part
		// of PHP core and
		// enabled by default but it can be disabled at compile time but we
	 ...
	 * 
	 * @return boolean string random binary string or false on error.
	 * @since 1.1.14
	 */
	public function generateSessionRandomBlock() {
		...
		if (ini_get ( 'session.entropy_length' ) != 20)
			...
			// These calls are (supposed to be, according to PHP manual) safe
		// even if
		...
		if (! $bytes)
			...
			// $bytes has 20 bytes of entropy but the session manager converts
		// the binary
			// random bytes into something readable. We have to convert that
		// back.
	 ...
	 * @return string the extracted part of string, or FALSE on failure or an
	 *         empty string.
	 */
