 ...
 * }
 * </pre>
 * where <code>$event</code> is an instance of {@link CEvent} or its child
 * class.
 * One can then raise the event by calling the on-method instead of {@link
 * raiseEvent} directly.
 *
 * Both property names and event names are case-insensitive.
 *
 * CComponent supports behaviors. A behavior is an
 * instance of {@link IBehavior} which is attached to a component. The methods
 * of
 * the behavior can be invoked as if they belong to the component. Multiple
 * behaviors
 * can be attached to the same component.
 *
 * To attach a behavior to a component, call {@link attachBehavior}; and to
 * detach the behavior
 * from the component, call {@link detachBehavior}.
 *
 * A behavior can be temporarily enabled or disabled by calling {@link
 * enableBehavior}
 * or {@link disableBehavior}, respectively. When disabled, the behavior methods
 * cannot
 * be invoked via the component.
 *
 * Starting from version 1.1.0, a behavior's properties (either its public
 * member variables or
 * its properties defined via getters and/or setters) can be accessed through
 * the component it
 * is attached to.
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class CComponent {
	...
	/**
	 * Returns a property value, an event handler list or a behavior based on
	 * its name.
	 * Do not call this method. This is a PHP magic method that we override
	 * to allow using the following syntax to read a property or obtain event
	 * handlers:
	 * <pre>
	 ...
	 * Do not call this method. This is a PHP magic method that we override
	 * to allow using the following syntax to set a property or attach an event
	 * handler
	 * <pre>
	 ...
	 * @return mixed
	 * @throws CException if the property/event is not defined or the property
	 *         is read only.
	 * @see __get
	 */
	 ...
	 *        	method parameters
	 * @throws CException if current class and its behaviors do not have a
	 *         method or closure with the given name
	 ...
	 *        	the behavior name
	 * @return IBehavior the behavior object, or null if the behavior does not
	 *         exist
	 */
	 ...
	 * 
	 * @param string $name
	 *        	the behavior's name. It should uniquely identify this
	 *        	behavior.
	 * @param mixed $behavior
	 *        	the behavior configuration. This is passed as the first
	 *        	parameter to {@link YiiBase::createComponent} to create the
	 *        	behavior object.
	 *        	You can also pass an already created behavior instance (the
	 *        	new behavior will replace an already created
	 ...
	 * @return IBehavior the detached behavior. Null if the behavior does not
	 *         exist.
	 */
	 ...
	 * @param string $name
	 *        	the event name
	 * @return boolean whether an event has been attached one or several
	 *         handlers
	 */
	 ...
	 *
	 * An event handler must be a valid PHP callback, i.e., a string referring
	 * to
	 ...
	 * @param CEvent $event
	 *        	the event parameter
	 * @throws CException if the event is undefined or an event handler is
	 *         invalid.
	 */
	...
	/**
	 * Evaluates a PHP expression or callback under the context of this
	 * component.
	 *
	 * Valid PHP callback can be class method name in the form of
	 * array(ClassName/Object, MethodName), or anonymous function (only
	 * available in PHP 5.3.0 or above).
	 *
	 * If a PHP callback is used, the corresponding function/method signature
	 * should be
	 * <pre>
	 ...
	 * passed
	 * to the callback as $param1, $param2, ...; and the last parameter will be
	 * the component itself.
	 *
	 * If a PHP expression is used, the second parameter will be "extracted"
	 * into PHP variables
	 * that can be directly accessed in the expression. See {@link
	 * http://us.php.net/manual/en/function.extract.php PHP extract}
	 * for more details. In the expression, the component object can be accessed
	 * using $this.
	 *
	 ...
	 * @param array $_data_
	 *        	additional parameters to be passed to the above
	 *        	expression/callback.
...
class CEvent extends CComponent {
	 ...
	 *      When a handler sets this true, the rest of the uninvoked event
	 *      handlers will not be invoked anymore.
	 */
