 ...
 * implement.
 *
 * After the application completes configuration, it will invoke the {@link
 * init()}
...
interface ICache {
	 ...
	 * Some caches (such as memcache, apc) allow retrieving multiple cached
	 * values at one time,
	 * which may improve the performance since it reduces the communication
	 * cost.
	 * In case a cache doesn't support this feature natively, it will be
	 ...
	 *         If a value is not cached or expired, the corresponding array
	 *         value will be false.
	 */
	 ...
	 *        	the number of seconds in which the cached value will expire. 0
	 *        	means never expire.
	 * @param ICacheDependency $dependency
	 *        	dependency of the cached item. If the dependency changes, the
	 *        	item is labelled invalid.
	 ...
	 * Be careful of performing this operation if the cache is shared by
 ...
 * to intercept the remote method invocation (e.g. for logging or authentication
 * purpose).
 * 
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @package system.base
...
interface IUserIdentity {
	 ...
	 * 
	 * @return mixed a value that uniquely represents the identity (e.g. primary
	 *         key value).
	 */
	public function getId();
	/**
	 ...
	 * 
	 * @return array additional identity information that needs to be persistent
	 *         during the user session (excluding {@link id}).
	 */
	public function getPersistentStates();
}

/**
 * IWebUser interface is implemented by a {@link CWebApplication::user user
...
interface IAuthManager {
	 ...
	 * @param mixed $userId
	 *        	the user ID. This should be either an integer or a string
	 *        	representing
	 ...
	 * An authorization item represents an action permission (e.g. creating a
	 * post).
	 * It has three types: operation, task and role.
	 * Authorization items form a hierarchy. Higher level items inheirt
	 * permissions representing
	 ...
	 *        	PHP code that will be executed when {@link checkAccess} is
	 *        	called for the item.
	 * @param mixed $data
	 ...
	 * 
	 * @param integer $type
	 *        	the item type (0: operation, 1: task, 2: role). Defaults to
	 *        	null,
	 *        	meaning returning all items regardless of their type.
	 * @param mixed $userId
	 *        	the user ID. Defaults to null, meaning returning all items
	 *        	even if
	 ...
	 *        	the old item name. If null, it means the item name is not
	 *        	changed.
	 */
	 ...
	 *        	the child item name
	 * @throws CException if either parent or child doesn't exist or if a loop
	 *         has been detected.
	 */
	public function addItemChild($itemName, $childName);
	/**
	 * Removes a child from its parent.
	 * Note, the child item is not deleted. Only the parent-child relationship
	 * is removed.
	 * 
	 * @param string $itemName
	 *        	the parent item name
	 ...
	 * @param string $bizRule
	 *        	the business rule to be executed when {@link checkAccess} is
	 *        	called
	 *        	for this particular authorization item.
	 * @param mixed $data
	 *        	additional data associated with this assignment
	 * @return CAuthAssignment the authorization assignment information.
	 * @throws CException if the item does not exist or if the item has already
	 ...
	 * sure you call this method to save the changed data into persistent
	 * storage.
	 */
	 ...
	 * A business rule is a piece of PHP code that will be executed when {@link
	 * checkAccess} is called.
	 * 
	 * @param string $bizRule
	 *        	the business rule to be executed.
	 * @param array $params
	 *        	additional parameters to be passed to the business rule when
	 *        	being executed.
	 * @param mixed $data
	 *        	additional data that is associated with the corresponding
 ...
 * class.
 *
 * When calling {@link CBaseController::createWidget}, if a widget factory is
 * available,
...
interface IWidgetFactory {
	/**
	 * Creates a new widget based on the given class name and initial
	 * properties.
	 * 
	 * @param CBaseController $owner
	 *        	the owner of the new widget
	 * @param string $className
	 *        	the class name of the widget. This can also be a path alias
	 *        	(e.g. system.web.widgets.COutputCache)
	 * @param array $properties
	 *        	the initial property values (name=>value) of the widget.
	 * @return CWidget the newly created widget whose properties have been
...
interface IDataProvider {
	/**
	 *
	 * @return string the unique ID that identifies the data provider from other
	 *         data providers.
	 */
	public function getId();
	/**
	 ...
	 * When {@link pagination} is set false, this returns the same value as
	 * {@link totalItemCount}.
	 * 
	 ...
	 * 
	 * @param boolean $refresh
	 *        	whether the total number of data items should be
	 *        	re-calculated.
	 ...
	 * @return array the list of data items currently available in this data
	 *         provider.
	 */
	 ...
	 * @return array the list of key values corresponding to {@link data}. Each
	 *         data item in {@link data}
	 *         is uniquely identified by the corresponding key value in this
	 *         array.
	 */
	public function getKeys($refresh = false);
	/**
	 *
	 * @return CSort the sorting object. If this is false, it means the sorting
	 *         is disabled.
	 */
	public function getSort();
	/**
	 *
	 * @return CPagination the pagination object. If this is false, it means the
 ...
 * log route.
 * You can attach classes that implement ILogFilter to {@link
 * CLogRoute::$filter}.
 *
...
interface ILogFilter {
	/**
	 * This method should be implemented to perform actual filtering of log
	 * messages
	 * by working on the array given as the first parameter.
	 * Implementation might reformat, remove or add information to logged
	 * messages.
	 * 
	 * @param array $logs
