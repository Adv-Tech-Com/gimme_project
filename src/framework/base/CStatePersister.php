 ...
 *
 * To retrieve the data from CStatePersister, call {@link load()}. To save the
 * data,
 * call {@link save()}.
 *
 ...
 * <li>state persister: data persisting through all requests/sessions (e.g. hit
 * counter).</li>
 * <li>cache: volatile and fast storage. It may be used as storage medium for
 * session or state persister.</li>
 * </ul>
 *
 * Since server resource is often limited, be cautious if you plan to use
 * CStatePersister
 * to store large amount of data. You should also consider using database-based
 * persister
...
class CStatePersister extends CApplicationComponent implements IStatePersister {
	/**
	 *
	 * @var string the file path storing the state data. Make sure the directory
	 *      containing
	 *      the file exists and is writable by the Web server process. If using
	 *      relative path, also
	 ...
	 *      cache the state values.
	 *      Defaults to 'cache' which refers to the primary cache application
	 *      component.
	 *      Set this property to false if you want to disable caching state
	 *      values.
	 */
	 ...
	 * This method overrides the parent implementation by making sure {@link
	 * stateFile}
