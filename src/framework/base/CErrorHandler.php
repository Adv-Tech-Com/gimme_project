 ...
 * (i.e. YII_DEBUG is defined as true). Detailed error information with source
 * code
...
class CErrorHandler extends CApplicationComponent {
	/**
	 *
	 * @var integer maximum number of source code lines to be displayed.
	 *      Defaults to 25.
	 */
	...
	/**
	 *
	 * @var integer maximum number of trace source code lines to be displayed.
	 *      Defaults to 10.
	 * @since 1.1.6
	 */
	...
	/**
	 *
	 * @var string the application administrator information (could be a name or
	 *      email link). It is displayed in error pages to end users. Defaults
	 *      to 'the webmaster'.
	 */
	public $adminInfo = 'the webmaster';
	/**
	 *
	 * @var boolean whether to discard any existing page output before error
	 *      display. Defaults to true.
	 */
	public $discardOutput = true;
	/**
	 *
	 * @var string the route (eg 'site/error') to the controller action that
	 *      will be used to display external errors.
	 *      Inside the action, it can retrieve the error information by
	 *      Yii::app()->errorHandler->error.
	 *      This property defaults to null, meaning CErrorHandler will handle
	 *      the error display.
	 */
	public $errorAction;
	private $_error;
	...
	public function handle($event) {
		// set event as handled to prevent it from being handled by other event
		// handlers
		$event->handled = true;
		...
		if ($this->discardOutput) {
			...
			// the following manual level counting is to deal with
			// zlib.output_compression set to On
			// for an output buffer created by zlib.output_compression set to On
			// ob_end_clean will fail
			for($level = ob_get_level (); $level > 0; -- $level) {
				if (! @ob_end_clean ())
					ob_clean ();
			}
			// reset headers in case there was an ob_start("ob_gzhandler")
			// before
	 ...
	 * The error is returned in terms of an array, with the following
	 * information:
	 * <ul>
	 ...
	 *        	See {@link getViewFile} for how a view file is located given
	 *        	its name.
	 * @param array $data
	...
	/**
	 * Returns a value indicating whether the call stack is from application
	 * code.
	 * 
	 * @param array $trace
	 *        	the trace data
	 ...
	 * @return string the textual representation of the given error code or the
	 *         replacement string if the error code is unknown
	 */
