 ...
 * </pre>
 * Please note that you must use {@link CUploadedFile::getInstances} for
 * multiple
 * file uploads.
 *
 ...
 * different
 * validation scenarios. When the file is too large, you may use the {@link
 * tooLarge} property
 * to define a custom error message. Similarly for {@link tooSmall}, {@link
 * wrongType} and
 * {@link tooMany}. The messages may contain additional placeholders that will
 * be replaced
 * with the actual content. In addition to the "{attribute}" placeholder,
 * recognized by all
 * validators (see {@link CValidator}), CFileValidator allows for the following
 * placeholders
 * to be specified:
 * <ul>
 * <li>{file}: replaced with the name of the file.</li>
 * <li>{limit}: when using {@link tooLarge}, replaced with {@link maxSize};
 * when using {@link tooSmall}, replaced with {@link minSize}; and when using
 * {@link tooMany}
 * replaced with {@link maxFiles}.</li>
 * <li>{extensions}: when using {@link wrongType}, it will be replaced with the
 * allowed extensions.</li>
 * </ul>
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class CFileValidator extends CValidator {
	...
	/**
	 *
	 * @var mixed a list of file name extensions that are allowed to be
	 *      uploaded.
	 *      This can be either an array or a string consisting of file extension
	 *      names
	 *      separated by space or comma (e.g. "gif, jpg").
	 *      Extension names are case-insensitive. Defaults to null, meaning all
	 *      file name
	...
	/**
	 *
	 * @var mixed a list of MIME-types of the file that are allowed to be
	 *      uploaded.
	 *      This can be either an array or a string consisting of MIME-types
	 *      separated
	 *      by space or comma (e.g. "image/gif, image/jpeg"). MIME-types are
	 *      case-insensitive. Defaults to null, meaning all MIME-types are
	 *      allowed.
	 *      In order to use this property fileinfo PECL extension should be
	 *      installed.
	 * @since 1.1.11
	 */
	public $mimeTypes;
	 ...
	 *      Defaults to null, meaning no limit.
	 *      Note, the size limit is also affected by 'upload_max_filesize' INI
	 *      setting
	...
	/**
	 *
	 * @var string the error message used when the uploaded file has an
	 *      extension name
	 ...
	 *      that is not listed among {@link mimeTypes}. In order to use this
	 *      property
	 ...
	 *      It defaults to 1, meaning single file upload. By defining a higher
	 *      number,
	...
	/**
	 *
	 * @var string the error message used if the count of multiple uploads
	 *      exceeds
	 *      limit.
	 */
	public $tooMany;
	/**
	 *
	 * @var boolean whether attributes listed with this validator should be
	...
	protected function validateFile($object, $attribute, $file) {
		...
		elseif (defined ( 'UPLOAD_ERR_EXTENSION' ) && $error == UPLOAD_ERR_EXTENSION) // available
		                                                                        // for
		                                                                        // PHP
		                                                                        // 5.2.0
		                                                                        // or
	 ...
	 * is case insensitive). If you pass here the number with a fractional part,
	 * then everything after
	 * the decimal point will be ignored (php.ini values common behavior). For
	 * example 1.5G value would be
	 * treated as 1G and 1073741824 number will be returned as a result. This
	 * method is public
