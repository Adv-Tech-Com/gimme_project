 ...
 * message,
 * CStringValidator has a couple custom error messages you can set that
 * correspond to different
 * validation scenarios. For defining a custom message when the string is too
 * short,
 * you may use the {@link tooShort} property. Similarly with {@link tooLong}.
 * The messages may contain
 * placeholders that will be replaced with the actual content. In addition to
 * the "{attribute}"
 * placeholder, recognized by all validators (see {@link CValidator}),
 * CStringValidator allows for the following
 * placeholders to be specified:
 * <ul>
 * <li>{min}: when using {@link tooShort}, replaced with minimum length, {@link
 * min}, if set.</li>
 * <li>{max}: when using {@link tooLong}, replaced with the maximum length,
 * {@link max}, if set.</li>
 * <li>{length}: when using {@link message}, replaced with the exact required
 * length, {@link is}, if set.</li>
 * </ul>
 *
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class CStringValidator extends CValidator {
	...
	/**
	 *
	 * @var integer exact length. Defaults to null, meaning no exact length
	 *      limit.
	 */
	public $is;
	/**
	...
	/**
	 *
	 * @var string the encoding of the string value to be validated (e.g.
	 *      'UTF-8').
	 ...
	 *      mb_strlen() function. If this property is not set, the application
	 *      charset
	 *      will be used.
	 *      If this property is set false, then strlen() will be used even if
	 *      mbstring is enabled.
	 * @since 1.1.1
	 */
	public $encoding;
