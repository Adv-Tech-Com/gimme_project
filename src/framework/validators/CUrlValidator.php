...
class CUrlValidator extends CValidator {
	 ...
	 *      Since version 1.1.7 the pattern may contain a {schemes} token that
	 *      will be replaced
	...
	/**
	 *
	 * @var array list of URI schemes which should be considered valid. By
	...
	/**
	 *
	 * @var string the default URI scheme. If the input doesn't contain the
	 *      scheme part, the default
	 *      scheme will be prepended to it (thus changing the input). Defaults
	 *      to null, meaning a URL must
	...
	/**
	 *
	 * @var boolean whether validation process should care about IDN
	 *      (internationalized domain names). Default
	 *      value is false which means that validation of URLs containing IDN
	 *      will always fail.
	 * @since 1.1.13
	 */
	 ...
	 * @return mixed false if the the value is not a valid URL, otherwise the
	 *         possibly modified value ({@see defaultScheme})
	 * @since 1.1.1
	 */
	public function validateValue($value) {
		if (is_string ( $value ) && strlen ( $value ) < 2000) 		// make sure the length is
		                                             // limited to avoid DOS attacks
		{
	...
	public function clientValidateAttribute($object, $attribute) {
		if ($this->validateIDN) {
			...
			// punycode.js works only with the domains - so we have to extract
			// it before punycoding
			$validateIDN = '
