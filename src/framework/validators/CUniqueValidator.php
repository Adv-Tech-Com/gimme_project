...
class CUniqueValidator extends CValidator {
	 ...
	 *      Note, by setting it to false, you are assuming the attribute type is
	 *      string.
	 */
	 ...
	 *      used to look for the attribute value being validated. Defaults to
	 *      null,
	 ...
	 *      This will be combined with the condition that checks if the
	 *      attribute
	...
	/**
	 *
	 * @var string the user-defined error message. The placeholders
	 *      "{attribute}" and "{value}"
	 *      are recognized, which will be replaced with the actual attribute
	 *      name and value, respectively.
	 */
	public $message;
	/**
	 *
	 * @var boolean whether this validation rule should be skipped if when there
	 *      is already a validation
		...
		if (! $object instanceof CActiveRecord || $object->isNewRecord || $object->tableName () !== $finder->tableName ())
			...
			if ($n === 1) {
				if ($column->isPrimaryKey) // primary key is modified and not unique
					...
					// non-primary key, need to exclude the current record based
					// on PK
