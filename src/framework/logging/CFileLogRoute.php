 ...
 *
 * @property string $logPath Directory storing log files. Defaults to
 *           application runtime path.
 * @property string $logFile Log file name. Defaults to 'application.log'.
 * @property integer $maxFileSize Maximum log file size in kilo-bytes (KB).
 *           Defaults to 1024 (1MB).
 * @property integer $maxLogFiles Number of files used for rotation. Defaults to
 *           5.
 *          
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class CFileLogRoute extends CLogRoute {
	...
	/**
	 *
	 * @return integer maximum log file size in kilo-bytes (KB). Defaults to
	 *         1024 (1MB).
	 */
	...
	protected function rotateFiles() {
		...
		for($i = $max; $i > 0; -- $i) {
			...
			if (is_file ( $rotateFile )) {
				// suppress errors because it's possible multiple processes
		...
		if (is_file ( $file )) {
			// suppress errors because it's possible multiple processes enter
			// into this section
