 ...
 * messages
 * before they are handled by the route. The default implementation of
 * CLogFilter
 * prepends additional context information to the logged messages. In
 * particular,
...
class CLogFilter extends CComponent implements ILogFilter {
	 ...
	 *
	 * @var boolean whether to prefix each log message with the current user
	 *      {@link CWebUser::name name} and {@link CWebUser::id ID}. Defaults to
	 *      false.
	 */
	public $prefixUser = false;
	/**
	 *
	 * @var boolean whether to log the current user name and ID. Defaults to
	 *      true.
	 */
	public $logUser = true;
	/**
	 ...
	 *      Note that a variable must be accessible via $GLOBALS. Otherwise it
	 *      won't be logged.
	 */
	 ...
	 *      Defaults to `var_export`. If you're experiencing issues with
	 *      circular references
	 *      problem change it to `print_r`. Any kind of callable (static
	 ...
	 * The default implementation will dump user information, system variables,
	 * etc.
	 * 
	 * @return string the context information. If an empty string, it means no
	 *         context information.
	 */
