 ...
 *
 * @property array $logs List of messages. Each array element represents one
 *           message
 ...
 * @property float $executionTime The total time for serving the current
 *           request.
...
class CLogger extends CComponent {
	...
	/**
	 *
	 * @var integer how many messages should be logged before they are flushed
	 *      to destinations.
	 *      Defaults to 10,000, meaning for every 10,000 messages, the {@link
	 *      flush} method will be
	 *      automatically invoked once. If this is 0, it means messages will
	 *      never be flushed automatically.
	 * @since 1.1.0
	 */
	public $autoFlush = 10000;
	/**
	 *
	 * @var boolean this property will be passed as the parameter to {@link
	 *      flush()} when it is
	 *      called in {@link log()} due to the limit of {@link autoFlush} being
	 *      reached.
	 *      By default, this property is false, meaning the filtered messages
	 *      are still kept in the memory
	 *      by each log route after calling {@link flush()}. If this is true,
	 *      the filtered messages
	 *      will be written to the actual medium each time {@link flush()} is
	 *      called within {@link log()}.
	 * @since 1.1.8
	 */
	...
	/**
	 *
	 * @var array log categories for excluding from filtering (used when
	 *      filtering)
	 */
	...
	/**
	 *
	 * @var boolean if we are processing the log or still accepting new log
	 *      messages
	 * @since 1.1.9
	 */
	 ...
	 * A level filter is specified by a list of levels separated by comma or
	 * space
	 ...
	 * with 'system'.
	 *
	 * If you do not specify level filter, it will bring back logs at all
	 * levels.
	 ...
	 * be defined as early as possible (best at the beginning of the entry
	 * script.)
	 * 
	 ...
	 * If no filter is specified, the returned results would be an array with
	 * each element
	 * being array($token,$category,$time).
	 * If a filter is specified, the results would be an array of timings.
	 *
	 * Since 1.1.11, filtering results by category supports the same format used
	 * for filtering logs in
	 * {@link getLogs}, and similarly supports filtering by multiple categories
	 * and wildcard.
	 * 
	 * @param string $token
	 *        	token filter. Defaults to null, meaning not filtered by token.
	 * @param string $categories
	 *        	category filter. Defaults to null, meaning not filtered by
	 *        	category.
	 * @param boolean $refresh
	 *        	whether to refresh the internal timing calculations. If false,
	 *        	only the first time calling this method will the timings be
	 ...
	 * The attached event handlers can process the log messages before they are
	 * removed.
	 * 
	 * @param boolean $dumpLogs
	 *        	whether to process the logs immediately as they are passed to
	 *        	log route
	 * @since 1.1.0
	 */
