 ...
 * <ul>
 * <li>A console application consists of one or several possible user
 * commands;</li>
 * <li>Each user command is implemented as a class extending {@link
 * CConsoleCommand};</li>
 * <li>User specifies which command to run on the command line;</li>
 * <li>The command processes the user request with the specified
 * parameters.</li>
 * </ul>
 *
 * The command classes reside in the directory {@link getCommandPath
 * commandPath}.
 * The name of the class follows the pattern: &lt;command-name&gt;Command, and
 * its
 * file name is the same as the class name. For example, the 'ShellCommand'
 * class defines
 ...
 * </pre>
 *
 * @property string $commandPath The directory that contains the command
 *           classes. Defaults to 'protected/commands'.
...
class CConsoleApplication extends CApplication {
	 ...
	 *      If the former, the string should be the file path of the command
	 *      class.
	 *      If the latter, the array must contain a 'class' element which
	 *      specifies
	 *      the command's class name or {@link YiiBase::getPathOfAlias class
	 *      path alias}.
	 ...
	 * This method uses a console command runner to handle the particular user
	 * command.
	 * Since version 1.1.11 this method will exit application with an exit code
	 * if one is returned by the user command.
	 */
	...
	/**
	 *
	 * @return string the directory that contains the command classes. Defaults
	 ...
	 * 
	 * @return CConsoleCommand null currently active command.
	 * @since 1.1.14
	 */
