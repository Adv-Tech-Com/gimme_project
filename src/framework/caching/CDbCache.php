 ...
 * By setting {@link autoCreateCacheTable} to false, you can also manually
 * create the DB table.
 *
 * CDbCache relies on {@link http://www.php.net/manual/en/ref.pdo.php PDO} to
 * access database.
 * By default, it will use a SQLite3 database under the application runtime
 * directory.
 ...
 * CDbCache.
 *
 * @property integer $gCProbability The probability (parts per million) that
 *           garbage collection (GC) should be performed
 *           when storing a piece of data in the cache. Defaults to 100, meaning
 *           0.01% chance.
...
class CDbCache extends CCache {
	/**
	 *
	 * @var string the ID of the {@link CDbConnection} application component. If
	 *      not set,
	 *      a SQLite3 database will be automatically created and used. The
	...
	/**
	 *
	 * @var string name of the DB table to store cache content. Defaults to
	 *      'YiiCache'.
	 *      Note, if {@link autoCreateCacheTable} is false and you want to
	 *      create the DB table
	 *      manually by yourself, you need to make sure the DB table is of the
	 *      following structure:
	 *      <pre>
	 *      (id CHAR(128) PRIMARY KEY, expire INTEGER, value BLOB)
	 *      </pre>
	 *      Note, some DBMS might not support BLOB type. In this case, replace
	 *      'BLOB' with a suitable
	 ...
	 *      If you already have the table created, it is recommended you set
	 *      this property to be false to improve performance.
	 * @see cacheTableName
	 */
	...
	/**
	 *
	 * @return integer the probability (parts per million) that garbage
	 *         collection (GC) should be performed
	 *         when storing a piece of data in the cache. Defaults to 100,
	 *         meaning 0.01% chance.
	 */
	 ...
	 *
	 * @param integer $value
	 *        	the probability (parts per million) that garbage collection
	 *        	(GC) should be performed
	 *        	when storing a piece of data in the cache. Defaults to 100,
	 *        	meaning 0.01% chance.
	 *        	This number should be between 0 and 1000000. A value 0 meaning
...
EOD;
	 ...
	 * @throws CException if {@link connectionID} does not point to a valid
