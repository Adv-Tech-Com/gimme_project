 ...
 * implementation.
 *
 * A data item can be stored in cache by calling {@link set} and be retrieved
 * back
 * later by {@link get}. In both operations, a key identifying the data item is
 * required.
 * An expiration time and/or a dependency can also be specified when calling
 * {@link set}.
 * If the data item expires or the dependency changes, calling {@link get} will
 * not
...
abstract class CCache extends CApplicationComponent implements ICache, ArrayAccess {
	 ...
	 *      Defaults to null which means
	 *      to use the {@link CApplication::getId() application ID}. If
	 *      different applications need to access the same
	 *      pool of cached data, the same prefix should be set for each of the
	 *      applications explicitly.
	 */
	public $keyPrefix;
	/**
	 *
	 * @var boolean whether to md5-hash the cache key for normalization
	 *      purposes. Defaults to true. Setting this property to false makes
	 *      sure the cache
	 *      key will not be tampered when calling the relevant methods {@link
	 *      get()}, {@link set()}, {@link add()} and {@link delete()}. This is
	 *      useful if a Yii
	 *      application as well as an external application need to access the
	 *      same cache pool (also see description of {@link keyPrefix} regarding
	 *      this use case).
	 *      However, without normalization you should make sure the affected
	 *      cache backend does support the structure (charset, length, etc.) of
	 *      all the provided
	...
	/**
	 *
	 * @var array boolean functions used to serialize and unserialize cached
	 *      data. Defaults to null, meaning
	 *      using the default PHP `serialize()` and `unserialize()` functions.
	 *      If you want to use some more efficient
	 *      serializer (e.g. {@link http://pecl.php.net/package/igbinary
	 *      igbinary}), you may configure this property with
	 *      a two-element array. The first element specifies the serialization
	 *      function, and the second the deserialization
	 *      function. If this property is set false, data will be directly sent
	 *      to and retrieved from the underlying
	 *      cache component without any serialization or deserialization. You
	 *      should not turn off serialization if
	 *      you are using {@link CCacheDependency cache dependency}, because it
	 *      relies on data serialization.
	 */
	public $serializer;
	 ...
	 * @return string a key generated from the provided key which ensures the
	 ...
	 *         cache, expired or the dependency has changed.
	 */
	 ...
	 * cost.
	 * In case a cache does not support this feature natively, it will be
	 ...
	 * @return string boolean value stored in cache, false if the value is not
	 *         in the cache or expired.
	...
	/**
	 * Deletes a value with the specified key from cache
	 * This method should be implemented by child classes to delete the data
	 * from actual cache storage.
	 * 
	 * @param string $key
	 *        	the key of the value to be deleted
	 * @return boolean if no error happens during deletion
	 ...
	 * replaced with the new ones. To add expiration and dependencies, use the
	 * set() method.
	 * This method is required by the interface ArrayAccess.
	 * 
	 * @param string $id
